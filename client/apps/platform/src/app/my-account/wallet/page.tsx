    ...
    <div className="space-y-6">
      ...
      {verifying && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-blue-700">Verifying your payment…</p>
        </div>
      )}
      {verifyMsg && (
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100">
          <h2 className="font-black text-stone-900 text-sm flex items-center gap-2">
            <Icon.PiReceiptBold size={15} className="text-red-700" /> Recent Activity
          </h2>
          {transactions.length > 0 && (
            <button onClick={() => { setShowAll(true); fetchTransactions(1); }}
              className="text-xs font-semibold text-red-700 hover:underline flex items-center gap-1">
              View all <Icon.PiArrowRight size={12} />
            </button>
          )}
        </div>
        <WalletTransactionList items={showAll ? transactions : (wallet?.recent || [])} loading={loading && !wallet} />
      </div>

      {showAll && transactions.length > 0 && (
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-stone-100">
            <h2 className="font-black text-stone-900 text-sm">All Transactions</h2>
          </div>
          ...
          {txTotalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-3 border-t border-stone-100">
              <button disabled={txPage <= 1} onClick={() => fetchTransactions(txPage - 1)}
              ...
              <button disabled={txPage >= txTotalPages} onClick={() => fetchTransactions(txPage + 1)}
