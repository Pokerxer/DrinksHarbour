...
function wasSeenThisSession(id: string): boolean {
  try {
    return JSON.parse(sessionStorage.getItem(SESSION_KEY) || '[]').includes(id);
  } catch {
    return false;
  }
}
...
function getLastSeen(id: string): number | null {
  try {
    return JSON.parse(localStorage.getItem(SEEN_KEY) || '{}')[id] ?? null;
  } catch {
    return null;
  }
}
...
function PopupContent({ banner, onClose }: { banner: BannerData; onClose: () => void }) {
  ...
  const slideDir = position.includes('right')
    ? { x: 60 }
        ...
        <div className="relative p-5">
          ...
          {!imgErr && banner.image?.url && (
            <div className="relative -mx-5 -mt-5 mb-4 h-28 overflow-hidden">
              ...
              />
              {nOverlay > 0 && (
                <div className="absolute inset-0" style={{ backgroundColor: `rgba(0,0,0,${nOverlay})` }} />
              )}
              <div
                className="absolute inset-0"
                style={{
                  background: `linear-gradient(to top, ${bg}, transparent 60%)`
                }}
              />
            </div>
          )}
          {banner.subtitle && (
            <p className="mb-1 text-xs font-bold uppercase tracking-wider" style={{ color: `${nTxt}99` }}>
              {banner.subtitle}
            </p>
          )}
          <h3 className="mb-1 text-base font-black leading-snug" style={{ color: nTxt }}>
            {banner.title}
          </h3>
          {banner.description && (
            <p className="mb-3 text-xs leading-relaxed" style={{ color: `${nTxt}99` }}>
              {banner.description}
            </p>
          )}
          {cfg.collectEmail ? (
            ...
            banner.ctaText && (
              <Link
                ...
                className={cn(
                  'mt-3 block w-full rounded-xl px-4 py-2 text-center text-sm font-bold transition-all',
                  nCta
                )}
              >
                {banner.ctaText}
          ...
          >
              ...
              <div className="relative h-72 md:h-96">
                ...
                {fsOverlay > 0 && (
                  <div
                    className="absolute inset-0"
                    style={{
                      backgroundColor: `rgba(0,0,0,${Math.min(fsOverlay, 0.35)})`
                    }}
                  />
                )}
                <div
            ...
            <div className="p-8 text-center md:p-10">
              {cfg.showCloseButton !== false && imgErr && (
                <button
                  onClick={onClose}
                  className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-all hover:bg-white/20 hover:scale-110"
                >
                  <Icon.PiXBold size={16} />
                </button>
              )}
              {banner.subtitle && (
                <span
                  className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider mb-4"
                  style={{ color: `${fsTxt}bb` }}
                >
                  <Icon.PiSparkleFill size={10} className="text-amber-400" />
                  {banner.subtitle}
                </span>
              )}
              <h2 className="text-3xl font-black leading-tight md:text-4xl" style={{ color: fsTxt }}>
                {banner.title}
              </h2>
              {banner.description && (
                <p className="mx-auto mt-2 max-w-md text-base leading-relaxed" style={{ color: `${fsTxt}99` }}>
                  {banner.description}
                </p>
              )}
              ...
              {cfg.couponCode && !cfg.collectEmail && (
                <div className="inline-flex items-center gap-3 rounded-2xl border border-white/20 bg-white/10 px-5 py-3 my-4">
                  ...
                  <span className="font-black tracking-widest text-lg" style={{ color: fsTxt }}>
                    {cfg.couponCode}
                  </span>
                </div>
              )}
              {cfg.collectEmail ? (
                ...
                banner.ctaText && (
                  <Link
                    ...
                    target={banner.linkType === 'external' ? '_blank' : undefined}
                    className={cn(
                      'inline-flex items-center gap-2 rounded-2xl px-10 py-4 text-sm font-black transition-all mt-4',
                      fsCta
                    )}
                  >
              ...
              <button
                onClick={onClose}
                className="mx-auto mt-4 block text-xs transition-colors hover:underline"
                style={{ color: `${fsTxt}55` }}
              >
                No thanks, I&apos;ll skip
              </button>
            </div>
  ...
  if (variant === 'slide_in') {
    ...
    return (
      <motion.div
        ...
        transition={transition}
        className={cn(
          'fixed z-[9999] w-80 max-w-[calc(100vw-2rem)] rounded-2xl shadow-2xl overflow-hidden sm:w-96',
          posClass
        )}
        style={{ backgroundColor: bg }}
      >
        {!imgErr && banner.image?.url && (
          <div className="relative h-36 overflow-hidden">
            ...
            />
            {siOverlay > 0 && (
              <div className="absolute inset-0" style={{ backgroundColor: `rgba(0,0,0,${siOverlay})` }} />
            )}
            <div
              className="absolute inset-0"
              style={{
                background: `linear-gradient(to top, ${bg}, transparent 50%)`
              }}
            />
          </div>
        )}
        <div className="relative p-5">
          ...
          {banner.subtitle && (
            <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: `${siTxt}99` }}>
              {banner.subtitle}
            </p>
          )}
          <h3 className="pr-8 text-base font-black leading-tight" style={{ color: siTxt }}>
            {banner.title}
          </h3>
          {banner.description && (
            <p className="mt-1 text-xs leading-relaxed" style={{ color: `${siTxt}99` }}>
              {banner.description}
            </p>
          )}
          {banner.endDate && <MiniCountdown endDate={banner.endDate} />}
          {cfg.couponCode && !cfg.collectEmail && (
            <div className="my-3 flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 px-3 py-2">
              ...
              <span className="text-sm font-black tracking-widest" style={{ color: siTxt }}>
                {cfg.couponCode}
              </span>
            </div>
          )}
          {cfg.collectEmail ? (
            ...
            banner.ctaText && (
              <Link
                ...
                target={banner.linkType === 'external' ? '_blank' : undefined}
                className={cn(
                  'mt-3 block w-full rounded-xl px-4 py-2.5 text-center text-sm font-bold transition-all',
                  siCta
                )}
              >
                {banner.ctaText} →
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Image-only artwork does not need a second empty content panel. */}
          {hasOverlayContent && (
            <div className={cn('flex flex-col gap-2.5 p-7 md:p-8', posCls)}>
              {banner.subtitle && (
                <span
                  className="inline-flex w-fit items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider"
              ...
              {cfg.couponCode && !cfg.collectEmail && (
                <div className="inline-flex w-fit items-center gap-3 rounded-2xl border border-white/20 bg-white/10 px-5 py-2.5 my-2">
                  ...
                  <span className="font-black tracking-widest" style={{ color: txtColor }}>
                    {cfg.couponCode}
                  </span>
                </div>
              )}
              ...
              {cfg.collectEmail ? (
                ...
                <Link
                  ...
                  target={banner.linkType === 'external' ? '_blank' : undefined}
                  className={cn(
                    'inline-flex w-fit items-center gap-2 rounded-2xl px-8 py-3.5 text-sm font-black transition-all',
                    ctaCls
                  )}
                >
...
export default function PopupBanner({ allowedPaths, preview }: PopupBannerProps) {
  ...
  useEffect(() => {
    ...
    queue.forEach((banner) => {
      ...
      const show = () => {
        ...
        setVisible(banner);
        // Track impression
        fetch(`${API_URL}/api/banners/${banner._id}/impression`, {
          method: 'POST'
        }).catch(() => {});
      };
  ...
  return (
    <AnimatePresence mode="wait">
      {visible && <PopupContent key={visible._id} banner={visible} onClose={handleClose} />}
    </AnimatePresence>
  );
}
