...
describe('tenantFormSchema', () => {
  ...
  it.each([
    ...
    'venue',
    'custom',
  ])('accepts the %s plan that the Tenant model allows', (plan) => {
    expect(tenantFormSchema.safeParse({ ...valid, plan }).success).toBe(true);
  });

  it('rejects a plan the model does not define', () => {
    expect(
      tenantFormSchema.safeParse({ ...valid, plan: 'platinum' }).success
    ).toBe(false);
  });
  ...
  it('requires a rejection reason when the status is rejected', () => {
    const missing = tenantFormSchema.safeParse({
      ...valid,
      status: 'rejected',
      rejectionReason: '  ',
    });
    ...
    expect(
      tenantFormSchema.safeParse({
        ...valid,
        status: 'rejected',
        rejectionReason: 'Incomplete CAC docs',
      }).success
    ).toBe(true);
  });
  ...
  it('does not demand a rejection reason for other statuses', () => {
    expect(
      tenantFormSchema.safeParse({ ...valid, status: 'approved' }).success
    ).toBe(true);
  });
  ...
  it('requires a NAFDAC number only when NAFDAC applies', () => {
    expect(
      tenantFormSchema.safeParse({ ...valid, nafdacRequired: true }).success
    ).toBe(false);
    expect(
      tenantFormSchema.safeParse({
        ...valid,
        nafdacRequired: true,
        nafdacNumber: 'A1-2345',
      }).success
    ).toBe(true);
    expect(
      tenantFormSchema.safeParse({ ...valid, nafdacRequired: false }).success
    ).toBe(true);
  });
  ...
  it('validates CAC, TIN and account number formats but allows blanks', () => {
    expect(
      tenantFormSchema.safeParse({ ...valid, cacNumber: 'RC1234567' }).success
    ).toBe(true);
    expect(
      tenantFormSchema.safeParse({ ...valid, cacNumber: '' }).success
    ).toBe(true);
    expect(
      tenantFormSchema.safeParse({ ...valid, cacNumber: 'nope' }).success
    ).toBe(false);

    expect(
      tenantFormSchema.safeParse({ ...valid, tin: '1234567890' }).success
    ).toBe(true);
    expect(tenantFormSchema.safeParse({ ...valid, tin: '123' }).success).toBe(
      false
    );

    expect(
      tenantFormSchema.safeParse({ ...valid, bankAccountNumber: '0123456789' })
        .success
    ).toBe(true);
    expect(
      tenantFormSchema.safeParse({ ...valid, bankAccountNumber: '123' }).success
    ).toBe(false);
  });

  it('rejects a malformed slug', () => {
    expect(
      tenantFormSchema.safeParse({ ...valid, slug: 'Acme Liquors' }).success
    ).toBe(false);
    expect(
      tenantFormSchema.safeParse({ ...valid, slug: 'acme-liquors-2' }).success
    ).toBe(true);
  });
});
