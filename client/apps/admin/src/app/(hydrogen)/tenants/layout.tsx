import ModuleNavHeader from '@/app/shared/module-nav-header';
import { TenantBrandTheme } from '@/app/shared/ecommerce/tenant/tenant-brand-theme';
import { PiStorefrontDuotone, PiPlusCircleDuotone } from 'react-icons/pi';

/**
 * POS-style module chrome: nav header + content canvas. Responsive rules
 * (label hiding / wrapping) live in ModuleNavHeader.
 */
export default function TenantsLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <TenantBrandTheme className="-mx-4 -mt-2 flex flex-col md:-mx-5 lg:-mx-6 3xl:-mx-8">
      <div className="px-4 md:px-5 lg:px-6 3xl:px-8">
        <ModuleNavHeader
          brand="Tenants"
          tabs={[
            { label: 'Tenants', href: '/tenants', icon: <PiStorefrontDuotone /> },
            { label: 'New Tenant', href: '/tenants/create', icon: <PiPlusCircleDuotone /> },
          ]}
        />
      </div>
      <div className="flex-1 px-4 pb-10 pt-6 md:px-10 lg:px-14">{children}</div>
    </TenantBrandTheme>
  );
}
