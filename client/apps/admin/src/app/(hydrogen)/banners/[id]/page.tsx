...
const PRIORITY_CONFIG: Record<string, { label: string; bg: string; text: string; dot: string; ring: string }> = {
  low:    { label: 'Low',    bg: 'bg-gray-100',   text: 'text-gray-600',   dot: 'bg-gray-400',   ring: 'ring-gray-200' },
  medium: { label: 'Medium', bg: 'bg-amber-50',   text: 'text-amber-700',  dot: 'bg-amber-500',  ring: 'ring-amber-200' },
  high:   { label: 'High',   bg: 'bg-orange-50',  text: 'text-orange-700', dot: 'bg-orange-500', ring: 'ring-orange-200' },
  urgent: { label: 'Urgent', bg: 'bg-red-50',     text: 'text-red-700',    dot: 'bg-red-500',    ring: 'ring-red-200' },
};
...
function PriorityBadge({ priority, size = 'md' }: { priority?: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full font-semibold ${cfg.bg} ${cfg.text} ring-1 ${cfg.ring} ${pad}`}>
...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; dot: string }> = {
  ...
  paused:    { label: 'Paused',    bg: 'bg-orange-50',  text: 'text-orange-700', dot: 'bg-orange-500' },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${cfg.bg} ${cfg.text} ring-1 ring-current/10`}>
      <span className={`h-1.5 w-1.5 rounded-full ${cfg.dot}`} />
      {cfg.label}
    </span>
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl ${accent || 'bg-gray-50 text-gray-500'}`}>
          {icon}
        </div>
...
function InfoRow({ label, value, badge }: { label: string; value?: string; badge?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 py-2 border-b border-gray-50 last:border-0">
      ...
      <dd className="flex items-center gap-2 text-sm font-medium text-gray-900 truncate">
        {badge || (value || '—')}
      </dd>
    </div>
  );
...
export default function BannerDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleClone = async () => {
    try {
      ...
      router.push(routes.eCommerce.editBanner(res.data?.banner?._id || res.data?._id));
    } catch (err: any) {
  ...
  const typeLabel = BANNER_TYPE_OPTIONS.find(t => t.value === banner.type)?.label || banner.type;
  const placementLabel = BANNER_PLACEMENT_OPTIONS.find(p => p.value === banner.placement)?.label || banner.placement;
  const ctaStyleLabel = BANNER_CTA_STYLE_OPTIONS.find(c => c.value === banner.ctaStyle)?.label || banner.ctaStyle;
  ...
  return (
    <>
      ...
      >
        <div className="mt-4 flex items-center gap-2 @lg:mt-0 flex-wrap">
          ...
          {(isActive || isPaused) && (
            ...
            >
              {isActive ? <><PiPauseBold className="mr-1.5 h-4 w-4" />Pause</> : <><PiPlayBold className="mr-1.5 h-4 w-4" />Activate</>}
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={handleClone}>
            <PiCopyBold className="mr-1.5 h-4 w-4" />Clone
          </Button>
          ...
          <Button size="sm" onClick={handleDelete} className="bg-red-500 hover:bg-red-600 text-white border-0">
            <PiTrashBold className="mr-1.5 h-4 w-4" />Delete
          </Button>
        </div>
      </PageHeader>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            <Card
              icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiTagBold className="h-4 w-4" /></span>}
              title="Banner Details"
            >
              <dl>
                <InfoRow label="Type" badge={<span className="capitalize">{typeLabel}</span>} />
                <InfoRow label="Placement" badge={<span className="capitalize">{placementLabel}</span>} />
                <InfoRow label="Priority" badge={<PriorityBadge priority={banner.priority} size="sm" />} />
            ...
            {/* CTA card */}
            <Card
              icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiArrowRightBold className="h-4 w-4" /></span>}
              title="Call to Action"
            >
              <dl>
                <InfoRow label="CTA Text" value={banner.ctaText || '—'} />
                <InfoRow label="CTA Style" badge={<span className="capitalize">{ctaStyleLabel}</span>} />
                <InfoRow label="Link Type" value={banner.linkType ? String(banner.linkType).replace(/_/g, ' ') : '—'} />
              </dl>
              {banner.ctaLink && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2">
                  ...
                  <a
                    href={banner.ctaLink.startsWith('http') ? banner.ctaLink : `https://drinksharbour.com${banner.ctaLink}`}
                    target="_blank"
              ...
              {banner.ctaText && (
                <div className="mt-4 border-t border-gray-100 pt-4">
                  <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-gray-400">Preview</p>
                  <span className={`inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-bold ${ctaCls}`}>
                    {banner.ctaText}
            ...
            {/* Styling card */}
            <Card
              icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiSparkleBold className="h-4 w-4" /></span>}
              title="Styling"
            >
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Background</p>
                  <div className="flex items-center gap-2">
                    <span className="h-8 w-8 rounded-lg border border-gray-200" style={{ backgroundColor: banner.backgroundColor || '#fff' }} />
                    <code className="text-sm text-gray-700">{banner.backgroundColor || '#FFFFFF'}</code>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Text Color</p>
                  <div className="flex items-center gap-2">
                    <span className="h-8 w-8 rounded-lg border border-gray-200" style={{ backgroundColor: banner.textColor || '#000' }} />
                    <code className="text-sm text-gray-700">{banner.textColor || '#000000'}</code>
                  </div>
                </div>
              </div>
              <dl className="mt-4">
                <InfoRow label="Text Alignment" value={banner.textAlignment ? String(banner.textAlignment).charAt(0).toUpperCase() + banner.textAlignment.slice(1) : '—'} />
                <InfoRow label="Overlay Opacity" value={`${banner.overlayOpacity ?? 0}%`} />
              </dl>
              {/* Content position visual indicator */}
              <div className="mt-4 flex items-center gap-4 border-t border-gray-100 pt-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Content Position</p>
                  <p className="text-sm font-medium text-gray-900 capitalize">{String(banner.contentPosition || 'center').replace(/-/g, ' ')}</p>
                </div>
            ...
            {banner.mobileImage?.url && (
              <Card
                icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiDeviceMobileBold className="h-4 w-4" /></span>}
                title="Mobile Image"
              >
                <div className="relative mx-auto w-32 overflow-hidden rounded-xl border border-gray-200 bg-gray-50" style={{ aspectRatio: '16/9' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={banner.mobileImage.url} alt="Mobile banner" className="h-full w-full object-cover" />
                </div>
                {banner.mobileImage?.alt && (
                  <p className="mt-2 text-center text-xs text-gray-400">{banner.mobileImage.alt}</p>
                )}
              </Card>
            )}
            ...
            {(banner.description || banner.subtitle) && (
              <Card
                icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiImageBold className="h-4 w-4" /></span>}
                title="Content"
              >
                {banner.subtitle && <p className="text-sm font-semibold text-gray-800 mb-1">{banner.subtitle}</p>}
                {banner.description && <p className="text-sm text-gray-500 leading-relaxed">{banner.description}</p>}
              </Card>
            )}

            {banner.notes && (
              <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-5">
                <h3 className="mb-2 text-sm font-semibold text-amber-900">Internal Notes</h3>
          ...
          <div className="space-y-4">
            {/* Schedule */}
            <Card
              icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiCalendarBold className="h-4 w-4" /></span>}
              title="Schedule"
            >
              <dl>
                <InfoRow label="Scheduled" badge={banner.isScheduled ? <span className="text-amber-600">Yes</span> : <span className="text-gray-500">No</span>} />
                {banner.startDate && <InfoRow label="Start" value={new Date(banner.startDate).toLocaleDateString()} />}
                {banner.endDate && <InfoRow label="End" value={new Date(banner.endDate).toLocaleDateString()} />}
                {banner.daysUntilExpiration != null && (
                  <InfoRow
                    label="Expires in"
                    badge={
                      <span className={banner.daysUntilExpiration <= 3 ? 'text-red-600 font-bold' : 'text-gray-900'}>
            ...
            {/* Timestamps */}
            <Card
              icon={<span className="flex h-5 w-5 items-center justify-center text-gray-500"><PiClockBold className="h-4 w-4" /></span>}
              title="Timestamps"
            >
              <dl>
                {banner.createdAt && <InfoRow label="Created" value={new Date(banner.createdAt).toLocaleDateString()} />}
                {banner.updatedAt && <InfoRow label="Updated" value={new Date(banner.updatedAt).toLocaleDateString()} />}
                {banner.publishedAt && <InfoRow label="Published" value={new Date(banner.publishedAt).toLocaleDateString()} />}
              </dl>
            </Card>
          </div>
        </div>
