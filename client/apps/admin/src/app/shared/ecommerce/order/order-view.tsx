...
import {
  ...
  PiWalletBold, PiArrowBendUpLeftBold, PiPrinterBold, PiStorefrontBold,
  PiPauseCircleBold, PiTicketBold, PiMapPinBold, PiUserBold, PiProhibitBold,
} from 'react-icons/pi';
...
const TERMINAL_STATES: Record<string, { label: string; tone: string; Icon: React.ElementType; tsKey?: string }> = {
  cancelled: { label: 'Order Cancelled', tone: 'text-red-600',  Icon: PiXCircleBold,     tsKey: 'cancelledAt' },
  refunded:  { label: 'Order Refunded',  tone: 'text-blue-600', Icon: PiArrowBendUpLeftBold },
...
function getStatusIndex(status: string) {
  ...
  if (status === 'partially_shipped') return STATUS_STEPS.findIndex((s) => s.key === 'shipped');
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-gray-0 p-6 shadow-2xl">{children}</div>
    </div>
  );
}

function ConfirmModal({
  title, message, confirmLabel, danger, withReason, reasonPlaceholder, loading, onConfirm, onClose,
}: {
  title: string;
  message: string;
  confirmLabel: string;
    ...
    <Modal onClose={onClose}>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button
          ...
          isLoading={loading}
          className={danger ? 'border-0 bg-red-500 text-white hover:bg-red-600' : ''}
        >
          {confirmLabel}
        </Button>
      </div>
...
function StatusStepper({ order, onUpdate }: { order: Order; onUpdate: (o: Order) => void }) {
    ...
    <div>
      {terminal ? (
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <p className={cn('text-sm font-semibold', terminal.tone)}>{terminal.label}</p>
          {terminal.tsKey && shortDate((order as any)[terminal.tsKey]) && (
            <p className="mt-0.5 text-xs text-gray-400">{shortDate((order as any)[terminal.tsKey])}</p>
          )}
          {order.cancelReason && order.status === 'cancelled' && (
            <p className="mt-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">{order.cancelReason}</p>
          )}
          ...
          <div className="mt-4 w-full border-t border-muted pt-3 text-start">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-gray-400">History</p>
              ...
              <div key={step.key} className="relative flex gap-3 pb-5 last:pb-0">
                ...
                <div
                  className={cn(
                    ...
                    isActive    && 'border-primary bg-gray-0 text-primary shadow-md shadow-primary/20',
                    isFuture    && 'border-muted bg-gray-50 text-gray-300'
                  )}
                >
                  {isCompleted ? <PiCheckBold className="h-4 w-4" /> : <step.Icon className="h-4 w-4" />}
                </div>

                <div className="flex-1 pt-1">
                  <p className={cn('text-sm font-semibold leading-tight', isCompleted || isActive ? 'text-gray-900' : 'text-gray-400')}>
                    {step.label}
                    {isActive && (
                  ...
                  {ts ? (
                    ...
                    <p className={cn('mt-0.5 text-[11px]', isFuture ? 'text-gray-300' : 'text-gray-400')}>{step.description}</p>
                  )}
                </div>
              </div>
            );
      ...
      {(nextStatus || canCancel) && (
        <div className="mt-5 space-y-2 border-t border-muted pt-4 print:hidden">
          {nextStatus && (
            <Button className="w-full gap-2" onClick={() => setModal('advance')} disabled={busy}>
...
function WidgetCard({
  title, className, children, childrenWrapperClass,
}: {
  title?: string;
  className?: string;
...
function Row({ label, value, tone }: { label: string; value: React.ReactNode; tone?: string }) {
  return (
    <div className="flex justify-between gap-4 text-xs">
      ...
      <span className={cn('text-end font-medium text-gray-700', tone)}>{value}</span>
    </div>
  );
}
...
const PAY_STATUS_STYLE: Record<string, string> = {
  ...
  partially_refunded: 'bg-purple-500/10 text-purple-600 ring-purple-500/20 dark:text-purple-400',
};
...
function PaymentBadge({ status }: { status: string }) {
  return (
    <span className={cn(
      ...
      PAY_STATUS_STYLE[status] ?? 'bg-gray-500/10 text-gray-600 ring-gray-500/20'
    )}>
      {humanize(status)}
    </span>
  );
    ...
    <div className="@container animate-pulse">
      ...
      <div className="@5xl:grid @5xl:grid-cols-12 @5xl:gap-7 @6xl:grid-cols-10 @7xl:gap-10">
        ...
        <div className="space-y-4 pt-8 @5xl:col-span-4 @5xl:pt-0 @6xl:col-span-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-32 rounded bg-gray-100" />)}
        </div>
      </div>
    </div>
  );
...
const METHOD_META: Record<string, { label: string; Icon: React.ElementType; color: string }> = {
  cash_on_delivery: { label: 'Cash on Delivery', Icon: PiHandCoinsBold,    color: 'text-orange-600 bg-orange-500/10' },
  cash:             { label: 'Cash',             Icon: PiHandCoinsBold,    color: 'text-orange-600 bg-orange-500/10' },
  bank_transfer:    { label: 'Bank Transfer',    Icon: PiBankBold,         color: 'text-blue-600 bg-blue-500/10' },
  card:             { label: 'Card Payment',     Icon: PiCreditCardBold,   color: 'text-violet-600 bg-violet-500/10' },
  mobile_money:     { label: 'Mobile Money',     Icon: PiDeviceMobileBold, color: 'text-green-600 bg-green-500/10' },
  wallet:           { label: 'Wallet',           Icon: PiWalletBold,       color: 'text-indigo-600 bg-indigo-500/10' },
  split:            { label: 'Split Payment',    Icon: PiCreditCardBold,   color: 'text-purple-600 bg-purple-500/10' },
};

function PaymentPanel({ order, onUpdate }: { order: Order; onUpdate: (o: Order) => void }) {
  const { data: session } = useSession();
  ...
  const refundInvalid = refundAmt !== '' && (!Number.isFinite(refundAmtNum) || refundAmtNum <= 0 || refundAmtNum > refundable);
  ...
  return (
    <div>
      ...
      {(ref || txId || paidAt || channel || splits.length > 0 || order.paymentDetails?.notes || order.paymentDetails?.failureReason) && (
        <div className="mt-3 space-y-1.5 rounded-xl border border-muted px-4 py-3">
          {ref && <Row label="Reference" value={<span className="font-mono">{ref}</span>} />}
          {txId && <Row label="Transaction ID" value={<span className="break-all font-mono">{txId}</span>} />}
          {channel && <Row label="Channel" value={humanize(channel)} />}
          {paidAt && <Row label="Paid at" value={shortDate(paidAt)} />}
          {splits.map((s, i) => (
            <Row key={i} label={`Split · ${humanize(s.method)}`} value={formatCurrency(s.amount, order.currency)} />
          ))}
          {typeof order.paymentDetails?.change === 'number' && order.paymentDetails.change > 0 && (
            <Row label="Change given" value={formatCurrency(order.paymentDetails.change, order.currency)} />
          )}
          {order.paymentDetails?.notes && <Row label="Notes" value={order.paymentDetails.notes} />}
          {order.paymentDetails?.failureReason && (
            <Row label="Failure reason" value={order.paymentDetails.failureReason} tone="text-red-600" />
          )}
        </div>
      )}
      ...
      {order.refundDetails?.amount ? (
        <div className="mt-3 space-y-1.5 rounded-xl border border-blue-500/20 bg-blue-500/5 px-4 py-3">
          <p className="mb-1 text-xs font-semibold text-blue-600 dark:text-blue-400">Refund Details</p>
          <Row label="Amount" value={formatCurrency(order.refundDetails.amount, order.currency)} />
          {order.refundDetails.reason && <Row label="Reason" value={order.refundDetails.reason} />}
          {order.refundDetails.createdAt && <Row label="Processed" value={shortDate(order.refundDetails.createdAt)} />}
        </div>
      ) : null}
      ...
      {alreadyRefunded > 0 && (
        <div className="mt-3 flex justify-between rounded-xl bg-gray-50 px-4 py-2.5 text-xs">
          ...
          <span className="font-semibold text-gray-900">
            {formatCurrency(alreadyRefunded, order.currency)} · {formatCurrency(refundable, order.currency)} remaining
          </span>
        </div>
      )}
        ...
        <div className="mt-4 space-y-2 border-t border-muted pt-4 print:hidden">
          {canMarkPaid && (
            <Button className="w-full gap-2" onClick={() => setModal('paid')} disabled={busy}>
              ...
              {isCOD ? 'Mark Cash Received' : isBank ? 'Verify & Mark Paid' : 'Mark as Paid'}
            </Button>
          )}
      ...
      {modal === 'paid' && (
        <Modal onClose={closeModal}>
          ...
          <Textarea value={notesField} onChange={(e) => setNotesField(e.target.value)} placeholder="Notes (optional)" className="mb-4" rows={2} />
          <div className="flex justify-end gap-3">
      ...
      {modal === 'failed' && (
        <Modal onClose={closeModal}>
          ...
          <p className="mb-4 text-sm text-gray-500">This flags the payment as failed. The order status stays unchanged.</p>
          <Textarea value={notesField} onChange={(e) => setNotesField(e.target.value)} placeholder="Reason for failure (optional)" className="mb-4" rows={2} />
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={closeModal} disabled={busy}>Cancel</Button>
            <Button className="border-0 bg-red-500 text-white hover:bg-red-600" onClick={() => doAction('mark_failed', { notes: notesField || undefined })} isLoading={busy}>
              Mark Failed
            </Button>
          </div>
        </Modal>
      ...
      {modal === 'refund' && (
        <Modal onClose={closeModal}>
          ...
          <p className="mb-4 text-sm text-gray-500">
            Up to {formatCurrency(refundable, order.currency)} can still be refunded on this order. Leave the amount blank for a full refund.
          </p>
          <input
            type="number"
            value={refundAmt}
            ...
            className={cn(
              ...
              refundInvalid ? 'border-red-400' : 'border-muted focus:border-primary'
            )}
          />
          {refundInvalid && (
          ...
          <Textarea value={notesField} onChange={(e) => setNotesField(e.target.value)} placeholder="Reason for refund" className="mb-4 mt-2" rows={2} />
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={closeModal} disabled={busy}>Cancel</Button>
            <Button
        ...
        {refunds.map((r, i) => (
          <div key={i} className="rounded-xl border border-muted px-4 py-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <span className="font-mono text-xs font-semibold text-gray-900">{r.receiptNumber ?? `Refund ${i + 1}`}</span>
              <span className="text-sm font-bold text-blue-600 dark:text-blue-400">
                {formatCurrency(r.totalRefunded, order.currency)}
              </span>
            </div>
            <div className="space-y-1">
              {shortDate(r.refundedAt) && <Row label="Processed" value={shortDate(r.refundedAt)} />}
              {r.paymentMethod && <Row label="Method" value={humanize(r.paymentMethod)} />}
              {r.reason && <Row label="Reason" value={r.reason} />}
            </div>
            {!!r.items?.length && (
              <ul className="mt-2 space-y-1 border-t border-muted pt-2">
                {r.items.map((line, li) => {
                  const product = order.items[line.orderItemIndex ?? -1]?.product?.name;
                  return (
                    <li key={li} className="flex justify-between gap-3 text-xs">
                      <span className="text-gray-500">
                        {line.quantity ?? 0} × {product ?? `Item ${(line.orderItemIndex ?? 0) + 1}`}
                        {line.restock === false && <span className="ms-1 text-orange-500">(not restocked)</span>}
                      </span>
                      <span className="font-medium text-gray-700">{formatCurrency(line.amount, order.currency)}</span>
                    </li>
                  );
                })}
              </ul>
...
function resolveCustomer(order: Order) {
  ...
  if (addr?.fullName || addr?.email || addr?.phone) {
    return {
      name:  addr.fullName || (order.user ? `${order.user.firstName} ${order.user.lastName}` : '—'),
...
export default function OrderView({
  ...
  useEffect(() => {
    ...
    orderService
      ...
      .catch((e: any) => !cancelled && setError(e?.message ?? 'Could not load this order.'))
  ...
  const eta = ship?.daysMin != null && ship?.daysMax != null
    ...
    <div className="@container">
      {order.isVoided && (
        <div className="mb-5 flex items-start gap-3 rounded-xl border border-red-500/30 bg-red-500/5 px-4 py-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-600">This order was voided</p>
            <p className="text-xs text-gray-500">
              {[shortDate(order.voidedAt), order.voidReason].filter(Boolean).join(' · ') || 'No reason recorded.'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-center border-b border-t border-muted py-4 font-medium text-gray-700 @5xl:justify-start">
        ...
        <span className="my-2 ms-2"><PaymentBadge status={order.paymentStatus} /></span>
        <button
          type="button"
      ...
      <div className="items-start pt-10 @5xl:grid @5xl:grid-cols-12 @5xl:gap-7 @6xl:grid-cols-10 @7xl:gap-10">
        ...
        <div className="space-y-7 @5xl:col-span-8 @5xl:space-y-10 @6xl:col-span-7">
            ...
            <div className="overflow-x-auto rounded-lg border border-muted">
              <table className="w-full min-w-[600px] text-sm">
                <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-500">
                  <tr>
                    <th scope="col" className="px-5 py-3 text-left">Product</th>
                    <th scope="col" className="px-5 py-3 text-right">Unit Price</th>
                        ...
                        <div className="flex items-center gap-3">
                          {item.product?.images?.[0]?.url ? (
                            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md">
                              <Image src={item.product.images[0].url} alt="" fill sizes="40px" className="object-cover" />
                            </div>
                          ) : (
                          ...
                          <div>
                            ...
                            {item.subproduct?.name && <p className="text-xs text-gray-500">{item.subproduct.name}</p>}
                            {(item.size?.displayName || item.size?.size) && (
                              <p className="text-xs text-gray-500">Size: {item.size.displayName || item.size.size}</p>
                            )}
            ...
            {vendors.length > 0 && (
              <div className="mt-6">
                ...
                <div className="overflow-hidden rounded-xl border border-muted">
                  {vendors.map((v, i) => (
                    <div key={i} className="flex items-center justify-between border-b border-muted px-4 py-3 last:border-0 hover:bg-gray-50">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {v.items} item{v.items === 1 ? '' : 's'} · revenue {formatCurrency(v.revenue, order.currency)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-blue-600 dark:text-blue-400">{formatCurrency(v.payout, order.currency)}</p>
          ...
          <div>
            <Title as="h3" className="mb-3.5 text-base font-semibold @5xl:mb-5 @7xl:text-lg">Payment</Title>
        ...
        {/* Right column */}
        <div className="@container space-y-7 pt-8 @5xl:col-span-4 @5xl:space-y-10 @5xl:pt-0 @6xl:col-span-3">
          ...
          {addr && (addr.addressLine1 || addr.city) && (
            <WidgetCard title="Shipping Address" childrenWrapperClass="py-5 @5xl:py-6">
              <div className="space-y-2.5">
                <Field label="Address" value={[addr.addressLine1, addr.addressLine2].filter(Boolean).join(', ')} />
                ...
                <Field label="Instructions" value={addr.additionalInstructions} />

                {(order.shippingMethod || ship?.zoneLabel || eta || ship?.distanceKm != null) && (
                  <div className="space-y-2.5 border-t border-muted pt-2.5">
                    <Field label="Shipping Method" value={order.shippingMethod ? humanize(order.shippingMethod) : undefined} />
                    <Field label="Delivery Zone" value={ship?.zoneLabel ?? undefined} />
                    <Field label="Est. Delivery" value={eta ?? undefined} />
                    <Field
                      label="Distance"
                      value={ship?.distanceKm != null ? `${ship.distanceKm.toFixed(1)} km${ship.stops ? ` · ${ship.stops} stop${ship.stops === 1 ? '' : 's'}` : ''}` : undefined}
                    />
                    {ship?.isFree && (
                      <span className="inline-flex items-center gap-1 rounded-full bg-green-500/10 px-2 py-0.5 text-[11px] font-medium text-green-600 dark:text-green-400">
                        Free delivery
                    ...
                    {addr.coordinates?.latitude != null && addr.coordinates?.longitude != null && (
                      <a
          ...
          {billing && (billing.addressLine1 || billing.city) && (
            <WidgetCard title="Billing Address" childrenWrapperClass="py-5 @5xl:py-6">
              <div className="space-y-2.5">
                <Field label="Name" value={billing.fullName} />
                <Field label="Address" value={[billing.addressLine1, billing.addressLine2].filter(Boolean).join(', ')} />
                <div className="grid grid-cols-2 gap-2">
          ...
          <WidgetCard title="Order Info" childrenWrapperClass="py-5 space-y-2">
            ...
            {order.receiptNumber && <Row label="Receipt #" value={<span className="font-mono">{order.receiptNumber}</span>} />}
            <Row label="Source" value={humanize(order.source ?? 'web')} />
            <Row label="Placed" value={longDate(order.placedAt) ?? longDate(order.createdAt) ?? '—'} />
            <Row label="Currency" value={order.currency} />
            {order.posStaff && (
              <Row
                label="Cashier"
                value={order.posStaff.posName || `${order.posStaff.firstName ?? ''} ${order.posStaff.lastName ?? ''}`.trim() || order.posStaff.email}
              />
            )}
            {order.appliedPricelist?.pricelistName && (
              <Row label="Pricelist" value={order.appliedPricelist.pricelistName} />
            )}
          </WidgetCard>
        </div>
      </div>
