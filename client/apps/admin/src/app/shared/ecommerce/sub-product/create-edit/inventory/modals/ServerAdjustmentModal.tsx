            ...
            <div>
              ...
              <div className="flex overflow-hidden rounded-lg border border-gray-200 focus-within:border-gray-400">
                <button
                  ...
                  onClick={() =>
                    setQuantity((v) =>
                      String(Math.max(1, parseInt(v || '1', 10) - 1))
                    )
                  }
                ...
                <button
                  type="button"
                  onClick={() =>
                    setQuantity((v) => String(parseInt(v || '0', 10) + 1))
                  }
                ...
                >
                  ...
                  {sizes.map((s) => (
                    <option key={s._id || s.size} value={s._id || s.size}>
                      ...
                      {(s.stockQuantity ?? s.stock) !== undefined
                        ? ` (${s.stockQuantity ?? s.stock})`
                        : ''}
                    </option>
                  ))}
                </select>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  ...
                  {warehouses
                    .filter((w) => w.isActive && w._id !== toWh)
                    .map((w) => (
              ...
              <div>
                ...
                >
                  <option value="">Select…</option>
                  {warehouses
                    .filter((w) => w.isActive && w._id !== fromWh)
                    .map((w) => (
                      <option key={w._id} value={w._id}>
                        {w.name}
                        {w.code ? ` (${w.code})` : ''}
            ...
            <input
              ...
              placeholder={
                type === 'received'
                  ? 'e.g. Monthly restock'
                  : type === 'shipped'
                    ? 'e.g. Delivered to customer'
                    : type === 'adjustment_in'
                      ? 'e.g. Physical count correction'
                      : type === 'adjustment_out'
                        ? 'e.g. Shrinkage, counting error'
                        : type === 'return'
                          ? 'e.g. Customer changed mind'
                          : type === 'transfer'
                            ? 'e.g. Relocating slow stock'
                            : type === 'damaged'
                              ? 'e.g. Broken during handling'
        ...
        <div className="flex items-center justify-between border-t border-gray-100 px-5 py-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!canSubmit}
              className="flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-gray-700 disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
