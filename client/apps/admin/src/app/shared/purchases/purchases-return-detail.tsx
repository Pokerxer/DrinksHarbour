  ...
  return (
    <div className="flex items-start gap-3">
      <div className="flex flex-col items-center">
        ...
        <div
          className={`mt-1 h-full w-0.5 ${done ? 'bg-emerald-200' : 'bg-gray-200'}`}
          style={{ minHeight: 8 }}
        />
      </div>
      <div className="pb-6">
        <p
          className={`text-sm font-medium ${done ? 'text-gray-900' : active ? 'text-[#b20202]' : 'text-gray-400'}`}
        >
          {label}
        </p>
        {date && (
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-full ${danger ? 'bg-red-100' : 'bg-amber-100'}`}
          >
            {danger ? (
              <PiWarning className="h-5 w-5 text-red-600" />
            ) : (
              <PiCheck className="h-5 w-5 text-amber-600" />
            )}
          </div>
          <div>
...
export default function PurchasesReturnDetail({ id }: { id: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : 'Failed to delete return'
      );
    } finally {
      setActing(false);
    }
  ...
  const timelineSteps = [
    ...
    },
    {
      ...
      icon: <PiCheck className="h-4 w-4" />,
        ...
        !!ret.confirmedAt ||
        ['confirmed', 'received', 'refunded'].includes(ret.status),
      date: ret.confirmedAt,
      active: ret.status === 'draft',
    },
    {
      key: 'requested',
      label: 'Requested',
      icon: <PiPackage className="h-4 w-4" />,
        ...
        !!ret.requestedDate ||
        ['requested', 'shipped', 'in_transit', 'received', 'refunded'].includes(
          ret.status
        ),
      date: ret.requestedDate,
      active: ret.status === 'confirmed',
    },
    {
      key: 'shipped',
      label: 'Shipped',
      icon: <PiShippingContainer className="h-4 w-4" />,
      done:
        !!ret.shippedDate ||
        ['shipped', 'in_transit', 'received', 'refunded'].includes(ret.status),
      date: ret.shippedDate,
      active: ret.status === 'requested',
    },
    {
      key: 'in_transit',
      label: 'In Transit',
      icon: <PiDotsNine className="h-4 w-4" />,
      done: ['in_transit', 'received', 'refunded'].includes(ret.status),
      active: ret.status === 'shipped',
    },
    {
      key: 'received',
      label: 'Received',
      icon: <PiHandArrowDown className="h-4 w-4" />,
      done: !!ret.receivedDate || ['received', 'refunded'].includes(ret.status),
      date: ret.receivedDate,
      active: ret.status === 'in_transit',
    },
    {
      key: 'refunded',
      label: 'Refunded',
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() =>
              printReturnInvoice(ret, tenant?.name || 'DrinksHarbour')
            }
            ...
            <>
              <button
                ...
                onClick={() =>
                  setConfirmAction({
                    ...
                    message: 'Mark this return as received from vendor.',
              ...
              <button
                ...
                onClick={() =>
                  setConfirmAction({
                    title: 'Cancel Return',
                    message: 'This will cancel the return request.',
        ...
        <div className="mb-5 rounded-xl border border-green-200 bg-green-50 p-4">
          ...
          {bill && (
            <div className="mb-3 rounded-lg border border-green-200 bg-white px-3 py-2 text-xs text-green-700">
              Linked Bill: <strong>{bill.billNumber || bill._id}</strong>{' '}
              &middot; Paid:{' '}
              <strong>{fmtCur(bill.paidAmount || 0, cur)}</strong>
              {bill.status === 'paid' ? ' (Fully Paid)' : ' (Partially Paid)'}
              &middot; Max Refund:{' '}
              <strong>
                {fmtCur(Math.min(total, bill.paidAmount || 0), cur)}
              </strong>
            </div>
          )}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <input
                ...
                placeholder={fmtCur(
                  Math.min(total, bill?.paidAmount ?? total),
                  cur
                )}
                ...
                {[25, 50, 75, 100].map((pct) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setRefundAmount(((capped * pct) / 100).toFixed(2))
                      }
            ...
            {ret.items.map((item, i) => (
              <tr key={i}>
                ...
                <td className="px-4 py-3">
                  {item.condition ? (
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium uppercase ${CONDITION_BADGE[item.condition] ?? 'bg-gray-100 text-gray-600'}`}
                    >
          ...
          <div className="mt-1 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
            ...
            {ret.refundDate && (
              <div>
                ...
                <p className="font-medium text-green-900">
                  {new Date(ret.refundDate).toLocaleDateString()}
                </p>
              </div>
            )}
          </div>
