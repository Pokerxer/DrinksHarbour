...
function FilterValueEditor({
  ...
  function commit() {
    ...
    // The document path the server filters on. Emitting only `fieldId` — the
    // UI's own snake_case identity — is why every filter here was dropped: the
    // server looks up `field` and skips a filter that does not carry one.
      ...
      case 'number-range': {
        ...
        if (!isNaN(n1) && !isNaN(n2))
          onApply({
            ...
            field,
            operator: 'between',
            value: [String(n1), String(n2)] as [string, string],
            label: `${label}: ${n1} - ${n2}`,
          });
        break;
      }
      ...
      case 'date-range': {
        if (dateFrom && dateTo)
          onApply({
            ...
            field,
            operator: 'between',
            value: [dateFrom, dateTo] as [string, string],
  ...
  const inputCls =
    'w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-brand';
...
export default function AdvancedSearchFilterList({
  activeFilters,
  onAddFilter,
  onRemoveFilter,
  ...
  return (
    <div>
      ...
      <div className="custom-scrollbar mt-2 max-h-64 space-y-0.5 overflow-y-auto">
        {filtered.map((config) => (
          <div key={config.id}>
            <button
              type="button"
              onClick={() =>
                setEditingFilter(editingFilter === config.id ? null : config.id)
              }
              className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm transition-all hover:bg-gray-50 ${
                isActive(config.id) ? 'font-medium text-brand' : 'text-gray-700'
              }`}
            >
              <span
              ...
              <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-gray-400">
                {config.category}
              </span>
              {isActive(config.id) && (
                <span
            ...
            {editingFilter === config.id && (
              <FilterValueEditor
                config={config}
                onApply={(v) => handleApply(config, v)}
