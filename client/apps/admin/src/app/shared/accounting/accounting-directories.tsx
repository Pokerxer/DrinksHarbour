...
import { useDirectory } from './use-accounting-directory';
  ...
  return (
    <div className="relative ml-auto w-full sm:w-auto">
      ...
      <input
        ...
        className={`${SELECT_CLS} w-full pl-8 sm:w-56`}
        value={value}
...
export function CustomersView() {
  const {
    ...
    error,
    retry,
  } = useDirectory<AccountingCustomer>((t, p) =>
    arApService.customers(t, p as never)
  );
  return (
    <div>
      {error && (
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {rows.map((c) => (
              <tr key={c._id} className="hover:bg-gray-50">
                ...
                <td
                  className={`whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums ${c.outstanding > 0 ? 'text-[#b20202]' : 'text-gray-400'}`}
                >
...
export function VendorsView() {
  const {
    ...
    error,
    retry,
  } = useDirectory<AccountingVendor>((t, p) =>
    arApService.vendors(t, p as never)
  );
  return (
    <div>
      {error && (
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {rows.map((v) => (
              <tr key={v._id} className="hover:bg-gray-50">
                ...
                <td
                  className={`whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums ${v.outstanding > 0 ? 'text-[#b20202]' : 'text-gray-400'}`}
                >
...
export function ProductsView() {
  const {
    ...
    error,
    retry,
  } = useDirectory<AccountingProduct>((t, p) =>
    arApService.products(t, p as never)
  );
  return (
    <div>
      {error && (
            ...
            {rows.map((p) => (
              <tr key={p._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${p.availability ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}
                  >
