 ...
 * priority alone, two employees in the same state came back in whatever order
 * Mongo happened to return, so the list reshuffled itself between visits and a
 * manager could not rely on position to find anyone.
 */
export function sortTeamAppraisals(rows: Appraisal[]): Appraisal[] {
  return [...rows].sort((a, b) => {
    const byState = priorityOf(a.state) - priorityOf(b.state);
    if (byState !== 0) return byState;
    const byDeadline = deadlineValue(a) - deadlineValue(b);
    if (byDeadline !== 0 && Number.isFinite(byDeadline)) return byDeadline;
    // Infinity - Infinity is NaN: both missing a deadline falls through to name.
    if (deadlineValue(a) !== deadlineValue(b)) {
      return deadlineValue(a) === Number.POSITIVE_INFINITY ? 1 : -1;
    }
    return personName(a.employee).localeCompare(personName(b.employee));
  });
}
...
export function filterTeamAppraisals(
  rows: Appraisal[],
  opts: { query?: string; scope?: TeamScope } = {},
  now = Date.now()
): Appraisal[] {
