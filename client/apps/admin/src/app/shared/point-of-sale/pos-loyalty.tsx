...
import type {
  ...
  POSPromotion,
  POSBuyXGetY,
  POSLoyaltyCardConfig,
  POSLoyaltyTier,
  POSNextOrderCouponConfig,
  POSDiscountAvailability,
  POSDiscountRules,
  POSDiscountReward,
  POSRewardApplyOn,
  POSApplicableItems,
...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors ${checked ? 'bg-[#b20202]' : 'bg-gray-200'} ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'}`}
    >
      <span
...
function Input(
  ...
  return (
    <input
      {...rest}
      className={`w-full rounded-xl border px-3.5 py-2.5 text-sm outline-none transition-colors ${hasError ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#b20202]'} ${rest.className || ''}`}
    />
  );
}
    ...
    <div className="flex overflow-hidden rounded-xl border border-gray-200">
      {(
        ...
        <button
          ...
          style={
            value === t ? { backgroundColor: color || '#b20202' } : undefined
          }
        >
          {lbl}
        </button>
...
function StatusBadge({ active }: { active: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-400'}`}
    >
...
function SavingsBadge({
  ...
  return (
    <span
      ...
      style={{ backgroundColor: color || '#b20202' }}
    >
...
function DateInput({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-[10px] font-semibold text-gray-400">
        {label}
      </label>
      <input
        type="datetime-local"
...
function ModalShell({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="flex max-h-[90vh] w-full flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:w-[80vw] sm:max-w-[80vw] sm:rounded-2xl">
        <div
...
function ListRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 border-b border-gray-50 px-5 py-3.5 last:border-0 ${faded ? 'opacity-50' : ''}`}
    >
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200">
      ...
      >
        ...
        {open ? (
          <PiCaretUp className="h-4 w-4 text-gray-400" />
        ) : (
          <PiCaretDown className="h-4 w-4 text-gray-400" />
        )}
      </button>
      {open && (
...
function PricelistPicker({
  ...
  useEffect(() => {
    ...
    pricelistService
      .list(adminToken)
      .then((d: any) => {
        setPricelists(d.data?.pricelists || d.pricelists || []);
        setError('');
      })
  ...
  const selectedNames = pricelists
    .filter((pl) => value.includes(pl._id))
    .map((pl) => pl.name);
    ...
    <div>
      <div className="mb-2 flex items-center justify-between">
        <label className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
          Pricelists
          <span className="ml-1.5 font-normal normal-case text-gray-300">
            — restrict to these customer groups
          </span>
        </label>
        {value.length > 0 && (
          <button
      ...
      {value.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1.5">
          {selectedNames.map((n) => (
            <span
              key={n}
              className="inline-flex items-center gap-1 rounded-full bg-[#b20202]/10 px-2.5 py-0.5 text-[11px] font-semibold text-[#b20202]"
            >
              {n}
            </span>
          ))}
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {pricelists.map((pl) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border-2 transition-colors ${checked ? 'border-[#b20202] bg-[#b20202]' : 'border-gray-300 bg-white'}`}
                >
                  {checked && (
                    <svg
                ...
                <span
                  className={`text-sm font-semibold leading-tight ${checked ? 'text-[#b20202]' : 'text-gray-800'}`}
                >
                  {pl.name}
                </span>
              </button>
      ...
      {value.length === 0 && pricelists.length > 0 && (
        <p className="mt-1.5 text-[10px] text-gray-400">
          None selected — discount applies to all customers regardless of
          pricelist
        </p>
      )}
    </div>
        ...
        {limitEnabled && (
          <div className="flex items-center gap-2">
            <input
              ...
              min={1}
              value={maxUsage || 1}
              onChange={(e) => onMaxUsage(parseInt(e.target.value) || 1)}
              className="w-28 rounded-xl border border-gray-200 px-3 py-2 text-center text-sm font-bold outline-none"
              style={{ borderColor: limitEnabled ? accentColor : undefined }}
            />
            <span className="text-sm text-gray-500">total uses allowed</span>
            {(usageCount ?? 0) > 0 && (
              <span className="ml-auto text-xs text-gray-400">
                {usageCount} used so far
              </span>
            )}
          </div>
        )}
...
function ItemPicker({
  ...
  value: POSApplicableItems;
  onChange: (v: POSApplicableItems) => void;
  adminToken?: string;
}) {
    ...
    Promise.all([
      ...
      .then(([catData, brandData]: any[]) => {
        ...
        tops.forEach((t: any) => {
          ...
          subs
            .filter((s: any) => s.parent?.toString() === t._id?.toString())
            .forEach((s: any) =>
              ordered.push({
                id: s._id,
                name: s.name,
        ...
        subs
          .filter(
            (s: any) =>
              !tops.find((t: any) => t._id?.toString() === s.parent?.toString())
          )
          .forEach((s: any) =>
            ordered.push({
              id: s._id,
  ...
  // Product search
  useEffect(() => {
    if (!adminToken || !productQuery.trim()) {
      setProductResults([]);
      return;
    }
    setLoadingProducts(true);
    subproductService
      .getSubProducts(adminToken, { search: productQuery.trim(), limit: 20 })
      .then((d: any) => {
        ...
        for (const sp of sps) {
          ...
          items.push({
            ...
            image: resolveSubProductThumb(sp),
          });
        }
  ...
  function toggleProduct(item: PickItem) {
    const next = products.includes(item.id)
      ? products.filter((x) => x !== item.id)
      : [...products, item.id];
    onChange({ ...value, products: next });
  }
  function toggleCat(id: string) {
    const next = cats.includes(id)
      ? cats.filter((x) => x !== id)
      : [...cats, id];
  ...
  useEffect(() => {
    ...
    subproductService
      .getSubProducts(adminToken, { limit: 200 })
      .then((d: any) => {
        const map: Record<string, string> = {};
        const sps = d.data?.subProducts || d.subProducts || [];
        sps.forEach((sp: any) => {
          if (sp.product?._id) map[sp.product._id] = sp.product.name;
        });
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {cats.map((id) => (
            <Chip
              ...
              color="#059669"
              onRemove={() =>
                onChange({ ...value, categories: cats.filter((x) => x !== id) })
              }
            />
          ))}
          {brandIds.map((id) => (
            <Chip
              key={`b-${id}`}
              label={brandMap[id] || id}
              color="#7c3aed"
              onRemove={() =>
                onChange({ ...value, brands: brandIds.filter((x) => x !== id) })
              }
            />
          ))}
          <button
            type="button"
            onClick={() =>
              onChange({ products: [], categories: [], brands: [] })
            }
            className="ml-1 self-center text-[10px] text-gray-400 underline hover:text-red-500"
          >
            Clear all
          </button>
        </div>
      ...
      <div className="flex overflow-hidden rounded-xl border border-gray-200">
        {TABS.map((t) => (
          <button
            ...
            type="button"
            onClick={() => setTab(t.id)}
            className={`flex flex-1 items-center justify-center gap-1.5 py-2 text-xs font-bold transition-colors ${tab === t.id ? 'bg-[#b20202] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
          >
            {t.label}
            {t.count > 0 && (
              <span
                className={`rounded-full px-1.5 py-0.5 text-[9px] font-black ${tab === t.id ? 'bg-white/30 text-white' : 'bg-[#b20202] text-white'}`}
              >
                {t.count}
              </span>
            )}
      ...
      {tab === 'products' && (
        <div className="space-y-2">
          <div className="relative">
            ...
            {loadingProducts && (
              <span className="absolute right-3 top-1/2 inline-block h-3.5 w-3.5 -translate-y-1/2 animate-spin rounded-full border-2 border-gray-300 border-t-[#b20202]" />
            )}
            <input
              value={productQuery}
          ...
          {productResults.length > 0 && (
            <div className="max-h-52 divide-y divide-gray-50 overflow-y-auto rounded-xl border border-gray-200">
          ...
          {!productQuery && (
            <p className="text-xs text-gray-400">
              Type to search products by name
            </p>
          )}
        </div>
      )}
      ...
      {/* Categories tab */}
      {tab === 'categories' && (
        <div className="max-h-60 divide-y divide-gray-50 overflow-y-auto rounded-xl border border-gray-200">
          {loadingMeta && (
          ...
          {categories.map((cat) => (
            ...
            >
              ...
              {cat.indent ? (
                <span className="mr-0.5 text-[10px] text-gray-300">└</span>
              ) : null}
              <span
                className={`text-sm ${cat.indent ? 'text-gray-600' : 'font-semibold text-gray-800'}`}
              >
                {cat.name}
              </span>
            </button>
      ...
      {tab === 'brands' && (
        <div className="space-y-2">
          ...
          <div className="max-h-52 divide-y divide-gray-50 overflow-y-auto rounded-xl border border-gray-200">
            ...
            {!loadingMeta && filteredBrands.length === 0 && (
      ...
      {totalSelected === 0 && (
        <p className="text-[10px] text-gray-400">
          Nothing selected — discount applies to all items regardless of
          category or brand
        </p>
      )}
    </div>
...
function Chip({
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-full border py-0.5 pl-2 pr-1.5"
...
function AvailableOnSection({
  ...
  value: POSDiscountAvailability;
  onChange: (v: POSDiscountAvailability) => void;
}) {
  const channels: {
    key: keyof POSDiscountAvailability;
    label: string;
    icon: React.ReactNode;
    desc: string;
  }[] = [
    {
      ...
      desc: 'Cashiers can apply this in the POS terminal',
    },
    {
      key: 'sales',
      label: 'Sales',
      icon: <PiTag className="h-4 w-4" />,
      desc: 'Available on manual sales orders',
    },
    {
      key: 'website',
      label: 'Website',
      icon: <PiInfo className="h-4 w-4" />,
      desc: 'Customers can use this on the webshop',
    },
  ];
    ...
    <div className="space-y-2">
      {channels.map(({ key, label, icon, desc }) => (
        <div
          ...
          className={`flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 transition-colors ${value[key] ? 'border-[#b20202]/30 bg-red-50/50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}
        >
          <div
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${value[key] ? 'bg-[#b20202] text-white' : 'bg-gray-100 text-gray-500'}`}
          >
            {icon}
          </div>
          <div className="min-w-0 flex-1">
            <p
              className={`text-sm font-semibold ${value[key] ? 'text-[#b20202]' : 'text-gray-800'}`}
            >
              {label}
            </p>
          ...
          <div
            className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${value[key] ? 'border-[#b20202] bg-[#b20202]' : 'border-gray-300'}`}
          >
...
function RulesSection({
  ...
  value: POSDiscountRules;
  onChange: (v: POSDiscountRules) => void;
}) {
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">
        All conditions must be met for the discount to apply.
      </p>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1.5 block text-[11px] font-semibold text-gray-400">
            Min. quantity in cart
          </label>
          <div className="flex items-center gap-2">
            <input
              type="number"
              ...
              value={value.minQty || ''}
              onChange={(e) =>
                onChange({ ...value, minQty: parseInt(e.target.value) || 0 })
              }
            ...
            />
            <span className="shrink-0 text-xs text-gray-400">items</span>
          </div>
        </div>
        <div>
          <label className="mb-1.5 block text-[11px] font-semibold text-gray-400">
...
function RewardSection({
  ...
  value: POSDiscountReward;
  onChange: (v: POSDiscountReward) => void;
  color?: string;
}) {
  const accentColor = color || '#b20202';

  const applyOnOptions: {
    value: POSRewardApplyOn;
    label: string;
    desc: string;
  }[] = [
    {
      value: 'order',
      label: 'On Order',
      desc: 'Applied to the total cart value',
    },
    {
      value: 'cheapest',
      label: 'On Cheapest Item',
      desc: 'Applied to the lowest-priced item',
    },
    {
      value: 'most_expensive',
      label: 'On Most Expensive Item',
      desc: 'Applied to the highest-priced item',
    },
  ];
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <TypeToggle
            ...
            onChange={(t) => onChange({ ...value, discountType: t })}
            color={accentColor}
          />
          <input
            type="number"
            min={0}
            max={value.discountType === 'pct' ? 100 : undefined}
        ...
        <div className="space-y-1.5">
          {applyOnOptions.map((opt) => (
            <div
              ...
              className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-2.5 transition-colors ${value.applyOn === opt.value ? 'border-[#b20202]/30 bg-red-50/40' : 'border-gray-200 bg-white hover:bg-gray-50'}`}
            >
              <div
                className={`h-4 w-4 shrink-0 rounded-full border-2 transition-colors ${value.applyOn === opt.value ? 'border-[#b20202] bg-[#b20202]' : 'border-gray-300'}`}
              />
              <div className="min-w-0">
                <p
                  className={`text-sm font-semibold ${value.applyOn === opt.value ? 'text-[#b20202]' : 'text-gray-800'}`}
                >
                  {opt.label}
                </p>
      ...
      <div>
        ...
        {(value.maxDiscount ?? 0) > 0 && (
          <p className="mt-1 text-[10px] text-gray-400">
            Discount will not exceed {formatCurrency(value.maxDiscount ?? 0)}{' '}
...
// Default values
const DEFAULT_AVAILABILITY: POSDiscountAvailability = {
  pos: true,
  sales: false,
  website: false,
};
const DEFAULT_RULES: POSDiscountRules = { minQty: 0, minOrderValue: 0 };
const DEFAULT_REWARD: POSDiscountReward = {
  discountType: 'pct',
  discountValue: 10,
  applyOn: 'order',
  maxDiscount: 0,
...
function CouponModal({
  ...
  const [pricelistIds, setPricelistIds] = useState<string[]>(
    initial?.pricelistIds ?? []
  );
  const [applyTo, setApplyTo] = useState<POSApplicableItems>(
    initial?.applyTo ?? {}
  );
  const [availability, setAvailability] = useState<POSDiscountAvailability>(
    initial?.availableOn ?? DEFAULT_AVAILABILITY
  );
  const [rules, setRules] = useState<POSDiscountRules>(
    initial?.rules ?? DEFAULT_RULES
  );
  const [reward, setReward] = useState<POSDiscountReward>(
    initial?.reward ?? {
      ...DEFAULT_REWARD,
      discountValue: initial?.value ?? 10,
      discountType: initial?.type ?? 'pct',
    }
  );
  ...
  function handleSave() {
    ...
    if (reward.discountValue <= 0) {
      setErr('Reward value must be > 0');
      return;
    }
    if (reward.discountType === 'pct' && reward.discountValue > 100) {
  ...
  return (
    <ModalShell
      title={initial ? 'Edit Coupon' : 'New Coupon'}
      headerBg="#1d4ed8"
      onClose={onClose}
      onSave={handleSave}
      saveLabel={initial ? 'Save' : 'Create Coupon'}
    >
      ...
      <AccordionSection
        title={`Applicable Items${(applyTo.products?.length ?? 0) + (applyTo.categories?.length ?? 0) + (applyTo.brands?.length ?? 0) > 0 ? ` (${(applyTo.products?.length ?? 0) + (applyTo.categories?.length ?? 0) + (applyTo.brands?.length ?? 0)})` : ''}`}
      ...
      <AccordionSection
        title="Validity & Usage"
        icon={<PiCalendar className="h-4 w-4" />}
        open={openValidity}
        onToggle={() => setOpenValidity((v) => !v)}
      >
        <ValiditySection
          validFrom={validFrom}
          validTo={validTo}
        ...
        <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
          <div>
            ...
            <p className="text-[10px] text-gray-400">
              Prevents stacking multiple coupon uses in one order
            </p>
          </div>
...
function DiscountCodeModal({
  ...
  initial?: POSDiscountCode;
  onSave: (d: POSDiscountCode) => void;
  onClose: () => void;
  adminToken?: string;
}) {
  const [code, setCode] = useState(initial?.code ?? '');
  ...
  const applyToCount =
    (applyTo.products?.length ?? 0) +
    (applyTo.categories?.length ?? 0) +
    (applyTo.brands?.length ?? 0);

  return (
    <ModalShell
      title={initial ? 'Edit Discount Code' : 'New Discount Code'}
      headerBg={color}
      onClose={onClose}
      onSave={handleSave}
      saveLabel={initial ? 'Save' : 'Create Code'}
    >
      {/* Color row */}
      <div className="flex items-center gap-3">
      ...
      <Field label="Discount code" required>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className={`w-full rounded-xl border px-3.5 py-2.5 font-mono text-sm font-bold uppercase tracking-widest outline-none transition-colors ${err && !code ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#059669]'}`}
            />
          </div>
          <button
            type="button"
          ...
          <button
            ...
            onClick={copyCode}
            disabled={!code}
            className={`shrink-0 rounded-xl border px-3 text-xs font-semibold transition-colors ${copied ? 'border-emerald-300 bg-emerald-50 text-emerald-600' : 'border-gray-200 text-gray-500 hover:border-emerald-300 hover:text-emerald-600 disabled:opacity-40'}`}
          >
            {copied ? '✓ Copied' : 'Copy'}
          </button>
        </div>
        ...
        {code && (
          <div className="mt-2 flex items-center gap-2">
            <span
              className="rounded-lg px-3 py-1.5 font-mono text-sm font-black tracking-widest text-white"
              style={{ backgroundColor: color }}
            >
              {code}
            </span>
      ...
      {(validFrom || validTo) && (
        <div
          className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold ${expired ? 'bg-red-50 text-red-600' : scheduled ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'}`}
        >
          <PiCalendar className="h-3.5 w-3.5 shrink-0" />
          {expired &&
            `Expired — ${endD!.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`}
          {scheduled &&
            `Scheduled — activates ${startD!.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`}
          {!expired &&
            !scheduled &&
            daysLeft !== null &&
      ...
      <AccordionSection
        title={`Applicable Items${applyToCount > 0 ? ` (${applyToCount})` : ''}`}
        icon={<PiPackage className="h-4 w-4" />}
        open={openItems}
        onToggle={() => setOpenItems((v) => !v)}
      >
...
function PromotionModal({
  ...
  initial?: POSPromotion;
  onSave: (p: POSPromotion) => void;
  onClose: () => void;
  adminToken?: string;
}) {
  const [name, setName] = useState(initial?.name ?? '');
  ...
  function daysAgo(d: Date) {
    return Math.ceil((now.getTime() - d.getTime()) / 86400000);
  }
  ...
  return (
    <ModalShell
      title={initial ? 'Edit Promotion' : 'New Promotion'}
      headerBg={color}
      onClose={onClose}
      onSave={handleSave}
      saveLabel={initial ? 'Save' : 'Create Promotion'}
    >
      {/* Color picker row */}
      <div className="flex items-center gap-3">
        <div className="flex gap-1.5">
      ...
      <Field label="Promotion name" required>
        <Input
          ...
          placeholder="e.g. Christmas Sale, Weekend Special"
          hasError={!!err && !name}
        />
      </Field>
      <Field label="Description" hint="(optional)">
        ...
        >
          ...
          >
            ...
            {status === 'live' &&
              ...
              `● Live — ${daysUntil(end)} day${daysUntil(end) !== 1 ? 's' : ''} remaining`}
            {status === 'live' && !end && '● Live — no end date set'}
            {status === 'scheduled' &&
              `Scheduled — starts in ${daysUntil(start!)} day${daysUntil(start!) !== 1 ? 's' : ''} (${start!.toLocaleDateString()})`}
            {status === 'expired' &&
              `Expired ${daysAgo(end!)} day${daysAgo(end!) !== 1 ? 's' : ''} ago`}
            {status === 'inactive' && 'Inactive (toggle active to enable)'}
          </div>
          {/* Visual timeline */}
          {(startDate || endDate) && (
            <div className="flex items-center gap-2 bg-white px-4 py-3 text-[11px] text-gray-500">
              <div className="flex items-center gap-1.5">
                <div
                  className="h-2 w-2 rounded-full"
                  style={{
                    backgroundColor: start && start <= now ? color : '#d1d5db',
                  }}
                />
                <span>
                  {startDate
              ...
              <div className="relative mx-1 h-1 flex-1 rounded-full bg-gray-100">
                {startDate &&
                  endDate &&
                  (() => {
                    const total =
              ...
              <div className="flex items-center gap-1.5">
                <div
                  className="h-2 w-2 rounded-full"
                  style={{
                    backgroundColor: end && end <= now ? color : '#d1d5db',
                  }}
                />
                <span>
                  {endDate
      ...
      >
        <ValiditySection
          ...
          maxUsage={maxUsage}
          usageCount={initial?.usageCount}
          onValidFrom={setStartDate}
          onValidTo={setEndDate}
          onMaxUsage={setMaxUsage}
          color={color}
        />
      </AccordionSection>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2 flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 sm:col-span-1">
          <div>
            <p className="text-sm font-semibold text-gray-700">Stackable</p>
            <p className="text-[10px] text-gray-400">
              Applies alongside other active promotions
            </p>
          </div>
          <Toggle checked={stackable} onChange={setStackable} />
        </div>
        <div className="col-span-2 rounded-xl bg-gray-50 px-4 py-3 sm:col-span-1">
          <label className="mb-1 block text-[10px] font-semibold text-gray-400">
            Priority{' '}
            <span className="font-normal">(higher = applied first)</span>
          </label>
          <input
            type="number"
            min={0}
...
function BuyXGetYModal({
  ...
  const [rewardApplyTo, setRewardApplyTo] = useState<POSApplicableItems>(
    initial?.rewardApplyTo ?? {}
  );
  const [availability, setAvailability] = useState<POSDiscountAvailability>(
    initial?.availableOn ?? DEFAULT_AVAILABILITY
  );
  ...
  const [getDiscountPct, setGetDiscountPct] = useState(
    initial?.getDiscountPct ?? 100
  );
  const [rules, setRules] = useState<POSDiscountRules>(
    initial?.rules ?? { minQty: 0, minOrderValue: initial?.minOrderValue ?? 0 }
  );
  const [validFrom, setValidFrom] = useState(initial?.validFrom ?? '');
  const [validTo, setValidTo] = useState(initial?.validTo ?? '');
  const [maxUsage, setMaxUsage] = useState(initial?.maxUsage ?? 0);
  ...
  return (
    <ModalShell
      title={initial ? 'Edit Offer' : 'New Buy X Get Y'}
      headerBg={color}
      onClose={onClose}
      onSave={handleSave}
      saveLabel={initial ? 'Save' : 'Create Offer'}
    >
      {/* Color row */}
      <div className="flex items-center gap-3">
        <div className="flex gap-1.5">
          ...
          {BXGY_PRESETS.map((p) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border px-3 py-1.5 text-xs font-bold transition-colors ${active ? 'border-transparent text-white' : 'border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50'}`}
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200">
        <div className="grid grid-cols-2 divide-x divide-gray-200">
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-200 text-lg text-gray-600 transition-colors hover:border-gray-300 hover:bg-gray-50"
              >
                −
              </button>
              <span
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setGetQty(Math.max(1, getQty - 1))}
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-200 text-lg text-gray-600 transition-colors hover:border-gray-300 hover:bg-gray-50"
              >
                −
              </button>
        ...
        <div className="space-y-2 border-t border-gray-100 px-5 py-4">
          <div className="flex items-center justify-between">
            <label className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
              Discount on the{' '}
              {getQty > 1 ? `${getQty} "get" items` : '"get" item'}
            </label>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex shrink-0 items-center gap-1">
              {Array.from({ length: Math.min(buyQty, 5) }).map((_, i) => (
                <div
                  key={i}
                  className="flex h-8 w-8 items-center justify-center rounded-lg border-2 text-[10px] font-black text-white"
              ...
              {Array.from({ length: Math.min(getQty, 3) }).map((_, i) => (
                <div
                  key={i}
                  className="flex h-8 w-8 items-center justify-center rounded-lg border-2 text-[10px] font-black"
                  style={{
                    borderColor: color,
                    color,
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-gray-500">
                {totalUnits} items total · {savedAmount}
              </p>
            </div>
          </div>
        </div>
      ...
      {(validFrom || validTo) && (
        ...
        >
          ...
          {expired && 'Expired'}
          {scheduled &&
            `Scheduled — starts ${startD!.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`}
          {!expired &&
            !scheduled &&
            daysLeft !== null &&
      ...
      >
        <div className="space-y-4">
          <Field
            ...
            hint="— which items the customer must buy"
          >
            <ItemPicker
              value={applyTo}
              onChange={setApplyTo}
      ...
      >
        <Field
          ...
          hint="— which items receive the free/discounted quantity"
        >
          <ItemPicker
            value={rewardApplyTo}
            onChange={setRewardApplyTo}
            adminToken={adminToken}
          />
        </Field>
      </AccordionSection>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2 flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 sm:col-span-1">
          <div>
            <p className="text-sm font-semibold text-gray-700">Stackable</p>
            <p className="text-[10px] text-gray-400">
              Can apply alongside other active offers
            </p>
          </div>
          <Toggle checked={stackable} onChange={setStackable} />
        </div>
...
export default function POSLoyalty() {
  ...
  const [editCode, setEditCode] = useState<POSDiscountCode | 'new' | null>(
    null
  );
  ...
  const [nocAvailableOn, setNocAvailableOn] = useState<POSDiscountAvailability>(
    { pos: true, sales: false, website: false }
  );
  ...
  function makeCrud<T extends { _id?: string; active?: boolean }>(
    ...
    return {
      ...
      toggle: (id: string) => {
        save_(
          list.map((x) => (x._id === id ? { ...x, active: !x.active } : x))
        );
      },
      delete: (id: string) => {
        const name =
          (list.find((x) => x._id === id) as any)?.name ||
          (list.find((x) => x._id === id) as any)?.code ||
          '';
    ...
    await save(
      {
        ...
        loyaltyCard: {
          enabled: lcEnabled,
          cardPrefix: lcPrefix,
          earnMultiplier: lcMultiplier,
          welcomeBonus: lcWelcome,
          pointsExpiry: lcPointsExpiry,
          minRedemption: lcMinRedemption,
          doublePointsDays: lcDoublePointsDays,
          bonusMultiplierDays: lcBonusMultiplierDays,
    ...
    await save(
      {
        nextOrderCoupon: {
          ...
          type: nocType,
          value: nocValue,
          validDays: nocValidDays,
          minOrderForCoupon: nocMinOrder,
          minRedeemOrder: nocMinRedeemOrder,
          codePrefix: nocPrefix,
          color: nocColor,
          oneUse: nocOneUse,
          availableOn: nocAvailableOn,
        },
      },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage coupons, promotions, discount codes, buy X get Y offers,
        ...
        {loading ? (
          ...
          <>
            {/* Tab bar */}
            <div className="scrollbar-none flex gap-1 overflow-x-auto rounded-2xl border border-gray-200 bg-white p-1.5 shadow-sm">
              {TABS.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  onClick={() => setTab(t.id)}
                  className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3.5 py-2.5 text-xs font-bold transition-all ${tab === t.id ? 'text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
                  style={
                    tab === t.id ? { backgroundColor: t.color } : undefined
                  }
                >
                  {t.icon} {t.label}
                </button>
            ...
            {tab === 'coupons' && (
              <SectionCard
                icon={<PiTicket className="h-5 w-5" />}
                color="#1d4ed8"
                title="Coupons"
                description="Code-based discounts with usage tracking and expiry"
                count={coupons.filter((c) => c.active).length}
                onAdd={() => setEditCoupon('new')}
                addLabel="New Coupon"
              >
                {coupons.length === 0 ? (
                  <EmptyState
                    icon={<PiTicket className="h-7 w-7 text-gray-300" />}
                    title="No coupons yet"
                    body="Create coupon codes like SAVE20 or WELCOME10 for customers to redeem at checkout"
                    action={
                      <button
                        type="button"
                        onClick={() => setEditCoupon('new')}
                  ...
                  <div className="divide-y divide-gray-50">
                    {coupons.map((c) => {
                      ...
                      const atLimit =
                        c.maxUsage > 0 && c.usageCount >= c.maxUsage;
                      return (
                        <ListRow
                          key={c._id}
                          active={c.active && !expired && !atLimit}
                          faded={!c.active || expired || atLimit}
                          onToggle={() => couponCrud.toggle(c._id)}
                          onEdit={() => setEditCoupon(c)}
                          onDelete={() => couponCrud.delete(c._id)}
                        >
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-lg bg-blue-50 px-2.5 py-1 font-mono text-xs font-bold tracking-widest text-blue-700">
                              {c.code}
                            </span>
            ...
            {tab === 'loyalty' &&
              (() => {
                ...
                const PRESETS = [
                  {
                    ...
                    tiers: [
                      ...
                      },
                      {
                        ...
                        multiplier: 1.5,
                        color: '#6b7280',
                        benefits: '1.5× earn, early access to promos',
                      },
                      {
                        name: 'Gold',
                        minPoints: 2000,
                        multiplier: 2,
                        color: '#d97706',
                        benefits: '2× earn, birthday bonus, priority service',
                      },
                    ],
                  },
                  {
                    ...
                    tiers: [
                      ...
                      },
                      {
                        ...
                        multiplier: 1.5,
                        color: '#6b7280',
                        benefits: '1.5× earn, early access',
                      },
                      {
                        name: 'Gold',
                        minPoints: 2000,
                        multiplier: 2,
                        color: '#d97706',
                        benefits: '2× earn, birthday bonus',
                      },
                      {
                        name: 'Platinum',
                        minPoints: 5000,
                        multiplier: 3,
                        color: '#7c3aed',
                        benefits: '3× earn, VIP events, free delivery',
                      },
                    ],
                  },
                ];
                ...
                const sortedTiers = [...lcTiers].sort(
                ...
                return (
                  <div className="space-y-4">
                    ...
                    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                      <div className="flex items-center justify-between gap-3 px-6 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              Points earned on every purchase, redeemable at
                              checkout
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span
                            className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold ${loyaltyEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-400'}`}
                          >
                      ...
                      <div
                        className={`flex items-center justify-between gap-3 border-t border-gray-100 px-6 py-4 ${!loyaltyEnabled ? 'pointer-events-none opacity-40' : ''}`}
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              Issue numbered loyalty cards with a prefix (e.g.{' '}
                              {lcPrefix}0001)
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span
                            className={`text-xs font-semibold ${lcEnabled ? 'text-emerald-600' : 'text-gray-400'}`}
                          >
                            {lcEnabled ? 'On' : 'Off'}
                          </span>
                          <Toggle
                    ...
                    <div
                      className={`space-y-3 ${!loyaltyEnabled ? 'pointer-events-none opacity-40' : ''}`}
                    >
                      {/* ── Earn Rules ── */}
                      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div className="divide-y divide-gray-50">
                          <div className="flex items-center justify-between gap-4 px-6 py-4">
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                value={
                                  Math.round(
                                    loyaltyPointsPerNaira * 100 * 100
                                  ) / 100
                                }
                                onChange={(e) =>
                                  setLoyaltyPointsPerNaira(
                          ...
                          <div
                            className={`flex items-center justify-between gap-4 px-6 py-4 ${!lcEnabled ? 'pointer-events-none opacity-40' : ''}`}
                          >
                            <div>
                              <p className="text-sm font-semibold text-gray-900">
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                Bonus for customers presenting a loyalty card
                                (base rate when no tiers)
                              </p>
                            </div>
                            <div className="flex items-center gap-3">
                              <input
                                ...
                                value={lcMultiplier}
                                onChange={(e) =>
                                  setLcMultiplier(parseFloat(e.target.value))
                                }
                                className="w-28 accent-[#d97706]"
                              />
                              <span className="w-8 text-right text-sm font-black text-amber-600">
                                {lcMultiplier}×
                              </span>
                            </div>
                          </div>
                          ...
                          >
                            <div>
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                One-time bonus points on a card's first use
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                value={lcWelcome || ''}
                                onChange={(e) =>
                                  setLcWelcome(parseInt(e.target.value) || 0)
                                }
                                placeholder="0"
                                className="w-24 rounded-xl border border-gray-200 px-3 py-2 text-center text-sm font-bold outline-none focus:border-[#d97706]"
                              />
                          ...
                          {lcEnabled && (
                            <div className="flex items-center justify-between gap-4 px-6 py-4">
                              <div>
                                ...
                                <p className="mt-0.5 text-xs text-gray-400">
                                  Prepended to every issued card number
                                </p>
                              </div>
                              <div className="flex items-center gap-2">
                                <input
                                  value={lcPrefix}
                                  onChange={(e) =>
                                    setLcPrefix(e.target.value.slice(0, 10))
                                  }
                                  className="w-24 rounded-xl border border-gray-200 px-3 py-2 font-mono text-sm font-bold outline-none focus:border-[#d97706]"
                                />
                                <span className="rounded-lg bg-amber-50 px-2.5 py-1.5 font-mono text-xs font-bold text-amber-700">
                                  {lcPrefix}0001
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div className="space-y-3 px-5 py-4">
                          <p className="text-xs text-gray-400">
                            Select days when earn rate is multiplied (e.g.
                            ...
                            {DAYS.map((day, d) => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    setLcDoublePointsDays((prev) =>
                                      active
                                        ? prev.filter((x) => x !== d)
                                        : [...prev, d]
                                    )
                                  }
                                  className={`rounded-xl border px-3.5 py-2 text-xs font-bold transition-colors ${active ? 'border-[#d97706]/40 bg-amber-50 text-amber-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                                >
                                  {day}
                                </button>
                              );
                          ...
                          {lcDoublePointsDays.length > 0 && (
                            <div className="flex items-center gap-3 pt-1">
                              ...
                              <input
                                ...
                                max={5}
                                step={0.5}
                                value={lcBonusMultiplierDays}
                                onChange={(e) =>
                                  setLcBonusMultiplierDays(
                        ...
                        <div className="divide-y divide-gray-50">
                          <div className="flex items-center justify-between gap-4 px-6 py-4">
                            <div>
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                How much ₦1 point is worth when redeemed
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center justify-between gap-4 px-6 py-4">
                            <div>
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                Cap on the % of an order total that can be paid
                                with points
                              </p>
                            </div>
                            <div className="flex items-center gap-3">
                              <input
                                ...
                                max={100}
                                step={5}
                                value={loyaltyMaxRedemptionPct}
                                onChange={(e) =>
                                  setLoyaltyMaxRedemptionPct(
                                    parseInt(e.target.value)
                                  )
                                }
                                className="w-28 accent-[#d97706]"
                              />
                              <span className="w-10 text-right text-sm font-black text-amber-600">
                                {loyaltyMaxRedemptionPct}%
                              </span>
                            </div>
                          </div>
                          <div className="flex items-center justify-between gap-4 px-6 py-4">
                            <div>
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                Customer must accumulate at least this many
                                points before redeeming any
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                              <input
                      ...
                      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div className="space-y-3 px-6 py-4">
                          <div className="flex gap-2">
                            {[
                              { label: 'Never expire', v: 0 },
                              { label: 'Expire after N days', v: -1 },
                            ].map((opt) => (
                              <button
                                key={opt.label}
                                type="button"
                          ...
                          {lcPointsExpiry === 0 && (
                            <p className="text-xs text-gray-400">
                              Points never expire — customers keep them
                              indefinitely
                            </p>
                          )}
                        </div>
                      ...
                      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                              ...
                              onClick={() =>
                                setLcTiers((prev) => [
                                  ...prev,
                                  {
                                    name: '',
                                    minPoints:
                                      (prev[prev.length - 1]?.minPoints ?? 0) +
                                      500,
                                    multiplier: Math.min(
                                      5,
                                      (prev[prev.length - 1]?.multiplier ?? 1) +
                                        0.5
                                    ),
                                    color: '#d97706',
                                    benefits: '',
                              ...
                              >
                                <div className="flex min-w-[72px] flex-col items-center gap-1">
                                  <div
                                    ...
                                    style={{
                                      backgroundColor: tier.color || '#d97706',
                                    }}
                                  >
                                    {tier.name
                                  ...
                                  <span className="text-center text-[10px] font-bold leading-tight text-gray-700">
                                    {tier.name || '—'}
                                  </span>
                                ...
                                {i < sortedTiers.length - 1 && (
                                  <div
                                    ...
                                    style={{
                                      background: `linear-gradient(to right, ${sortedTiers[i].color}, ${sortedTiers[i + 1].color})`,
                                    }}
                                  />
                                )}
                              </div>
                        ...
                        {lcTiers.length === 0 ? (
                          <div className="px-6 py-8 text-center">
                            <PiCrown className="mx-auto mb-2 h-8 w-8 text-gray-200" />
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    ...
                                    <input
                                      ...
                                      placeholder="0"
                                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold outline-none focus:border-[#d97706]"
                                    />
                                  </div>
                                  <div>
                                    <label className="mb-1 block text-[10px] font-semibold text-gray-400">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      />
                                      <span
                                        className="w-8 shrink-0 text-right text-sm font-black"
                                        style={{
                                          color: tier.color || '#d97706',
                                        }}
                                      >
                                        {tier.multiplier}×
                                      </span>
                      ...
                      {(() => {
                        const ptsPerN100 =
                          Math.round(loyaltyPointsPerNaira * 100 * 100) / 100;
                        const currentTier = tierForPoints(calcPoints);
                        const mult = currentTier?.multiplier ?? lcMultiplier;
                        const todayDay = new Date().getDay();
                        const isBonusDay =
                          lcDoublePointsDays.includes(todayDay);
                        const finalMult = isBonusDay
                          ? mult * lcBonusMultiplierDays
                          : mult;
                        const earnedPts = Math.round(
                          (calcOrder / 100) * ptsPerN100 * finalMult
                        );
                        const bonusPts = calcFirst ? lcWelcome : 0;
                        const totalNew = calcPoints + earnedPts + bonusPts;
                        const nextTier = sortedTiers.find(
                          (t) => t.minPoints > totalNew
                        );
                        const canRedeem = calcPoints >= lcMinRedemption;
                        const maxRedeem = Math.round(
                          (calcOrder * loyaltyMaxRedemptionPct) / 100
                        );
                        const redeemNaira = canRedeem
                          ? Math.min(
                              Math.round(calcPoints * loyaltyPointsValue),
                              maxRedeem
                            )
                          : 0;
                        return (
                          <div className="overflow-hidden rounded-2xl border border-amber-200 bg-amber-50/40">
                            <div className="flex items-center gap-2 border-b border-amber-200/60 px-5 py-3.5">
                              ...
                              <span className="text-xs text-amber-500">
                                — simulate how settings apply to a customer
                              </span>
                            </div>
                            <div className="space-y-4 p-5">
                              <div className="grid grid-cols-2 gap-3">
                                <div>
                                  <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-amber-600">
                                    Order amount
                                  </label>
                                  <div className="relative">
                                    <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-amber-500">
                                      ₦
                                    </span>
                                    <input
                                      type="number"
                              ...
                              <div className="flex flex-wrap items-center gap-4">
                                <label className="flex cursor-pointer select-none items-center gap-2 text-xs text-amber-800">
                                  <input
                                    type="checkbox"
                                    checked={calcFirst}
                                    onChange={(e) =>
                                      setCalcFirst(e.target.checked)
                                    }
                                ...
                                {isBonusDay && (
                                  <span className="rounded-full bg-amber-200 px-2.5 py-0.5 text-[10px] font-bold text-amber-800">
                                    🎉 Today ({DAYS[todayDay]}) is a{' '}
                                    {lcBonusMultiplierDays}× bonus day!
                                  </span>
                                )}
                              </div>
                              <div className="divide-y divide-amber-100 overflow-hidden rounded-xl border border-amber-200 bg-white">
                                {currentTier && (
                                  <div className="flex items-center gap-2 bg-amber-50/40 px-4 py-2.5">
                                    <div
                                    ...
                                    <span className="text-xs text-gray-600">
                                      ...
                                      — {finalMult}× earn rate
                                      {isBonusDay
                                        ? ` (${mult}× tier × ${lcBonusMultiplierDays}× bonus day)`
                                        : ''}
                                    </span>
                                  </div>
                                )}
                                <div className="flex justify-between px-4 py-2.5 text-xs text-gray-600">
                                  <span>
                                    Points earned on ₦
                                    {calcOrder.toLocaleString()} order
                                  </span>
                                ...
                                {bonusPts > 0 && (
                                  <div className="flex justify-between px-4 py-2.5 text-xs text-emerald-600">
                                    ...
                                    <strong>
                                      +{bonusPts.toLocaleString()} pts
                                    </strong>
                                  </div>
                                )}
                                <div className="flex justify-between bg-amber-50/60 px-4 py-2.5 text-xs font-bold text-gray-800">
                                  <span>New balance after this order</span>
                                  <span className="text-amber-700">
                                    {totalNew.toLocaleString()} pts
                                  </span>
                                </div>
                                <div className="flex justify-between px-4 py-2.5 text-xs text-gray-600">
                                  <span>
                                    ...
                                    {!canRedeem &&
                                      lcMinRedemption > 0 &&
                                      ` (need ${lcMinRedemption.toLocaleString()} pts min)`}
                                  </span>
                                  <strong
                                    className={
                                      canRedeem && redeemNaira > 0
                                        ? 'text-emerald-600'
                                        : 'text-gray-400'
                                    }
                                  >
                                    {canRedeem && redeemNaira > 0
                                      ? `₦${redeemNaira.toLocaleString()} off`
                                      : '—'}
                                  </strong>
                                </div>
                                {nextTier && (
                                  <div className="flex justify-between px-4 py-2.5 text-xs text-blue-600">
                                    <span>
                                      To reach <strong>{nextTier.name}</strong>
                                    </span>
                                    <strong>
                                      {Math.max(
                                        0,
                                        nextTier.minPoints - totalNew
                                      ).toLocaleString()}{' '}
                                      more pts needed
                                    </strong>
                                  </div>
            ...
            {tab === 'promotions' && (
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Automatic discounts applied at checkout when conditions
                        are met
                      </p>
                    </div>
                  </div>
                ...
                {promotions.length === 0 ? (
                  <EmptyState
                    icon={<PiLightning className="h-7 w-7 text-gray-300" />}
                    ...
                    {[...promotions]
                      ...
                      .map((p) => {
                        ...
                        const daysLeft =
                          endD && status === 'live'
                            ? Math.ceil(
                                (endD.getTime() - now.getTime()) / 86400000
                              )
                            : null;
                        const daysUntilStart =
                          startD && status === 'scheduled'
                            ? Math.ceil(
                                (startD.getTime() - now.getTime()) / 86400000
                              )
                            : null;
                        ...
                        const usagePct =
                          (p.maxUsage ?? 0) > 0
                            ? Math.min(
                                100,
                                Math.round(
                                  ((p.usageCount ?? 0) / p.maxUsage!) * 100
                                )
                              )
                            : null;

                        const applyCount =
                          (p.applyTo?.products?.length ?? 0) +
                          (p.applyTo?.categories?.length ?? 0) +
                          (p.applyTo?.brands?.length ?? 0);

                        return (
                          <div
                            key={p._id}
                            className={`flex gap-0 ${!p.active || expired ? 'opacity-50' : ''}`}
                          >
                            ...
                            <div
                              ...
                              style={{
                                backgroundColor:
                                  status === 'live' ? promoColor : '#e5e7eb',
                              }}
                            />

                            <div className="flex min-w-0 flex-1 items-start gap-3 px-5 py-4">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  {applyCount > 0 ? (
                                    <span className="text-[10px] text-gray-400">
                                      Applies to{' '}
                                      {[
                                        p.applyTo?.products?.length
                                          ? `${p.applyTo.products.length} product${p.applyTo.products.length > 1 ? 's' : ''}`
                                          : null,
                                        p.applyTo?.categories?.length
                                          ? `${p.applyTo.categories.length} categor${p.applyTo.categories.length > 1 ? 'ies' : 'y'}`
                                          : null,
                                        p.applyTo?.brands?.length
                                          ? `${p.applyTo.brands.length} brand${p.applyTo.brands.length > 1 ? 's' : ''}`
                                          : null,
                                      ]
                                        .filter(Boolean)
                                        .join(', ')}
                              ...
                              <div className="flex shrink-0 items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() => promoCrud.toggle(p._id)}
                                ...
                                <button
                                  type="button"
                                  onClick={() => promoCrud.delete(p._id)}
                                  className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-300 hover:bg-red-50 hover:text-red-500"
                                >
                                  <PiTrash className="h-4 w-4" />
                                </button>
            ...
            {tab === 'codes' && (
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Reusable codes cashiers enter at checkout — staff
                        discounts, vendor codes, etc.
                      </p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setEditCode('new')}
                ...
                {discountCodes.length === 0 ? (
                  <EmptyState
                    icon={<PiTag className="h-7 w-7 text-gray-300" />}
                  ...
                  <div className="divide-y divide-gray-50">
                    {discountCodes.map((d) => {
                      ...
                      const usagePct =
                        (d.maxUsage ?? 0) > 0
                          ? Math.min(
                              100,
                              Math.round(
                                ((d.usageCount ?? 0) / d.maxUsage!) * 100
                              )
                            )
                          : null;
                      const applyCount =
                        (d.applyTo?.products?.length ?? 0) +
                        (d.applyTo?.categories?.length ?? 0) +
                        (d.applyTo?.brands?.length ?? 0);
                      ...
                      return (
                        <div
                          key={d._id}
                          className={`flex gap-0 ${!d.active || expired ? 'opacity-50' : ''}`}
                        >
                          <div
                            className="w-1 shrink-0 rounded-l-sm"
                            style={{
                              backgroundColor:
                                d.active && !expired ? codeColor : '#e5e7eb',
                            }}
                          />
                            ...
                            <div className="min-w-0 flex-1 space-y-1.5">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                <button
                                  ...
                                  title="Click to copy"
                                  className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 font-mono text-xs font-black tracking-widest text-white transition-opacity hover:opacity-80"
                              ...
                              {(d.validFrom || d.validTo) && (
                                <p className="text-[10px] text-gray-400">
                                  {d.validFrom &&
                                    `From ${new Date(d.validFrom).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`}
                                  {d.validFrom && d.validTo && ' → '}
                                  {d.validTo &&
                              ...
                              <div className="flex flex-wrap items-center gap-1.5">
                                {applyCount > 0 ? (
                                  <span className="text-[10px] text-gray-400">
                                    {[
                                      d.applyTo?.products?.length
                                        ? `${d.applyTo.products.length} product${d.applyTo.products.length > 1 ? 's' : ''}`
                                        : null,
                                      d.applyTo?.categories?.length
                                        ? `${d.applyTo.categories.length} categor${d.applyTo.categories.length > 1 ? 'ies' : 'y'}`
                                        : null,
                                      d.applyTo?.brands?.length
                                        ? `${d.applyTo.brands.length} brand${d.applyTo.brands.length > 1 ? 's' : ''}`
                                        : null,
                                    ]
                                      .filter(Boolean)
                                      .join(', ')}
            ...
            {tab === 'bxgy' && (
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Quantity deals — Buy 2 Get 1 Free, Buy 3 get 1 at 50%
                        off, etc.
                      </p>
                    </div>
                  </div>
                ...
                {buyXGetY.length === 0 ? (
                  <EmptyState
                    icon={<PiShoppingCart className="h-7 w-7 text-gray-300" />}
                  ...
                  <div className="divide-y divide-gray-50">
                    {buyXGetY.map((b) => {
                      ...
                      return (
                        <div
                          key={b._id}
                          className={`flex gap-0 ${!b.active || expired ? 'opacity-50' : ''}`}
                        >
                          <div
                            className="w-1 shrink-0 rounded-l-sm"
                            style={{
                              backgroundColor: isLive ? offerColor : '#e5e7eb',
                            }}
                          />
                          ...
                          <div className="flex min-w-0 flex-1 items-start gap-3 px-5 py-4">
                            ...
                            <div
                              className="hidden shrink-0 flex-col items-center gap-0.5 rounded-xl border-2 px-3 py-2 text-center sm:flex"
                              style={{
                                borderColor: `${offerColor}50`,
                                backgroundColor: `${offerColor}0d`,
                              }}
                            >
                              <span
                              ...
                              <span className="text-[9px] font-bold leading-tight text-gray-400">
                                BUY
                              </span>
                              <div
                                className="my-0.5 w-6 border-t border-dashed"
                                style={{ borderColor: `${offerColor}60` }}
                              />
                              <span
                                className="text-2xl font-black leading-none"
                                style={{ color: offerColor }}
                            ...
                            <div className="min-w-0 flex-1 space-y-1.5">
                              ...
                              {(b.validFrom || b.validTo) && (
                                <p className="text-[10px] text-gray-400">
                                  ...
                                  {b.validFrom && b.validTo && ' → '}
                                  {b.validTo &&
                                    new Date(b.validTo).toLocaleDateString(
                                      'en-GB',
                                      {
                                        day: 'numeric',
                                        month: 'short',
                              ...
                              <div className="flex flex-wrap items-center gap-2 text-[10px] text-gray-400">
                                <span>
                                  ...
                                  {(b.buyProducts?.length ?? 0) > 0
                                    ? `${b.buyProducts!.length} product${b.buyProducts!.length > 1 ? 's' : ''}`
                                    : 'any'}
                                </span>
                                <span>·</span>
                                <span>
                                  Get:{' '}
                                  {(b.getProducts?.length ?? 0) > 0
                                    ? `${b.getProducts!.length} product${b.getProducts!.length > 1 ? 's' : ''}`
                                    : 'same'}
                                </span>
                                {b.description && (
                                  <>
            ...
            {tab === 'nextorder' && (
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3 px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          Automatically issue a discount coupon at the end of
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold ${nocEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-400'}`}
                      >
                ...
                <div
                  className={`space-y-3 ${!nocEnabled ? 'pointer-events-none opacity-40' : ''}`}
                >
                  {/* ── Earn Trigger ── */}
                  <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                    <div className="flex items-center gap-2 border-b border-gray-100 px-5 py-3.5">
                    ...
                    <div className="divide-y divide-gray-50">
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-gray-400">
                            Customer must spend at least this on the current
                            order to receive the coupon (₦0 = every order
                            qualifies)
                          </p>
                        </div>
                        <div className="relative shrink-0">
                          ...
                          <input
                            ...
                            value={nocMinOrder || ''}
                            onChange={(e) =>
                              setNocMinOrder(parseFloat(e.target.value) || 0)
                            }
                            placeholder="0"
                  ...
                  <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <div className="divide-y divide-gray-50">
                      ...
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          <p className="text-sm font-semibold text-gray-900">
                            Coupon colour & prefix
                          </p>
                          <p className="mt-0.5 text-xs text-gray-400">
                            Visual colour and code prefix for generated coupons
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-3">
                          <input
                            type="color"
                            value={nocColor}
                            onChange={(e) => setNocColor(e.target.value)}
                      ...
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-gray-400">
                            How much the generated coupon takes off the next
                            order
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-2">
                          ...
                          <input
                            type="number"
                            min={0}
                            max={nocType === 'pct' ? 100 : undefined}
                            value={nocValue}
                            onChange={(e) =>
                              setNocValue(parseFloat(e.target.value) || 0)
                            }
                            className="w-24 rounded-xl border border-gray-200 px-3 py-2.5 text-center text-sm font-bold outline-none focus:border-[#be185d]"
                          />
                      ...
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-gray-400">
                            Days before the generated coupon expires
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-2">
                          <input
                            ...
                            value={nocValidDays}
                            onChange={(e) =>
                              setNocValidDays(parseInt(e.target.value) || 1)
                            }
                            className="w-20 rounded-xl border border-gray-200 px-3 py-2.5 text-center text-sm font-bold outline-none focus:border-[#be185d]"
                          />
                      ...
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-gray-400">
                            The coupon can only be applied to orders above this
                            amount (₦0 = no minimum)
                          </p>
                        </div>
                        <div className="relative shrink-0">
                          ...
                          <input
                            type="number"
                            min={0}
                            value={nocMinRedeemOrder || ''}
                            onChange={(e) =>
                              setNocMinRedeemOrder(
                                parseFloat(e.target.value) || 0
                              )
                            }
                            placeholder="0"
                      ...
                      <div className="flex items-center justify-between gap-4 px-6 py-4">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-gray-400">
                            Each generated coupon can only be redeemed once
                          </p>
                        </div>
                  ...
                  <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                    <div className="flex items-center gap-2 border-b border-gray-100 px-5 py-3.5">
                      ...
                      <span className="text-xs text-gray-400">
                        — where can the coupon be used?
                      </span>
                    </div>
                    <div className="px-5 py-4">
                      <AvailableOnSection
                        value={nocAvailableOn}
                        onChange={setNocAvailableOn}
                      />
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    style={{ borderColor: `${nocColor}60` }}
                  >
                    <div
                      className="flex items-center gap-2 px-5 py-3.5"
                      style={{ backgroundColor: `${nocColor}0d` }}
                    >
                      <PiTicket
                        className="h-4 w-4"
                        style={{ color: nocColor }}
                      />
                      <p
                        className="text-[11px] font-bold uppercase tracking-wider"
                        style={{ color: nocColor }}
                      >
                        Coupon preview — how it appears at receipt
                      </p>
                    </div>
                    <div className="bg-white px-5 py-4">
                      {/* Ticket card */}
                      <div className="relative mx-auto max-w-sm overflow-hidden rounded-2xl shadow-md">
                        {/* Coloured left band */}
                        <div className="flex">
                          <div
                          ...
                          />
                          <div className="flex-1 rounded-r-2xl border border-gray-100 bg-white px-5 py-4">
                            <div className="mb-3 flex items-start justify-between gap-3">
                              <div>
                                ...
                                >
                                  {nocType === 'pct'
                                    ? `${nocValue}% OFF`
                                    : `₦${nocValue.toLocaleString()} OFF`}
                                </p>
                                {nocMinRedeemOrder > 0 && (
                                  <p className="mt-0.5 text-[9px] text-gray-400">
                                    on orders over ₦
                                    {nocMinRedeemOrder.toLocaleString()}
                                  </p>
                                )}
                              </div>
                              <div
                                className="rounded-xl px-3 py-2 text-center"
                                style={{ backgroundColor: `${nocColor}15` }}
                              >
                                <p
                            ...
                            <div className="mt-2.5 flex items-center justify-between border-t border-dashed border-gray-200 pt-2.5 text-[9px] text-gray-400">
                              <span>
                                Valid for {nocValidDays} days from today
                              </span>
                      ...
                      <div className="mt-4 flex items-start gap-2 text-xs text-gray-400">
                        <PiArrowRight className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                        <span>
                          {nocMinOrder > 0
                          ...
                          {nocMinRedeemOrder > 0
                            ? ` (orders ₦${nocMinRedeemOrder.toLocaleString()}+)`
                            : ''}
                        </span>
                      </div>
                    </div>
      ...
      {editCoupon && (
        <CouponModal
          initial={editCoupon === 'new' ? undefined : editCoupon}
      ...
      {editCode && (
        <DiscountCodeModal
          initial={editCode === 'new' ? undefined : editCode}
      ...
      {editPromo && (
        <PromotionModal
          initial={editPromo === 'new' ? undefined : editPromo}
      ...
      {editBxgy && (
        <BuyXGetYModal
          initial={editBxgy === 'new' ? undefined : editBxgy}
          onSave={handleSaveBxgy}
          onClose={() => setEditBxgy(null)}
          adminToken={token ?? undefined}
        />
      )}
    </div>
