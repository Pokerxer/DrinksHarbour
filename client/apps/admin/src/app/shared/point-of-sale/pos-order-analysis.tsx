...
function isSearchFilter(k: string) {
  return (
    k.startsWith('pos_search:') ||
    k.startsWith('customer_search:') ||
    k.startsWith('product_search:') ||
    k.startsWith('catname_search:')
  );
}
      ...
      (o.items || []).some((i) => {
        ...
        return (
          ...
          (m.catName.toLowerCase().includes(q) ||
            (m.subCatName ?? '').toLowerCase().includes(q))
        );
      }),
  },
] as const;
...
function applyFilters(
  ...
  if (filters.includes('not_cancelled'))
    r = r.filter(
      (o) => !o.isVoided && o.status !== 'voided' && o.status !== 'cancelled'
    );
  ...
  const posVals = filters
    .filter((f) => f.startsWith('pos_search:'))
    .map((f) => f.slice(11).toLowerCase());
  ...
  if (posVals.length > 0)
    r = r.filter((o) =>
      posVals.some((q) => cashierLabel(o.posStaff).toLowerCase().includes(q))
    );
...
function computeGroupData(
  ...
  const getBucket = (key: string): Bucket => {
    if (!groups[key])
      groups[key] = { orders: new Set(), orderList: [], items: [] };
    return groups[key];
  };
    ...
    if (isItemGroup) {
      ...
      items.forEach((item) => {
        ...
        if (groupBy === 'product') {
          ...
          key =
            prodMeta[item.name]?.subCatName ||
            prodMeta[item.name]?.catName ||
            'Uncategorized';
        } else {
          key = prodMeta[item.name]?.brandName || 'No Brand';
        }
        const b = getBucket(key);
        if (!b.orders.has(o._id)) {
          b.orders.add(o._id);
          b.orderList.push(o);
        }
        b.items.push(item);
  ...
  return Object.entries(groups).map(([key, { orderList, items }]) => {
    ...
    switch (measure) {
      case 'total_price':
        value = isItemGroup
          ? items.reduce(
              (s, i) => s + (i.itemSubtotal ?? i.priceAtPurchase * i.quantity),
              0
            )
      ...
      case 'avg_price':
        value = isItemGroup
          ? items.length > 0
            ? items.reduce((s, i) => s + i.priceAtPurchase, 0) / items.length
            : 0
          : orderList.length > 0
            ? orderList.reduce((s, o) => s + (o.total ?? 0), 0) /
              orderList.length
      ...
      case 'profit': {
        ...
        if (isItemGroup) {
          ...
          value = orderList.reduce((s, o) => {
            const lineProfit = (o.items || []).reduce(
              (cs, i) => cs + profitFn(i),
              0
            );
            const hasCost = (o.items || []).some(
              (i) => (i.sizeCostPrice ?? 0) > 0
            );
    ...
    if (groupBy === 'order_day') {
      ...
      label = new Date(parseInt(yr), parseInt(mo) - 1, 1).toLocaleDateString(
        'en-US',
...
function orderWarehouseName(o: PosOrder): string {
  return (
    o.items?.find((i) => (i as any).warehouse)?.warehouse?.name ??
    'No warehouse'
  );
}

function getOrderG1Key(
  o: PosOrder,
  groupBy: GroupByKey,
  prodMeta: Record<string, ProdMeta>
): string {
  ...
  if (groupBy === 'payment_method')
    return METHOD_LABEL[o.paymentMethod] || o.paymentMethod || 'Other';
  if (groupBy === 'order_day')
    return new Date(o.placedAt || o.createdAt).toISOString().split('T')[0];
  ...
  if (groupBy === 'order_year')
    return String(new Date(o.placedAt || o.createdAt).getFullYear());
  const firstName = o.items?.[0];
  const itemName = firstName
    ? firstName.variant
      ? `${firstName.name} · ${firstName.variant}`
      : firstName.name
    : 'Unknown';
  if (groupBy === 'product') return itemName;
  if (groupBy === 'product_category')
    return prodMeta[o.items?.[0]?.name || '']?.catName || 'Uncategorized';
  if (groupBy === 'subcategory') {
    const m = prodMeta[o.items?.[0]?.name || ''];
    return m?.subCatName || m?.catName || 'Uncategorized';
  }
  if (groupBy === 'brand')
    return prodMeta[o.items?.[0]?.name || '']?.brandName || 'No Brand';
  return itemName;
}

function formatG1Label(isoKey: string, groupBy: GroupByKey): string {
  if (groupBy === 'order_day')
    return new Date(isoKey).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
    });
  if (groupBy === 'order_week') {
    const [yr, w] = isoKey.split('-W');
    return `W${w} ${yr}`;
  }
  if (groupBy === 'order_month') {
...
function computeMultiSeries(
  ...
  Object.values(nested).forEach((m) =>
    Object.keys(m).forEach((k) => seriesSet.add(k))
  );
  const series = Array.from(seriesSet).sort();

  const rows: GroupRow2[] = Object.entries(nested).map(([g1Key, g2Map]) => {
    const row: GroupRow2 = {
      label: formatG1Label(g1Key, groupBy),
      isoKey: g1Key,
      __total__: 0,
    };
    series.forEach((s) => {
      ...
      switch (measure) {
        case 'total_price':
          val = ords.reduce((a, o) => a + (o.total ?? 0), 0);
          break;
        case 'count':
          val = ords.length;
          break;
        case 'avg_price':
          val =
            ords.length > 0
              ? ords.reduce((a, o) => a + (o.total ?? 0), 0) / ords.length
              : 0;
          break;
        case 'product_qty':
          val = ords.reduce(
          ...
          break;
        case 'line_count':
          val = ords.reduce((a, o) => a + (o.items || []).length, 0);
          break;
        case 'subtotal':
          val = ords.reduce((a, o) => a + (o.subtotal ?? o.total ?? 0), 0);
          break;
        case 'subtotal_notax':
          val = ords.reduce((a, o) => a + (o.subtotal ?? o.total ?? 0), 0);
          break;
        case 'total_discount':
          val = ords.reduce((a, o) => a + (o.discountTotal ?? 0), 0);
          break;
        case 'profit':
          val = ords.reduce((a, o) => {
          ...
          break;
        case 'delay_validation': {
          const delays = ords.map((o) =>
            Math.max(
              0,
              (new Date(o.placedAt || o.createdAt).getTime() -
          ...
          val =
            delays.length > 0
              ? delays.reduce((a, d) => a + d, 0) / delays.length
              : 0;
          break;
        }
      }
...
const ITEM_DIMS = new Set<GroupByKey>([
  'product',
  'product_category',
  'subcategory',
  'brand',
  ...
  switch (measure) {
    ...
    case 'avg_price':
      return isItemGroup
        ? items.length > 0
          ...
          : 0
        : orderList.length > 0
          ? orderList.reduce((s, o) => s + (o.total ?? 0), 0) / orderList.length
          : 0;
    case 'product_qty':
    ...
    case 'subtotal_notax':
      return isItemGroup
        ...
        : orderList.reduce((s, o) => s + (o.subtotal ?? o.total ?? 0), 0);
    case 'total_discount':
      return orderList.reduce((s, o) => s + (o.discountTotal ?? 0), 0);
    case 'profit': {
      const pFn = (i: OrderItem) => {
        const sp = i.sizeCostPrice ?? 0;
        const rev = i.itemSubtotal ?? i.priceAtPurchase * i.quantity;
...
function computePivot(
  ...
  orders.forEach((o) => {
    ...
    if (!eitherIsItem) {
      ...
      effItems.forEach((item) => {
        const rk = rowIsItem
          ? getItemDimKey(item, rowDim, prodMeta)
          : getOrderDimKey(o, rowDim);
        const ck = colIsItem
          ? getItemDimKey(item, colDim, prodMeta)
          : getOrderDimKey(o, colDim);
        rowSet.add(rk);
        colSet.add(ck);
        if (!cells[rk]) cells[rk] = {};
        if (!cells[rk][ck]) cells[rk][ck] = { orders: new Map(), items: [] };
  ...
  rowKeys.forEach((rk) => {
    rowTotals[rk] = colKeys.reduce(
      (s, ck) => s + (valueGrid[rk]?.[ck] ?? 0),
      0
    );
    rowOrders[rk] = Array.from(
      new Set(
        Object.values(cells[rk] ?? {}).flatMap((c) =>
          Array.from(c.orders.keys())
        )
      )
    ).length;
  });
  colKeys.forEach((ck) => {
    colTotals[ck] = rowKeys.reduce(
      (s, rk) => s + (valueGrid[rk]?.[ck] ?? 0),
      0
    );
  });
...
function drillFmtDate(d: string) {
...
function drillFmtTime(d: string) {
...
function drillStatusBadge(o: PosOrder) {
  ...
  const amt = o.total ?? 0;
  if (o.isVoided || o.status === 'voided')
    return { label: 'Voided', cls: 'bg-gray-100 text-gray-500' };
  if (ref >= amt && ref > 0)
    return { label: 'Refunded', cls: 'bg-red-50 text-red-600' };
  if (ref > 0)
    return { label: 'Part. Returned', cls: 'bg-amber-50 text-amber-600' };
  return { label: 'Paid', cls: 'bg-emerald-50 text-emerald-600' };
}
function drillCustomer(
  c?: { firstName?: string; lastName?: string; phone?: string } | null
) {
  return c?.firstName && c.firstName !== 'Walk-in'
...
function DrillOrderDetail({
  ...
  const payLabel =
    splits.length > 0
      ? splits
          .map(
            (s) =>
              `${s.method.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())} ${ng(s.amount)}`
          )
          .join(' + ')
      : (order.paymentMethod || '')
          .replace(/_/g, ' ')
  ...
  const sessionStr = order.session
    ? (() => {
        ...
        const date = order.session.openedAt
          ? new Date(order.session.openedAt).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
            })
          : '';
      ...
      <div className="flex shrink-0 items-start justify-between gap-3 border-b border-gray-100 px-6 py-4">
        <div className="flex min-w-0 items-center gap-3">
          <button
            type="button"
            onClick={onBack}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-700"
          >
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${stCls}`}
              >
                {stLabel}
              </span>
            </div>
            <p className="mt-0.5 text-[11px] text-gray-400">
              {drillFmtDate(order.placedAt || order.createdAt)} ·{' '}
              {drillFmtTime(order.placedAt || order.createdAt)}
            </p>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            type="button"
            onClick={() => printInvoices([order], DEFAULT_STORE)}
            title="Print invoice"
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition-colors hover:bg-gray-50 hover:text-[#b20202]"
          >
      ...
      <div className="flex shrink-0 border-b border-gray-100 text-xs font-semibold">
        {(
          [
            ...
            {
              id: 'returns',
              label: `Returns${(order.refunds?.length ?? 0) > 0 ? ` (${order.refunds!.length})` : ''}`,
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100 bg-gray-50/50">
            {[
              ...
              {
                label: 'Returned',
                value: refunded > 0 ? formatCurrency(refunded) : '—',
                amber: refunded > 0,
              },
              ...
              <div key={label} className="px-4 py-3 text-center">
                ...
                <p
                  className={`mt-0.5 text-sm font-bold tabular-nums ${red ? 'text-[#b20202]' : amber ? 'text-amber-600' : 'text-gray-900'}`}
                >
                  {value}
                </p>
              </div>
          ...
          <div className="space-y-2 border-b border-gray-100 px-6 py-4 text-xs">
            {[
              { label: 'Cashier', value: cashierLabel(order.posStaff) },
              {
                label: 'Customer',
                value: drillCustomer(order.customer) || 'Walk-in Customer',
              },
              { label: 'Session', value: sessionStr },
              { label: 'Payment', value: payLabel },
              ...(change > 0
                ? [{ label: 'Change', value: formatCurrency(change) }]
                : []),
          ...
          {(order.items || []).length > 0 && (
            <div>
              ...
              <div className="space-y-1 border-t border-gray-100 px-6 py-3 text-xs">
                {discount > 0 && (
                ...
                <div key={i} className="px-6 py-4">
                  ...
                  {r.refundedAt && (
                    <p className="text-[11px] text-gray-400">
                      {drillFmtDate(r.refundedAt)} ·{' '}
                      {drillFmtTime(r.refundedAt)}
                    </p>
                  )}
                  {r.paymentMethod && (
                    <p className="mt-0.5 text-[11px] capitalize text-gray-400">
                      via {r.paymentMethod.replace(/_/g, ' ')}
                    </p>
                  )}
                </div>
              ))}
...
function OrderDrillDrawer({
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter === 'paid')
      list = list.filter(
        (o) =>
          !o.isVoided &&
          (o.refunds || []).reduce((s, r) => s + r.totalRefunded, 0) === 0
      );
    if (statusFilter === 'refunded')
      list = list.filter((o) => {
        const r = (o.refunds || []).reduce((s, r) => s + r.totalRefunded, 0);
        return r > 0;
      });
    if (statusFilter === 'voided')
      list = list.filter((o) => o.isVoided || o.status === 'voided');
    ...
    list.sort((a, b) => {
      ...
      switch (sortCol) {
        ...
        case 'customer':
          ...
          break;
        case 'cashier':
          cmp = cashierLabel(a.posStaff).localeCompare(
            cashierLabel(b.posStaff)
          );
          break;
        case 'method':
        ...
        case 'total':
          ...
          break;
        case 'status':
          cmp = drillStatusBadge(a).label.localeCompare(
            drillStatusBadge(b).label
          );
          break;
      }
  ...
  const allChecked =
    filtered.length > 0 && filtered.every((o) => checked.has(o._id));
          ...
          <>
            ...
            <div className="grid shrink-0 grid-cols-2 gap-3 border-b border-gray-100 bg-gray-50/40 px-6 py-4 sm:grid-cols-4">
              {[
                ...
                },
                {
                  ...
                  sub: voidedCount > 0 ? `${voidedCount} voided` : undefined,
                  accent: '#4f46e5',
                },
                {
                  label: 'Avg Order',
            ...
            <div className="flex shrink-0 items-center gap-3 border-b border-gray-100 px-6 py-2.5">
              <div className="flex rounded-xl border border-gray-200 bg-gray-50/70 p-0.5 text-xs font-semibold">
                {(
                  ['all', 'paid', 'refunded', 'voided'] as DrillStatusFilter[]
                ).map((f) => (
                  <button
                    key={f}
                    type="button"
                ...
                {checked.size > 0 && (
                  <div className="flex items-center gap-3 border-b border-[#b20202]/20 bg-[#fef2f2] px-5 py-2.5">
                    <span className="flex-1 text-xs font-semibold text-gray-700">
                      <span className="font-bold text-[#b20202]">
                        {checked.size}
                      </span>{' '}
                      selected ·{' '}
                      ...
                      {filtered.map((o) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Date */}
                            <td
                              className="cursor-pointer px-3 py-2.5 text-gray-500"
                            ...
                            >
                              ...
                              {(o.isVoided || o.status === 'voided') && (
                                <span className="rounded bg-red-100 px-1 py-px text-[9px] font-bold uppercase text-red-600">
                                  void
                                </span>
                              )}
                            </td>
                            ...
                            {/* Customer */}
                            <td
                              className="max-w-[80px] cursor-pointer truncate px-3 py-2.5 text-[11px]"
                              onClick={() => setSelectedOrder(o)}
                            >
                              {custName ? (
                            ...
                            {/* Cashier */}
                            <td
                              className="cursor-pointer px-3 py-2.5 text-[11px] text-gray-600"
                              onClick={() => setSelectedOrder(o)}
                            >
                            ...
                            {/* Total */}
                            <td
                              className={`cursor-pointer px-3 py-2.5 text-right font-bold tabular-nums ${o.isVoided ? 'text-gray-300 line-through' : 'text-gray-900'}`}
                              onClick={() => setSelectedOrder(o)}
                            >
                            ...
                            >
                              <span
                                className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${stCls}`}
                              >
                                {stLabel}
                              </span>
                            </td>
    ...
    p.colVals0.forEach((ck) => {
      if (colDims.length >= 2 && expandedCols.has(ck)) {
        (p.subColValsMap[ck] ?? []).forEach((sk) => {
          visCols.push({
            path: [ck, sk],
            label: `${fmtDimKey(ck, colDims[0])} / ${fmtDimKey(sk, colDims[1])}`,
          });
        });
      } else {
  ...
  p.rowVals0.forEach((rk) => {
    ...
    if (rowDims.length >= 2 && expandedRows.has(rk)) {
      (p.subRowValsMap[rk] ?? []).forEach((srk) => {
        const sub = [
          `  ${fmtDimKey(rk, rowDims[0])} / ${fmtDimKey(srk, rowDims[1])}`,
          numVal([rk, srk], []),
        ];
...
function exportPivotExcel(
  ...
  p.rowVals0.forEach((rk) => {
    const rowCells = [
      strCell(fmtDimKey(rk, rowDims[0])),
  ...
  return (
    <div className="relative rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to right, ${accentColor}80, ${accentColor}10)`,
        }}
      />
    </div>
  );
...
function PiePanelView({
  ...
  const chartData =
    othersVal > 0
      ? [
          ...pieData,
          {
            label: `${othersRaw.length} others`,
            value: othersVal,
            orders: othersOrd,
            isoKey: '_others',
            orderList: othersOrds,
          },
        ]
      : pieData;
  ...
  const pieAvg =
    pieData.length > 0
      ? pieData.reduce((s, r) => s + r.value, 0) / pieData.length
      : 0;
  const pieMax = pieData.length > 0 ? pieData[0].value : 1;
  const getColor = (i: number) =>
    i >= pieData.length ? '#94a3b8' : PALETTE[i % PALETTE.length];
  ...
  const PieTooltip = ({
    ...
    active?: boolean;
    payload?: { payload: PieRow; fill: string }[];
  }) => {
  ...
  const activePct =
    activeSlice && pieTotal > 0 ? (activeSlice.value / pieTotal) * 100 : 0;
  const activeDiff =
    activeSlice && !activeIsOthers ? activeSlice.value - pieAvg : 0;
    ...
    <div className={`flex w-full flex-col gap-0 sm:flex-row ${h}`}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              onClick={(d: PieRow) => {
                if (onDrill && d.orderList?.length > 0)
                  onDrill(d.label, d.orderList);
              }}
              paddingAngle={2}
            ...
            >
              {chartData.map((_, i) => (
                <Cell
                  ...
                  fill={getColor(i)}
                  opacity={
                    activeIdx === undefined || activeIdx === i ? 1 : 0.38
                  }
                />
              ))}
            </Pie>
          ...
          {activeSlice ? (
            <>
              ...
              <p
                ...
                style={{ color: getColor(activeIdx!) }}
              >
                {activePct.toFixed(1)}%
              </p>
              {!activeIsOthers && (
                <span
                  className={`mt-1 inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                    activeDiff >= 0
                ...
                >
                  {activeDiff >= 0 ? '▲' : '▼'}{' '}
                  {fmtCompact(Math.abs(activeDiff))} vs avg
                </span>
              )}
              <p className="mt-1 text-[10px] text-gray-400">
                {activeSlice.orders.toLocaleString()} orders
              </p>
              {canDrill && activeSlice.orderList?.length > 0 && (
                <p className="mt-0.5 text-[10px] font-semibold text-[#b20202] opacity-70">
                  ↗ click to drill
                </p>
              )}
            </>
      ...
      <div className="flex w-full flex-col justify-center overflow-y-auto border-t border-gray-100 px-4 py-3 sm:w-72 sm:border-l sm:border-t-0 sm:py-4">
        {pieData.map((d, i) => {
          ...
          return (
            <div
              ...
              style={{
                borderLeft: `3px solid ${isAct ? color : 'transparent'}`,
              }}
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-1.5">
                  ...
                  <span
                    className={`truncate text-xs font-medium transition-colors ${isAct ? 'text-gray-900' : 'text-gray-600'}`}
                  >
                    {d.label}
                  </span>
                </div>
              ...
              >
                <div className="mt-2 grid grid-cols-3 gap-2 border-t border-gray-100 pt-2">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[11px] font-semibold text-gray-800">
                      {d.orders > 0 ? fmtCompact(avgPO) : '—'}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-gray-400">vs avg</p>
                    <p
                      className={`text-[11px] font-semibold ${diff >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
                    >
                      {diff >= 0 ? '+' : ''}
                      {fmtCompact(diff)}
                    </p>
        ...
        {othersVal > 0 && (
          <div
            className={`rounded-xl py-2 pl-3 pr-3 transition-all duration-150 ${canDrill ? 'cursor-pointer' : ''} ${
              activeIdx === pieData.length
                ? 'bg-gray-50'
                : 'hover:bg-gray-50/70'
            }`}
            style={{
              borderLeft: `3px solid ${activeIdx === pieData.length ? '#94a3b8' : 'transparent'}`,
            }}
            onMouseEnter={() => setActiveIdx(pieData.length)}
            onMouseLeave={() => setActiveIdx(undefined)}
            onClick={() => {
              if (onDrill && othersOrds.length > 0)
                onDrill(`${othersRaw.length} others`, othersOrds);
            }}
          >
            <div className="flex items-center justify-between gap-2">
              <div className="flex min-w-0 items-center gap-1.5">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <span className="text-xs font-semibold tabular-nums text-gray-400">
                  {fmtMeasureVal(othersVal, measure)}
                </span>
              </div>
            </div>
          </div>
...
function MainChart({
  ...
  const measureLabel =
    MEASURES.find((m) => m.key === measure)?.label ?? 'Value';
  ...
  if (data.length === 0) {
    return (
      <div
        className={`flex items-center justify-center text-sm text-gray-400 ${h}`}
      >
  ...
  if (chartType === 'table') {
    ...
    const rows = displayed.map((r) => {
      ...
      return {
        ...r,
        pct: grandTotal > 0 ? (r.value / grandTotal) * 100 : 0,
        cumPct: grandTotal > 0 ? (runningVal / grandTotal) * 100 : 0,
      };
    });
    ...
    const SortBtn = ({
      ...
      >
        {children}
        <span
          className={`text-[10px] ${sortCol === col ? 'text-[#b20202]' : 'text-gray-300'}`}
        >
          {sortCol === col ? (sortAsc ? '↑' : '↓') : '↕'}
        </span>
      </button>
      ...
      <div className={`flex flex-col ${h}`}>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-4 border-b border-gray-100 px-5 py-3">
          ...
          <div className="flex items-center gap-6 text-xs text-gray-500">
            <span>
              ...
              {search ? ` of ${data.length}` : ''} groups
            </span>
            <span>
              Total:{' '}
              <span className="font-semibold text-gray-800">
                {fmtMeasureVal(grandTotal, measure)}
              </span>
            </span>
            <span>
              Orders:{' '}
              <span className="font-semibold text-gray-800">
                {grandOrds.toLocaleString()}
              </span>
            </span>
            {showAvgCol && grandOrds > 0 && (
              <span>
                Avg/order:{' '}
                <span className="font-semibold text-gray-800">
                  {fmtMeasureVal(grandTotal / grandOrds, measure)}
                </span>
              </span>
            )}
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <table className="w-full text-sm">
            ...
            >
              <tr>
                <th className="w-10 py-3 pl-5 text-left text-xs font-semibold text-gray-400">
                  #
                </th>
                ...
                <th className="py-3 pr-5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">
                  <div className="flex justify-end">
                    <SortBtn col="value">{measureLabel}</SortBtn>
                  </div>
                </th>
                <th className="py-3 pr-5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">
                  Share
                ...
                <th className="py-3 pr-5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">
                  <div className="flex justify-end">
                    <SortBtn col="orders">Orders</SortBtn>
                  </div>
                </th>
                {showAvgCol && (
                  <th className="py-3 pr-5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="relative py-3.5 pr-5 text-right">
                        <div
                          className="absolute inset-y-2 left-0 rounded-r"
                      ...
                      <td className="py-3.5 pr-5">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="w-10 text-right text-xs font-medium tabular-nums text-gray-600">
                            {row.pct.toFixed(1)}%
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Avg per order */}
                      {showAvgCol && (
                        <td className="py-3.5 pr-5 text-right text-xs tabular-nums text-gray-500">
                          {row.orders > 0
                            ? fmtMeasureVal(row.value / row.orders, measure)
                            : '—'}
                        </td>
                      )}

                      {/* Cumulative % */}
                      <td className="py-3.5 pr-5 text-right text-xs tabular-nums text-gray-400">
            ...
            >
              <tr>
                ...
                <td className="py-3.5 pr-5 text-right text-sm font-bold tabular-nums text-gray-900">
                  {fmtMeasureVal(
                    search ? rows.reduce((s, r) => s + r.value, 0) : grandTotal,
                    measure
                  )}
                </td>
                ...
                {showAvgCol && (
                  <td className="py-3.5 pr-5 text-right text-xs tabular-nums text-gray-600">
                    {grandOrds > 0
                      ? fmtMeasureVal(grandTotal / grandOrds, measure)
                      : '—'}
                  </td>
  ...
  const CustomTooltip = ({
    ...
    active?: boolean;
    payload?: { value: number; name: string; payload: GroupRow }[];
    label?: string;
  }) => {
    if (!active || !payload?.length) return null;
    ...
    return (
      <div className="min-w-[170px] rounded-2xl border border-gray-100 bg-white p-3.5 shadow-xl">
        <p className="mb-2 truncate text-xs font-semibold text-gray-500">
          {label}
        </p>
        ...
        <div className="mt-2 space-y-1 border-t border-gray-100 pt-2">
          ...
          <div className="flex items-center justify-between gap-4 text-[11px] text-gray-500">
            <span>vs avg</span>
            <span
              className={`font-semibold ${aboveAvg ? 'text-emerald-600' : 'text-red-500'}`}
            >
              {aboveAvg ? '+' : ''}
              {fmtMeasureVal(diff, measure)}
            </span>
          </div>
          {row?.orders > 0 && (
            <div className="flex items-center justify-between gap-4 text-[11px] text-gray-500">
        ...
        {onBarClick && (
          <p className="mt-2 text-[10px] font-medium text-[#b20202]">
            Click to drill through
          </p>
        )}
      </div>
          ...
          <div className="mt-2 space-y-1 border-t border-gray-100 pt-2">
            ...
            {delta !== null && (
              <div className="flex items-center justify-between gap-4 text-[11px] text-gray-500">
                ...
                >
                  {delta >= 0 ? '▲' : '▼'}{' '}
                  {fmtMeasureVal(Math.abs(delta), measure)}
                </span>
              </div>
            )}
            <div className="flex items-center justify-between gap-4 text-[11px] text-gray-500">
              <span>vs avg</span>
              <span
                className={`font-medium ${val >= avg ? 'text-emerald-600' : 'text-red-500'}`}
              >
                {val >= avg ? '+' : ''}
                {fmtMeasureVal(val - avg, measure)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-x-8 gap-y-1 border-b border-gray-100 px-4 pb-3 pt-1">
          ...
          <div className={statCls}>
            <span className={statLabel}>
              {isTimeSeries ? 'Avg / period' : 'Avg / group'}
            </span>
          ...
          <div className={`${statCls} ml-auto`}>
            ...
            <span
              className={`text-sm font-bold ${trend >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
            >
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <ReferenceLine
                ...
                strokeWidth={1.5}
                label={{
                  value: `Avg ${fmtAxisVal(avg, measure)}`,
      ...
      <div className={`flex w-full flex-col ${h}`}>
        <div className="flex shrink-0 flex-wrap items-center gap-x-8 gap-y-1 border-b border-gray-100 px-4 pb-3 pt-1">
          <div className={sc}>
            ...
            <span className={sv}>{fmtMeasureVal(total, measure)}</span>
          </div>
          <div className={sc}>
            <span className={sl}>Avg / period</span>
  ...
  if (data.length <= 20) {
    ...
    const yWidth = Math.min(
      ...
      Math.max(
        120,
        data.reduce((m, r) => Math.max(m, r.label.length * 6.5), 0)
      )
    );
    return (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            ...
            margin={{
              ...
              right: 12,
              left: -4,
              bottom: display.length > 20 ? 56 : 32,
            }}
          >
            <defs>
              ...
              {display.map((row, i) => (
                <Cell
                  ...
                  fill={
                    i === 0
                      ? topColor
                      : `${topColor}${Math.round((1 - i * 0.015) * 255)
                          .toString(16)
                          .padStart(2, '0')}`
                  }
                  onClick={() => onBarClick?.(row.label, row.orderList)}
...
function FilterListSection({
  ...
  const matched = filter
    ? items.filter((i) => i.name.toLowerCase().includes(lc))
    : items;
  const visible = showAll ? matched : matched.slice(0, maxVisible);
  const activeCount = items.filter((i) =>
    activeFilters.includes(`${prefix}${i._id}`)
  ).length;
...
function StackedChart({
        ...
        <table className="w-full min-w-[400px] text-sm">
          <thead className="sticky top-0 bg-white">
            <tr className="border-b border-gray-200">
              ...
              {series.map((s) => (
                <th
                  key={s}
                  className="py-2 pr-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500"
                >
                  {s}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr
                key={row.isoKey}
                className={`border-b border-gray-50 ${i % 2 ? 'bg-gray-50/40' : ''}`}
              >
                ...
                {series.map((s) => {
                  ...
                  return (
                    <td
                      key={s}
                      className={`py-2 pr-3 text-right tabular-nums text-gray-700 ${cellOrds.length > 0 && onBarClick ? 'cursor-pointer rounded hover:bg-gray-100' : ''}`}
                      onClick={() => {
                        if (cellOrds.length > 0)
                          onBarClick?.(row.label, s, cellOrds);
                      }}
                    >
                ...
                <td
                  className={`py-2 text-right font-semibold tabular-nums text-gray-900 ${onBarClick ? 'cursor-pointer rounded hover:bg-gray-100' : ''}`}
                  onClick={() => {
                    const allOrds = Object.values(
                      orderMap?.[row.isoKey] ?? {}
                    ).flat();
                    if (allOrds.length > 0)
                      onBarClick?.(row.label, '', allOrds);
                  }}
                >
          ...
          <tfoot>
            <tr className="border-t-2 border-gray-200 bg-white">
              <td className="py-2 pr-4 text-xs font-bold uppercase text-gray-700">
                Total
              </td>
              {series.map((s) => {
                const colTotal = rows.reduce(
                  (acc, r) => acc + ((r[s] as number) || 0),
                  0
                );
                return (
                  <td
                    key={s}
                    className="py-2 pr-3 text-right text-sm font-bold tabular-nums text-gray-900"
                  >
                    {fmtMeasureVal(colTotal, measure)}
                  </td>
                );
              })}
              <td className="py-2 text-right text-sm font-bold tabular-nums text-gray-900">
                {fmtMeasureVal(grandTotal, measure)}
              </td>
            </tr>
          </tfoot>
  ...
  if (chartType === 'pie') {
    const seriesPieRows: PieRow[] = series
      .map((s) => {
        ...
        const ords = Object.values(orderMap ?? {}).flatMap(
          (rowMap) => rowMap[s] ?? []
        );
        return {
          label: s,
  ...
  if (chartType === 'line') {
    const TIME_KEYS = new Set<GroupByKey>([
      'order_day',
      'order_week',
      'order_month',
      'order_quarter',
    ...
    const trend =
      (rows[rows.length - 1]?.__total__ ?? 0) - (rows[0]?.__total__ ?? 0);
          ...
          {series.length > 1 && (
            <div className="mt-2 flex items-center justify-between border-t border-gray-100 pt-2 text-[10px]">
              ...
              <span className="font-semibold tabular-nums text-gray-700">
                {fmtMeasureVal(rowTotal, measure)}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-x-8 gap-y-1 border-b border-gray-100 px-4 pb-3 pt-1">
          ...
          {isTimeSeries && (
            <div className={`${sc} ml-auto`}>
              ...
              >
                {trend >= 0 ? '▲' : '▼'}{' '}
                {fmtMeasureVal(Math.abs(trend), measure)}
              </span>
            </div>
          )}
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart
              ...
              onClick={(d) => {
                ...
                const ords =
                  orderMap?.[row.isoKey]?.[sk] ??
                  Object.values(orderMap?.[row.isoKey] ?? {}).flat();
  ...
  const isHorizontal =
    chartType === 'bar' && rows.length > 1 && rows.length <= 12;
  const grandTotal = rows.reduce((s, r) => s + r.__total__, 0);
  ...
  const yWidth = isHorizontal
    ? Math.min(
        ...
        Math.max(
          100,
          rows.reduce((m, r) => Math.max(m, r.label.length * 6.5), 0)
        )
      )
    : 54;
  ...
  const StackedBarTooltip = ({
    ...
    const segments = payload
      .filter((p) => p.dataKey !== '__total__' && (p.value || 0) > 0)
      .sort((a, b) => b.value - a.value);
      ...
      <div className="min-w-[200px] rounded-2xl border border-gray-100 bg-white p-3.5 shadow-xl">
          ...
          {segments.map((p) => (
            ...
            >
              <div className="flex min-w-0 items-center gap-1.5">
                ...
                />
                <span className="max-w-[130px] truncate text-gray-600">
                  {p.name}
                </span>
              </div>
              <span className="shrink-0 font-semibold tabular-nums text-gray-800">
        ...
        <div className="mt-2 space-y-1 border-t border-gray-100 pt-2">
          ...
          <div className="flex items-center justify-between gap-4 text-[11px] text-gray-500">
            <span>vs avg</span>
            <span
              className={`font-semibold ${diff >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
            >
              {diff >= 0 ? '+' : ''}
              {fmtMeasureVal(diff, measure)}
            </span>
  ...
  return (
    <div className={`flex w-full flex-col ${h}`}>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-x-8 gap-y-1 border-b border-gray-100 px-4 pb-2 pt-1">
        ...
        <div className={sc}>
          <span className={sl}>Avg / group</span>
          <span className={sv}>{fmtMeasureVal(grandAvg, measure)}</span>
        </div>
        <div className={sc}>
          <span className={sl}>Groups</span>
          ...
          >
            ...
            {isHorizontal ? (
              <>
                <XAxis
                  ...
                  domain={[
                    0,
                    rows.reduce((m, r) => Math.max(m, r.__total__), 0) * 1.2,
                  ]}
                />
                <YAxis
                  type="category"
            ...
            {isHorizontal ? (
              ...
              <ReferenceLine
                ...
                strokeWidth={1.5}
                label={{
                  value: `Avg ${fmtAxisVal(grandAvg, measure)}`,
                  position: 'insideTopRight',
                  fontSize: 10,
                  fill: '#94a3b8',
                  dy: -8,
...
export default function POSOrderAnalysis() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    posApi
      .getAllOrders(token, { limit: 500 })
      .then((d) => setOrders(Array.isArray(d) ? d : []))
      .catch((err) => {
        console.error('[OrderAnalysis] fetch failed:', err);
        setOrders([]);
      })
  ...
  useEffect(() => {
    function handler(e: MouseEvent) {
      ...
      if (
        pivotAddRowRef.current &&
        !pivotAddRowRef.current.contains(e.target as Node)
      ) {
        setPivotAddRowOpen(false);
      }
      if (
        pivotAddColRef.current &&
        !pivotAddColRef.current.contains(e.target as Node)
      ) {
  ...
  const filtered = useMemo(() => {
    ...
    if (
      activeFilters.includes('date_custom') &&
      customDateStart &&
      customDateEnd
    ) {
  ...
  const itemFilter = useMemo((): ((item: OrderItem) => boolean) | null => {
    const pVals = activeFilters
      .filter((f) => f.startsWith('product_search:'))
      .map((f) => f.slice(15).toLowerCase());
    const cVals = activeFilters
      .filter((f) => f.startsWith('catname_search:'))
      .map((f) => f.slice(15).toLowerCase());
  ...
  const filteredScoped = useMemo((): PosOrder[] => {
    ...
    return filtered.map((o) => {
      ...
      const filtSubtotal = filtItems.reduce(
        (s, i) => s + (i.itemSubtotal ?? i.priceAtPurchase * i.quantity),
        0
      );
      const allSubtotal = (o.items || []).reduce(
        (s, i) => s + (i.itemSubtotal ?? i.priceAtPurchase * i.quantity),
        0
      );
      ...
      return {
        ...
        discountTotal:
          Math.round((o.discountTotal ?? 0) * discShare * 100) / 100,
      } as PosOrder;
    });
  }, [filtered, itemFilter]);
  ...
  const chartData = useMemo(() => {
    ...
    if (isTimeSeries)
      return [...data].sort((a, b) => a.isoKey.localeCompare(b.isoKey));
    const stack =
      sortStack.length > 0
        ? sortStack
        : [{ field: 'value' as SortField, dir: 'desc' as const }];
  ...
  const multiSeriesData = useMemo(() => {
    ...
    const result = computeMultiSeries(
      filteredScoped,
      groupBy,
      groupBy2,
      measure,
  ...
  const analytics = useMemo(() => {
    const active = filteredScoped.filter(
      (o) => !o.isVoided && o.status !== 'voided'
    );
    const voided = filteredScoped.filter(
      (o) => o.isVoided || o.status === 'voided'
    );

    const totalRevenue = active.reduce((s, o) => s + (o.total ?? 0), 0);
    const grossRevenue = active.reduce(
      (s, o) => s + (o.subtotal ?? o.total ?? 0),
      0
    );
    const totalOrders = active.length;
    const avgOrder = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    const totalRefunds = active.reduce(
      (s, o) =>
        s + (o.refunds || []).reduce((r, ref) => r + ref.totalRefunded, 0),
      0
    );
    ...
    const refundRate =
      totalOrders > 0
        ? (active.filter((o) => (o.refunds?.length ?? 0) > 0).length /
            totalOrders) *
          100
        : 0;
    const discountRate =
    ...
    active.forEach((o) => {
      ...
      else if (o.customer?.firstName)
        customerKeys.add(`${o.customer.firstName}${o.customer.lastName ?? ''}`);
    });
    ...
    const statusData = [
      {
        name: 'Paid',
        value: clean.length,
        revenue: clean.reduce((s, o) => s + o.total, 0),
      },
      {
        name: 'Refunded',
        value: withRefunds.length,
        revenue: withRefunds.reduce((s, o) => s + o.total, 0),
      },
    ...
    active.forEach((o) => {
      if (o.paymentDetails?.splitPayments?.length) {
        o.paymentDetails.splitPayments.forEach((sp) => {
          if (!methodMap[sp.method])
            methodMap[sp.method] = { revenue: 0, count: 0 };
          methodMap[sp.method].revenue += sp.amount;
          methodMap[sp.method].count += 1;
        });
      } else {
        const m = o.paymentMethod || 'other';
        if (!methodMap[m]) methodMap[m] = { revenue: 0, count: 0 };
        methodMap[m].revenue += o.total ?? 0;
        methodMap[m].count += 1;
      }
    });
    ...
    active.forEach((o) => {
      (o.items || []).forEach((item) => {
        ...
        if (!productMap[key])
          productMap[key] = { name: key, revenue: 0, qty: 0 };
        productMap[key].revenue +=
          item.itemSubtotal ?? item.priceAtPurchase * item.quantity;
    ...
    const dayOfWeekData = DOW_LABELS.map((day, i) => ({
      day,
      ...dowBuckets[i],
    }));
  ...
  const toggleFilter = useCallback((key: string) => {
    ...
    setActiveFilters((prev) => {
      ...
      if (key === 'date_custom')
        return [...prev.filter((f) => !isDateKey(f)), key];
      // regular date filters: multi-select OR, but remove date_custom
      if (isDateKey(key))
        return [...prev.filter((f) => f !== 'date_custom'), key];
      // invoiced / not_invoiced are mutually exclusive
      if (key === 'invoiced')
        return [...prev.filter((f) => f !== 'not_invoiced'), key];
      if (key === 'not_invoiced')
        return [...prev.filter((f) => f !== 'invoiced'), key];
  ...
  const effectiveGroupBy = groupBy ?? 'order_day';
  const measureLabel =
    MEASURES.find((m) => m.key === measure)?.label ?? 'Total Price';
  const groupByLabel =
    ALL_GROUP_ITEMS.find((g) => g.key === effectiveGroupBy)?.label ?? 'Day';
  const groupBy2Label = groupBy2
  ...
  const regularFilterTags = activeFilters
    ...
    .map((k) => {
      if (k === 'date_custom' && customDateStart && customDateEnd) {
        ...
        return {
          key: k,
          label: `${fmt(customDateStart)} – ${fmt(customDateEnd)}`,
        };
      }
  ...
  const searchChipGroups = SEARCH_DEFS.map(({ prefix, label }) => {
    ...
    return {
      ...
      label,
      keys,
      values: keys.map((k) => k.slice(prefix.length)),
    };
  ...
  const hasActiveState =
    activeFilters.length > 0 || groupByStack.length > 0 || !!appliedSearchName;
    ...
    (s: SavedSearch) => {
      ...
      return (
        JSON.stringify(sortArr(s.filters)) ===
          JSON.stringify(sortArr(activeFilters)) &&
    ...
    },
    {
      ...
      icon: (
        <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2a10 10 0 0 0-10 10h10V2Z" opacity={0.5} />
          <path d="M12 2v10h10A10 10 0 0 0 12 2Z" />
          <path d="M2 12a10 10 0 1 0 10-10v10H2Z" opacity={0.3} />
        </svg>
      ),
      title: 'Pie chart',
    },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── Controls ────────────────────────────────────────────────────── */}
      <div className="space-y-4 px-6 pb-3 pt-5">
        {/* Page title */}
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">Order Analysis</h1>
          {loading && (
            <PiArrowsClockwise className="h-4 w-4 animate-spin text-gray-400" />
          )}
        </div>
        ...
        <div ref={searchRef} className="relative">
          <div
            className={`flex min-h-[42px] flex-wrap items-center gap-1.5 rounded-xl border bg-white px-3 py-2 shadow-sm transition-shadow ${
              searchOpen
          ...
          >
            ...
            {regularFilterTags.map((f) => (
              ...
              >
                ...
                <button
                  ...
                  className="ml-0.5 rounded-full opacity-70 transition-opacity hover:text-red-500 hover:opacity-100"
                >
                  <PiX className="h-3 w-3" />
                </button>
              </span>
              ...
              >
                ...
                <span className="opacity-80">
                  {group.values.map((v, i) => (
                    <span key={v}>
                      {i > 0 && (
                        <span className="mx-1 italic opacity-60">or</span>
                      )}
                ...
                <button
                  onClick={() => {
                    setAppliedSearchName(null);
                    setActiveFilters((prev) =>
                      prev.filter((f) => !f.startsWith(group.prefix))
                    );
                  }}
            ...
            {groupByStack.length > 0 && (
              <span className="flex items-center gap-1.5 rounded-full bg-teal-700 px-2.5 py-0.5 text-xs font-medium text-white shadow-sm">
                ...
                {groupByStack
                  .map(
                    (k) => ALL_GROUP_ITEMS.find((g) => g.key === k)?.label ?? k
                  )
                  .join(' > ')}
                <button
            ...
            <input
              ...
              placeholder={
                !hasActiveState && !panelSearch
                  ? 'Search orders, products, cashiers…'
                  : ''
              }
            ...
            <div className="absolute left-0 top-full z-50 mt-1.5 w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
              {SEARCH_DEFS.map(({ prefix, label }) => {
                ...
                return (
                  ...
                  >
                    <PiCaretDown className="h-3.5 w-3.5 shrink-0 -rotate-90 text-gray-400" />
                    <span>
                      Search{' '}
                      ...
                      for:{' '}
                      <em className="font-semibold not-italic text-[#4f2d7f]">
                        {panelSearch.trim()}
                      </em>
                    </span>
                    {existing.length > 0 && (
              ...
              <div className="border-t border-gray-100">
                <button
                  ...
                  className="flex w-full items-center gap-3 px-5 py-3 text-sm font-medium text-teal-600 transition-colors hover:bg-gray-50"
                >
                  Add Custom Filter
                </button>
              </div>
            ...
            <div className="absolute left-0 top-full z-50 mt-1.5 grid w-full min-w-[640px] grid-cols-3 divide-x divide-gray-100 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
              ...
              <div className="flex flex-col">
                ...
                <div className="max-h-[420px] space-y-0.5 overflow-y-auto p-3">
                  {/* Status filters */}
                  {FILTER_STATIC.filter(
                    (f) =>
                      !panelSearch ||
                      f.label.toLowerCase().includes(panelSearch.toLowerCase())
                  ).map((f) => {
                  ...
                  {/* Order Date */}
                  {(!panelSearch ||
                    'order date today yesterday week month quarter year'.includes(
                    ...
                    <>
                      ...
                      >
                        ...
                        <PiCaretDown
                          className={`h-3 w-3 text-gray-400 transition-transform ${dateSubOpen ? 'rotate-180' : ''}`}
                        />
                      </button>
                    </>
                  )}
                    ...
                    <div className="ml-3 mt-0.5 space-y-0.5 border-l-2 border-gray-100 pl-3">
                      ...
                      {[
                        ...
                        .map((d, idx, arr) => {
                          ...
                          const prevIsMonth =
                            idx > 0 && arr[idx - 1].key.startsWith('date_m_');
                          const prevIsQuarter =
                            idx > 0 && arr[idx - 1].key.startsWith('date_q_');
                          const showDiv =
                            !panelSearch &&
                            ((isQuarter && prevIsMonth) ||
                              (!isMonth && !isQuarter && prevIsQuarter));
                          const active = activeFilters.includes(d.key);
                          return (
                            <div key={d.key}>
                              {showDiv && (
                  ...
                  {!panelSearch && (
                    <>
                      ...
                      >
                        ...
                        <PiCaretDown
                          className={`h-3 w-3 text-gray-400 transition-transform ${customFilterOpen ? 'rotate-180' : ''}`}
                        />
                      </button>
                      {customFilterOpen && (
              ...
              <div className="flex flex-col">
                ...
                <div className="max-h-[420px] space-y-0.5 overflow-y-auto p-3">
                  {ALL_GROUP_ITEMS.filter(
                    (g) =>
                      !panelSearch ||
                      g.label.toLowerCase().includes(panelSearch.toLowerCase())
                  ).map((g, idx, arr) => {
                    const stackIdx = groupByStack.indexOf(g.key);
                    const active = stackIdx !== -1;
                    const isDate = GROUP_BY_DATE_ITEMS.some(
                      (d) => d.key === g.key
                    );
                    const prevIsDate =
                      idx > 0 &&
                      GROUP_BY_DATE_ITEMS.some(
                        (d) => d.key === arr[idx - 1].key
                      );
                    ...
                    return (
                      <div key={g.key}>
                        ...
                        >
                          ...
                          {active && stackIdx > 0 && (
                            <span
                              className={`ml-auto ${badgeCls[stackIdx] ?? badgeCls[2]}`}
                            >
              ...
              <div className="flex flex-col">
                ...
                <div className="max-h-[420px] flex-1 space-y-1 overflow-y-auto p-3">
                  ...
                  {savedSearches.map((s) => {
                    ...
                    const grpLabel = s.groupBy
                      ? ALL_GROUP_ITEMS.find((g) => g.key === s.groupBy)?.label
                      : null;
                    const grp2Label = s.groupBy2
                      ? ALL_GROUP_ITEMS.find((g) => g.key === s.groupBy2)?.label
                      : null;
                    return (
                      <div
                        key={s.id}
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          >
                            ...
                            <span
                              className={`truncate text-sm font-medium ${active ? 'text-teal-700' : 'text-gray-800'}`}
                            >
                              {s.name}
                            </span>
                          </button>
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-1">
                          {filterCount > 0 && (
                            <span className="flex items-center gap-1 rounded bg-violet-50 px-1.5 py-0.5 text-[10px] font-medium text-violet-600">
                              <PiFunnel className="h-2.5 w-2.5" />
                          ...
                          <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500">
                            {MEASURES.find((m) => m.key === s.measure)?.label ??
                              s.measure}
                          </span>
                        </div>
                      </div>
                  ...
                  {savingSearch ? (
                    <div className="mt-1 space-y-2 rounded-xl border border-gray-200 p-3">
                      ...
                      <input
                        ...
                        placeholder="e.g. This month · Cashier"
                        autoFocus
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div ref={measRef} className="relative">
            ...
            >
              ...
              <PiCaretDown
                className={`h-3.5 w-3.5 text-gray-400 transition-transform ${measuresOpen ? 'rotate-180' : ''}`}
              />
            </button>
            {measuresOpen && (
                  ...
                  <div key={m.key}>
                    ...
                    <button
                      ...
                      className={`flex w-full items-center gap-2.5 px-4 py-2 text-left text-sm transition-colors ${
                        measure === m.key
                          ? 'bg-[#fef2f2] font-semibold text-[#b20202]'
                          : 'text-gray-700 hover:bg-gray-50'
                      }`}
                    >
                      <span
              ...
              <>
                {sortStack.map((s, rank) => {
                  ...
                  return (
                    ...
                    >
                      {sortStack.length > 1 && (
                        <span className="mr-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-sky-200 text-[9px] font-bold text-sky-700">
                          {rank + 1}
                        </span>
                      )}
                      <button
                      ...
                      [
                        {
                          ...
                          label: measureLabel,
                          icon: (
                            <PiArrowDown className="h-3.5 w-3.5 text-gray-400" />
                          ),
                        },
                        {
                          field: 'label' as SortField,
                          label: 'Name (A–Z)',
                          icon: (
                            <PiArrowUp className="h-3.5 w-3.5 text-gray-400" />
                          ),
                        },
                        {
                          field: 'orders' as SortField,
                          label: 'Order Count',
                          icon: (
                    ...
                    {sortStack.length > 0 && (
                      <div className="mt-1 border-t border-gray-100">
                        <button
                          type="button"
                          onClick={() => {
                            setSortStack([{ field: 'value', dir: 'desc' }]);
                            setSortPickerOpen(false);
                          }}
            ...
            {activeFilters.includes('date_custom') &&
              ...
              customDateEnd && (
                <span className="flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-600">
                  <PiCalendarBlank className="h-3 w-3" />
                  {new Date(customDateStart).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                  })}
                  {' – '}
                  {new Date(customDateEnd).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
                  })}
            ...
            <span>
              ...
              {orders.length !== filtered.length && (
                <span className="ml-1 text-gray-400">
                  of {orders.length.toLocaleString()}
                </span>
              )}
            </span>
            {logicalFilterCount > 0 && (
      ...
      {viewMode === 'pivot' &&
        (() => {
          ...
          const visibleRows = p
            ? searchQ
              ? p.rowVals0.filter((rk) =>
                  fmtDimKey(rk, pivotRowDims[0]).toLowerCase().includes(searchQ)
                )
              : p.rowVals0
            : [];
            ...
            p.colVals0.forEach((ck) => {
              if (canExpandCol && expandedCols.has(ck)) {
                (p.subColValsMap[ck] ?? []).forEach((sk) => {
                  visibleCols.push({
                    colPath: [ck, sk],
                    label: fmtDimKey(sk, pivotColDims[1]),
                    isSubCol: true,
                  });
                });
              } else {
                visibleCols.push({
                  colPath: [ck],
                  label: fmtDimKey(ck, pivotColDims[0]),
                  isSubCol: false,
                });
              }
            });
          ...
          const heatStyle = (val: number) => {
            ...
            return {
              backgroundColor: `rgba(178,2,2,${Math.max(0.04, share * 0.26)})`,
            };
          };
          ...
          const buildCellTitle = (rPath: string[], cPath: string[]): string => {
            const rLabel =
              rPath.length === 0
                ? 'All'
                : rPath
                    .map((k, i) => fmtDimKey(k, pivotRowDims[i]))
                    .join(' › ');
            const cLabel =
              cPath.length === 0
                ? 'Total'
                : cPath
                    .map((k, i) => fmtDimKey(k, pivotColDims[i]))
                    .join(' › ');
              ...
              val > 0
                ? () => {
                    ...
                    if (cellOrders.length > 0)
                      setDrillData({
                        orders: cellOrders,
                        title: buildCellTitle(rowPath, colPath),
                      });
                  }
                : undefined;
            if (val === 0) {
              ...
              <div className="flex flex-wrap items-center gap-2.5 border-b border-gray-100 px-6 py-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {pivotRowDims.length < 3 && (
                    <PivotDimDropdown
                      open={pivotAddRowOpen}
                      refEl={pivotAddRowRef}
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {pivotColDims.length < 2 && (
                    <PivotDimDropdown
                      open={pivotAddColOpen}
                      refEl={pivotAddColRef}
                  ...
                  >
                    <thead>
                      ...
                      <tr>
                        <th className="sticky left-0 top-0 z-30 min-w-[260px] border-b border-r border-gray-100 bg-gray-50 px-4 py-3 text-left align-bottom">
                          <div className="text-[10px] font-bold uppercase tracking-wider text-[#b20202]">
                            {pivotRowDims
                              .map(
                                (d) =>
                                  ALL_GROUP_ITEMS.find((g) => g.key === d)
                                    ?.label ?? d
                              )
                              .join(' › ')}
                          </div>
                        ...
                        {p.colVals0.map((ck) => {
                          const isExpanded =
                            canExpandCol && expandedCols.has(ck);
                    ...
                    <tbody>
                      ...
                      {visibleRows.map((rk, ri) => {
                        ...
                        const rowShare =
                          p.grandTotal > 0
                            ? (rowTotal / p.grandTotal) * 100
                            : 0;
                        const isRowExpanded =
                          canExpandRow && expandedRows.has(rk);
      ...
      {viewMode === 'graph' && (
        <>
          ...
          <div
            className="flex w-full flex-col border-y border-gray-100 bg-white shadow-sm"
            style={{ height: '70vh' }}
          >
                ...
                <h3 className="text-sm font-semibold text-gray-800">
                  ...
                  {groupByStack.length > 0
                    ? groupByStack.map((g, i) => {
                        const lbl =
                          ALL_GROUP_ITEMS.find((item) => item.key === g)
                            ?.label ?? g;
                        return (
                          <Fragment key={g}>
                            {i > 0 && (
            ...
            <div className="min-h-0 flex-1 p-5">
              {multiSeriesData ? (
                <StackedChart
                  ...
                  chartType={chartType}
                  className="h-full"
                  orderMap={multiSeriesData.orderMap}
                  groupBy={groupBy}
                ...
                <MainChart
                  ...
                  className="h-full"
                  onBarClick={(label, orders) =>
                    setDrillData({ orders, title: label })
                  }
                />
              )}
            </div>
          ...
          <div className="space-y-5 px-6 py-5">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
              ...
              <KpiCard
                ...
                badge={
                  analytics.discountRate > 0
                    ? `${analytics.discountRate.toFixed(1)}% of gross`
                    : undefined
                }
                sub={
              ...
              <KpiCard
                ...
                badge={
                  analytics.refundRate > 0
                    ? `${analytics.refundRate.toFixed(1)}% rate`
                    : undefined
                }
                sub={
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  ...
                  <span className="text-xs tabular-nums text-gray-400">
                    {analytics.methodData.reduce((s, m) => s + m.count, 0)}{' '}
                    transactions
                  </span>
                </div>
                {analytics.methodData.length === 0 ? (
                  ...
                  (() => {
                    const totalRev = analytics.methodData.reduce(
                      (s, m) => s + m.revenue,
                      0
                    );
                    return (
                      <div className="mt-4 space-y-3">
                        {analytics.methodData.map((m, i) => {
                          const pct =
                            totalRev > 0 ? (m.revenue / totalRev) * 100 : 0;
                          return (
                            <div key={m.method}>
                              <div className="mb-1.5 flex items-center gap-2 text-xs">
                                ...
                                <span className="w-[5.5rem] text-right font-semibold tabular-nums text-gray-800">
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                ...
                {analytics.statusData.length === 0 ? (
                  ...
                  (() => {
                    const statusTotal = analytics.statusData.reduce(
                      (acc, x) => acc + x.value,
                      0
                    );
                    ...
                    return (
                      <>
                          ...
                          {analytics.statusData.map((s, i) => {
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  borderColor: `${STATUS_COLORS[i]}35`,
                                  backgroundColor: `${STATUS_COLORS[i]}08`,
                                }}
                              >
                                <p
                        ...
                        <p className="mt-2 text-[10px] text-gray-400">
                          {statusTotal.toLocaleString()} total orders
                        </p>
                      </>
                    );
                  })()
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                {analytics.topProducts.length > 0 && (
                  <span className="text-xs tabular-nums text-gray-400">
                    top {analytics.topProducts.length} of{' '}
                    {Object.keys(analytics.topProducts).length > 0
                      ? 'many'
                      : '0'}{' '}
                    products
                  </span>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-gray-400">
                            ...
                            {meta?.brandName && (
                              <span className="flex items-center gap-0.5">
                                <PiStorefront className="h-3 w-3" />
                                {meta.brandName}
                              </span>
                            )}
                            <span className="ml-auto shrink-0">
                              {p.qty.toLocaleString()} sold · avg{' '}
                              {formatCurrency(avgUnit)}
                            </span>
                          </div>
                          <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
                            <div
                              className="h-full rounded-full bg-[#b20202] transition-all duration-500"
                              style={{
                                width: `${pct}%`,
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                {(() => {
                  const peak = analytics.dayOfWeekData.reduce(
                    (b, d) => (d.revenue > b.revenue ? d : b),
                    analytics.dayOfWeekData[0]
                  );
              ...
              <div className="mt-4 grid grid-cols-7 gap-2">
                {(() => {
                  const maxRev = Math.max(
                    ...analytics.dayOfWeekData.map((d) => d.revenue),
                    1
                  );
                  const maxOrd = Math.max(
                    ...analytics.dayOfWeekData.map((d) => d.orders),
                    1
                  );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative flex h-24 w-full items-end overflow-hidden rounded-lg bg-gray-50">
                          ...
                          />
                          {isPeak && (
                            <span className="absolute left-0 right-0 top-1 text-center text-[8px] font-bold text-[#b20202]">
                              ★
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex h-2.5 w-full items-center justify-center">
                          <div
                            className="rounded-full bg-indigo-400 transition-all duration-500"
                            style={{
                              width: `${Math.max(ordPct * 0.7, 10)}%`,
                              height: 6,
                            }}
                          />
                        </div>
                        <span
                          className={`text-[11px] font-semibold ${isPeak ? 'text-[#b20202]' : isWeekend ? 'text-gray-600' : 'text-gray-400'}`}
                        >
                          {d.day}
                        </span>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  ...
                  {(() => {
                    const peak = analytics.hourData.reduce(
                      (b, h) => (h.orders > b.orders ? h : b),
                      analytics.hourData[0]
                    );
                    return peak?.orders > 0 ? (
                      <span className="text-xs text-gray-400">
                        Peak at{' '}
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Bar dataKey="orders" name="orders" radius={[3, 3, 0, 0]}>
                        {(() => {
                          const maxOrd = Math.max(
                            ...analytics.hourData.map((h) => h.orders),
                            1
                          );
                          return analytics.hourData.map((d, i) => (
                            <Cell
                              key={i}
                              fill={`rgba(178,2,2,${0.12 + (d.orders / maxOrd) * 0.88})`}
                            />
                          ));
                        })()}
                      </Bar>
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  ...
                  {analytics.cashierData.length > 0 && (
                    <span className="text-xs text-gray-400">
                      {analytics.cashierData.length} cashier
                      {analytics.cashierData.length !== 1 ? 's' : ''}
                    </span>
                  )}
                </div>
                {analytics.cashierData.length === 0 ? (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 flex items-center justify-between gap-2 text-[11px] text-gray-400">
                              ...
                              <span>
                                avg {formatCurrency(c.avgOrder)}/order
                              </span>
                            </div>
                            <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
                              <div
