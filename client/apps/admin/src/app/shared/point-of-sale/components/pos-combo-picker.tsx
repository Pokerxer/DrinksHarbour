...
function spThumb(sp: POSComboSubProduct) {
  return resolveSubProductThumb(sp) || '';
}
...
function computeComboPrice(
  ...
  switch (combo.priceMode ?? 'dynamic') {
    ...
    case 'markup_on_cost':
      return (
        Math.round(cost * (1 + (combo.markupPercentage ?? 0) / 100) * 100) / 100
      );
    case 'discount_off_selling':
      return (
        Math.round(
          selling * (1 - (combo.discountPercentage ?? 0) / 100) * 100
        ) / 100
      );
    default:
      return selling;
...
function ProductRow({
  ...
  const selectedSize =
    typeof sel?.val === 'string'
      ? sizes.find((s) => String(s._id) === sel.val)
      : null;
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex h-5 w-5 shrink-0 items-center justify-center border-2 transition-colors ${picked ? 'border-[#b20202] bg-[#b20202]' : 'border-gray-300 bg-white'} ${isRadio ? 'rounded-full' : 'rounded-md'}`}
        >
        ...
        <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl bg-gray-100">
          {img ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              <PiPackage className="h-6 w-6 text-gray-300" />
            </div>
          )}
        </div>
        ...
        <div className="min-w-0 flex-1">
          <p
            className={`truncate text-sm font-semibold leading-tight ${picked ? 'text-[#b20202]' : 'text-gray-800'}`}
          >
        ...
        {picked && noSizes && (
          <PiCheckCircle className="h-5 w-5 shrink-0 text-[#b20202]" />
        )}
      ...
      {sizes.length > 0 && (
        <div
          className={`flex flex-wrap gap-1.5 border-t px-3 py-2.5 ${picked ? 'border-[#b20202]/20 bg-red-50/30' : 'border-gray-100 bg-gray-50/50'}`}
        >
          {sizes.map((size) => {
              ...
              <button
                ...
                className={`flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-all ${
                  active
                    ...
                    : oos
                      ? 'cursor-not-allowed border border-gray-200 bg-white text-gray-300 line-through'
        ...
        <div className="flex items-center gap-3 border-t border-[#b20202]/20 bg-red-50/20 px-3 py-2">
          ...
          <div className="flex items-center overflow-hidden rounded-lg border border-[#b20202]/30 bg-white">
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onQty(Math.max(minQty, qty - 1));
              }}
              className="flex h-7 w-7 items-center justify-center text-[#b20202] transition-colors hover:bg-[#b20202]/10 disabled:opacity-30"
            >
...
export default function POSComboPicker({
  ...
  function setPick(li: number, ii: number, val: string | true | null) {
    ...
    if (isNowComplete && val !== null) {
      ...
      if (next !== -1) {
        setActiveGroup(next);
        setTimeout(() => {
          groupRefs.current[next]?.scrollIntoView({
            behavior: 'smooth',
            block: 'start',
          });
        }, 120);
    ...
    combo.choiceLines.forEach((line, li) =>
      line.items.forEach((item, ii) => {
        ...
        if (typeof p.val === 'string') {
          ...
          if (sz) {
            unitSell = sz.sellingPrice;
            unitCost = sz.costPrice ?? unitCost;
          }
        }
  ...
  const addCount = combo.choiceLines.reduce(
    (s, _, li) =>
      s +
      Object.values(picks[li] || {}).reduce((ls, e) => ls + (e.qty || 1), 0),
    0
  );
  ...
  const lineStatus = combo.choiceLines.map((line, li) => {
    ...
    if (line.required && n < line.minSelect)
      return {
        ok: false,
        msg:
          n === 0 ? 'Select at least one' : `Need ${line.minSelect - n} more`,
      };
    ...
    for (let ii = 0; ii < line.items.length; ii++) {
      ...
      if (p.qty < iMin)
        return {
          ok: false,
          msg: `${line.items[ii]?.subProduct?.product?.name ?? 'Item'}: needs at least ${iMin}`,
        };
    }
    return { ok: true, msg: '' };
  });
  ...
  function buildItems(): POSCartItem[] {
    ...
    if (
      ...
      if (target > 0 && itemsSum > 0 && target < itemsSum) {
        ...
        raw.forEach((i) => {
          i.discount = Math.min(100, Math.max(0, discPct));
        });
      }
  ...
  return (
    ...
    >
      <div className="flex max-h-[96dvh] w-full flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:max-h-[88vh] sm:max-w-xl sm:rounded-3xl">
        {/* ── Header ── */}
          ...
          <div className="scrollbar-none flex gap-2 overflow-x-auto px-5 pb-4">
            {combo.choiceLines.map((line, li) => (
              <button
                ...
                onClick={() => {
                  setActiveGroup(li);
                  groupRefs.current[li]?.scrollIntoView({
                    behavior: 'smooth',
                    block: 'start',
                  });
                }}
                ...
                >
                  ...
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${n >= line.minSelect && n > 0 ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-600'}`}
                      >
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div>
              ...
              {mode === 'markup_on_cost' && cost > 0 && (
                <p className="text-[11px] text-gray-400">
                  {formatCurrency(cost)} cost + {combo.markupPercentage ?? 0}%
                </p>
              )}
            ...
            <button
              ...
              onClick={() => {
                if (canAdd && addCount > 0) onAdd(buildItems());
              }}
