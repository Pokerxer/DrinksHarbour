...
function SearchPanel({
  ...
  function FilterItem({ fkey, label }: { fkey: FilterKey; label: string }) {
    ...
    return (
      <button
        ...
        className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition-colors ${active ? 'bg-[#b20202]/8 font-semibold text-[#b20202]' : 'text-gray-700 hover:bg-gray-50'}`}
      >
        <span
          className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors ${active ? 'border-[#b20202] bg-[#b20202]' : 'border-gray-300'}`}
        >
  ...
  function GroupItem({ gkey, label }: { gkey: GroupKey; label: string }) {
    ...
    return (
      <button
        type="button"
        onClick={() => onSetGroupBy(active ? null : gkey)}
        className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition-colors ${active ? 'bg-[#b20202]/8 font-semibold text-[#b20202]' : 'text-gray-700 hover:bg-gray-50'}`}
      >
        <span
          className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${active ? 'border-[#b20202]' : 'border-gray-300'}`}
        >
  ...
  return (
    <div
      ref={ref}
      className="ring-black/8 absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-2xl bg-white shadow-xl ring-1"
      style={{ minWidth: 640 }}
    >
        ...
        <div className="flex-1 p-4">
          ...
          <div className="space-y-0.5">
            ...
            <div>
              <button
                type="button"
                onClick={() => setOpeningDateOpen((v) => !v)}
        ...
        <div className="flex-1 p-4">
          ...
          <div className="space-y-0.5">
            ...
            <div>
              <button
                type="button"
                onClick={() => setGroupOpeningOpen((v) => !v)}
                className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50"
              >
                <span>Opening Date</span>
                {groupOpeningOpen ? (
            ...
            <div>
              <button
                type="button"
                onClick={() => setGroupClosingOpen((v) => !v)}
                className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50"
              >
                ...
                {savedSearches.map((s) => (
                  <div key={s.id} className="group flex items-center gap-1">
                    <button
                      ...
                      className="flex flex-1 items-center gap-2 truncate rounded-lg px-3 py-1.5 text-left text-sm text-gray-700 hover:bg-gray-50"
                    >
    ...
    <div className="flex h-full flex-col border-l border-gray-200 bg-white">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400">
            {fmtDateTime(order.placedAt || order.createdAt)}
          </p>
        </div>
                ...
                {(order.items || []).map((item, i) => (
                  <tr key={i}>
                    <td className="px-4 py-2.5 font-medium text-gray-800">
                      {item.name}
                      {item.variant && (
                        <span className="font-normal text-gray-400"> · {item.variant}</span>
                      )}
                    </td>
      ...
      <div className="shrink-0 border-t border-gray-200 px-4 py-3">
        <div className="flex items-baseline justify-between">
          ...
          <span
            className={`text-lg font-extrabold tabular-nums ${order.isVoided ? 'text-gray-400 line-through' : 'text-gray-900'}`}
          >
            {formatCurrency(amount)}
          </span>
        </div>
        {refunded > 0 && (
          <div className="mt-0.5 flex justify-between text-xs text-red-500">
            <span>Returned</span>
...
function OrdersTab({
  ...
  const selectedRefunded = selectedOrders.reduce(
    (s, o) => s + (o.refunds || []).reduce((r, ref) => r + ref.totalRefunded, 0),
    0
  );
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Table ── */}
      <div
        className={`flex flex-col overflow-hidden transition-all ${drawerOrder ? 'w-[55%]' : 'w-full'}`}
      >
        {/* Summary strip */}
        <div className="grid shrink-0 grid-cols-3 divide-x divide-gray-100 border-b border-gray-100 bg-gray-50/60">
          ...
          <div className="px-3 py-2 text-center">
            ...
            <p className="text-sm font-bold tabular-nums text-[#b20202]">
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-100 transition-colors ${
                      isOpen
                        ? 'bg-[#b20202]/8 border-l-2 border-l-[#b20202]'
                        : isChecked
                          ? 'border-l-2 border-l-[#b20202] bg-[#b20202]/5'
                          : 'border-l-2 border-l-transparent bg-white hover:bg-gray-50'
                    }`}
                  >
                    ...
                    <td
                      className="cursor-pointer px-2 py-2.5 font-mono text-[10px] text-gray-500"
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {fmtTime(order.placedAt || order.createdAt)}
                    </td>
                    <td
                      className="cursor-pointer px-2 py-2.5 font-semibold text-gray-800"
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {order.receiptNumber || order.orderNumber || '—'}
                    </td>
                    <td
                      className="max-w-[90px] cursor-pointer truncate px-2 py-2.5 text-gray-600"
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {customer}
                    </td>
                    <td
                      className="cursor-pointer px-2 py-2.5 capitalize text-gray-500"
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {(order.paymentMethod || '').replace(/_/g, ' ')}
                    </td>
                    <td
                      className={`cursor-pointer px-2 py-2.5 text-right font-bold tabular-nums ${isVoided ? 'text-gray-400 line-through' : 'text-gray-900'}`}
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {formatCurrency(amount)}
                    </td>
                    <td
                      className="cursor-pointer px-2 py-2.5 text-center"
                      onClick={() => setDrawerOrder(isOpen ? null : order)}
                    >
                      {isVoided ? (
                        ...
                        <span className="rounded bg-red-50 px-1.5 py-0.5 text-[9px] font-bold text-red-600">
                          RTN
                        </span>
                      ) : refunded > 0 ? (
                        <span className="rounded bg-amber-50 px-1.5 py-0.5 text-[9px] font-bold text-amber-600">
                          P.RTN
                        </span>
                      ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-semibold text-gray-700">
                  <span className="font-bold text-[#b20202]">{checkedIds.size}</span> order
                  {checkedIds.size !== 1 ? 's' : ''} selected
                </p>
                <p className="text-lg font-extrabold tabular-nums leading-tight text-gray-900">
                  {formatCurrency(selectedTotal)}
                </p>
                {selectedRefunded > 0 && (
                  <p className="text-[10px] tabular-nums text-red-500">
                    −{formatCurrency(selectedRefunded)} returned · Net{' '}
                    {formatCurrency(Math.max(0, selectedTotal - selectedRefunded))}
                  </p>
                )}
              </div>
...
function SessionDetail({
  ...
  tenant?: POSTenant | null;
  onClose: () => void;
  onTabChange?: (tab: 'overview' | 'orders') => void;
}) {
  const [tab, setTab] = useState<'overview' | 'orders'>('overview');

  function switchTab(t: 'overview' | 'orders') {
    setTab(t);
    onTabChange?.(t);
  }
  ...
  const moves =
    ((session as any).cashMovements as Array<{
      type: string;
      amount: number;
      reason?: string;
      performedAt: string;
      performedBy?: { firstName: string; lastName: string; posName?: string };
    }>) || [];
  ...
  const cashierLogs =
    ((session as any).cashierLog as Array<{ cashier: any; startedAt: string; endedAt?: string }>) ||
    [];
    ...
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-5 py-3">
        <div>
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${session.status === 'open' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}
            >
              {session.status === 'open' ? (
                <>
          ...
          <p className="mt-0.5 select-all font-mono text-[10px] text-gray-300">{session._id}</p>
        </div>
      ...
      <div className="flex shrink-0 border-b border-gray-100 text-xs font-semibold">
        {(
          [
            ...
            {
              id: 'orders',
              label: `Orders${session.orderCount > 0 ? ` (${session.orderCount})` : ''}`,
              icon: <PiList className="h-3.5 w-3.5" />,
            },
          ] as const
        ).map((t) => (
          <button
          ...
          <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
            {[
              ...
              <div key={label} className="px-4 py-3 text-center">
                <span
                  className={`mb-1 inline-flex ${highlight ? 'text-[#b20202]' : 'text-gray-400'}`}
                >
                  {icon}
                </span>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                ...
                <p
                  className={`mt-0.5 text-sm font-bold tabular-nums ${highlight ? 'text-[#b20202]' : 'text-gray-900'}`}
                >
                  {value}
                </p>
              </div>
          ...
          <div className="space-y-1.5 border-b border-gray-100 px-5 py-3 text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="text-gray-800">
                ...
                <span className="font-medium">{cashierLabel(session.openedBy as any)}</span>
              </span>
            </div>
            {session.closedAt && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-800">
                  ...
                  <span className="font-medium">{cashierLabel(session.closedBy as any)}</span>
                </span>
              </div>
            )}
            <div className="flex items-center justify-between">
              ...
              <div className="space-y-2">
                {methods.map((m) => {
                  ...
                  return (
                    <div
                      key={m.method}
                      className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${hasDiff ? 'bg-red-50' : 'bg-gray-50'}`}
                    >
                      ...
                      <div className="text-right">
                        ...
                        {hasDiff && (
                          <p
                            className={`text-[10px] font-semibold tabular-nums ${diff > 0 ? 'text-emerald-600' : 'text-red-600'}`}
                          >
                            {diff > 0 ? '+' : ''}
                            {formatCurrency(diff)}
                          </p>
              ...
              <div className="space-y-1.5">
                {moves.map((m, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    <div
                      className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${m.type === 'in' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}
                    >
                      {m.type === 'in' ? (
                    ...
                    <span
                      className={`text-xs font-bold tabular-nums ${m.type === 'in' ? 'text-emerald-600' : 'text-red-500'}`}
                    >
                      {m.type === 'in' ? '+' : '−'}
                      {formatCurrency(m.amount)}
                    </span>
                    <span className="shrink-0 text-[10px] text-gray-400">
                      {fmtTime(m.performedAt)}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="border-b border-gray-100 px-5 py-3">
              ...
              <div className="space-y-1.5">
                {cashierLogs.map((log, i) => {
                  const name =
                    typeof log.cashier === 'object' ? cashierLabel(log.cashier) : 'Cashier';
                  return (
                    <div key={i} className="flex items-center gap-2.5 text-xs">
                      ...
                      <span className="text-gray-400">
                        {log.endedAt ? fmtTime(log.endedAt) : 'now'}
                      </span>
                    </div>
                  );
                })}
...
function SessionRow({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full border-b border-gray-100 text-left transition-colors ${compact ? 'px-3 py-3' : 'px-5 py-4'} ${selected ? 'bg-[#b20202] text-white' : 'bg-white hover:bg-gray-50'}`}
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-0.5 flex items-center gap-1.5">
            ...
            {!compact && (
              <span
                className={`rounded border px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${selected ? 'border-white/30 bg-white/15 text-white/80' : 'border-gray-200 bg-gray-50 text-gray-500'}`}
              >
                {session.shopName || (session.shopId ? 'Shop' : 'Legacy — unassigned')}
              </span>
            )}
            {session.hasDifference && (
              <PiWarningCircle
                className={`h-3 w-3 shrink-0 ${selected ? 'text-yellow-300' : 'text-amber-500'}`}
              />
            )}
          </div>
          <div
            className={`flex items-center gap-1.5 text-[10px] ${selected ? 'text-white/70' : 'text-gray-400'}`}
          >
            <span>
            ...
            {!compact && (
              <>
                <span>·</span>
                <span className="truncate">{cashierLabel(session.openedBy as any)}</span>
              </>
            )}
          </div>
          {/* Session ID — shown in compact mode for searchability */}
          <div
            className={`mt-0.5 truncate font-mono text-[9px] ${selected ? 'text-white/40' : 'text-gray-300'}`}
          >
            {session._id}
          </div>
        </div>
        ...
        {!compact && (
          <div className="shrink-0 text-right">
            <p
              className={`text-xs font-bold tabular-nums ${selected ? 'text-white' : 'text-gray-900'}`}
            >
...
export default function POSSessions({ initialShopId }: { initialShopId?: string } = {}) {
  ...
  useEffect(() => {
    ...
    setSessions([]);
    setPage(1);
  }, [token, defaultShop, initialShopId]);
  useEffect(
    () => () => {
      requestVersion.current++;
    },
  ...
  const fetchSessions = useCallback(() => {
    ...
    setLoading(true);
    posApi
      .getSessions(token, page, PAGE_SIZE, effectiveStatus, undefined, undefined, historyShop)
      .then((d) => {
  ...
  const groupedSessions = useMemo((): [string, POSSession[]][] | null => {
    ...
    filteredSessions.forEach((s) => {
      ...
      switch (groupBy) {
        case 'opened_by':
          key = cashierLabel(s.openedBy as any);
          break;
        case 'terminal':
          key =
            (s.terminalType || 'retail').charAt(0).toUpperCase() +
            (s.terminalType || 'retail').slice(1);
          break;
        case 'status':
          key = s.status === 'open' ? 'Open' : 'Closed';
          break;
        case 'opening_day':
          key = fmtDate(s.openedAt);
          break;
        case 'opening_week':
          key = weekLabel(new Date(s.openedAt));
          break;
        case 'opening_month':
          key = new Date(s.openedAt).toLocaleDateString('en-GB', {
            month: 'long',
            year: 'numeric',
          });
          break;
        case 'opening_quarter':
          key = quarterLabel(new Date(s.openedAt));
          break;
        case 'opening_year':
          key = yearLabel(new Date(s.openedAt));
          break;
        case 'closing_day':
          key = s.closedAt ? fmtDate(s.closedAt) : 'Not closed';
          break;
        case 'closing_week':
          key = s.closedAt ? weekLabel(new Date(s.closedAt)) : 'Not closed';
          break;
        case 'closing_month':
          key = s.closedAt
            ? new Date(s.closedAt).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })
            : 'Not closed';
          break;
        case 'closing_quarter':
          key = s.closedAt ? quarterLabel(new Date(s.closedAt)) : 'Not closed';
          break;
        case 'closing_year':
          key = s.closedAt ? yearLabel(new Date(s.closedAt)) : 'Not closed';
          break;
        default:
          key = '—';
      }
  ...
  const filterLabels: Record<FilterKey, string> = {
    ...
    opening_yesterday: 'Opening: Yesterday',
    opening_this_week: 'Opening: This Week',
    opening_this_month: 'Opening: This Month',
  };
  const groupByLabels: Record<GroupKey, string> = {
    ...
    opening_quarter: 'Opening: Quarter',
    opening_year: 'Opening: Year',
    closing_day: 'Closing: Day',
    closing_week: 'Closing: Week',
    closing_month: 'Closing: Month',
    closing_quarter: 'Closing: Quarter',
    closing_year: 'Closing: Year',
  };
  const activeChips: { key: string; label: string; type: 'filter' | 'group' }[] = Array.from(
    activeFilters
  ).map((f) => ({ key: f, label: filterLabels[f] || f, type: 'filter' as const }));
  if (groupBy)
    activeChips.push({
      key: groupBy,
  ...
  return (
    <div className="flex h-dvh flex-col bg-gray-50">
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-4 py-2.5">
        ...
        {/* Search + panel */}
        <div ref={searchBarRef} className="relative max-w-lg flex-1">
          <div
            className={`flex overflow-hidden rounded-xl border transition-all ${showPanel ? 'border-[#b20202] ring-1 ring-[#b20202]/10' : 'border-gray-200'} bg-white`}
          >
          ...
          {activeChips.map((chip) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() =>
                  chip.type === 'filter' ? removeFilter(chip.key as FilterKey) : setGroupBy(null)
                }
      ...
      <div className="flex shrink-0 border-b border-gray-200 bg-white">
        {[
          ...
          <div
            key={label}
            className={`flex-1 border-r border-gray-100 px-4 py-3 last:border-r-0 ${i === 0 ? '' : ''}`}
          >
            <div
              className={`mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider ${highlight ? 'text-[#b20202]' : open ? 'text-emerald-600' : 'text-gray-400'}`}
            >
              {icon}
              {label}
            </div>
            <p
              className={`text-lg font-bold tabular-nums ${highlight ? 'text-[#b20202]' : open && openCount > 0 ? 'text-emerald-600' : 'text-gray-900'}`}
            >
              {value}
            </p>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div
          className={`flex flex-col overflow-hidden border-r border-gray-200 transition-all duration-200 ${selected ? 'w-72 shrink-0' : 'flex-1'}`}
        >
          {loading ? (
            <div className="flex flex-1 items-center justify-center">
                      ...
                      return (
                        <div key={groupName}>
                          ...
                          >
                            ...
                            <span className="ml-auto text-[10px] font-semibold tabular-nums text-gray-400">
                          ...
                          {isExpanded &&
                            groupItems.map((session) => (
                              <SessionRow
                                ...
                                onClick={() => {
                                  setSelected((prev) =>
                                    prev?._id === session._id ? null : session
                                  );
        ...
        <div
          className={`flex flex-col transition-all duration-200 ${selected ? 'flex-1 overflow-hidden' : 'w-72 shrink-0'}`}
        >
          {selected ? (
            <>
              <LegacySessionClose
