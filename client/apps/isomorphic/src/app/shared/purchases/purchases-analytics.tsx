...
export default function PurchasesAnalytics() {
  ...
  // Categories & brands (no auth required)
  useEffect(() => {
    posApi
      .getCategories()
  ...
  // Product → category/subcategory/brand metadata map, keyed by product name
  useEffect(() => {
    if (!token) return;
    Promise.all([
      ...
      .then(([prodRes, catRes]) => {
        const catsById: Record<string, CatItem> = {};
        ...
        (prodRes.products || []).forEach((p) => {
          ...
          const rawCatId = p.product?.category?._id || '';
          const rawCatName = p.product?.category?.name || '';
          const brandId = p.product?.brand?._id || '';
          const brandName = p.product?.brand?.name || '';

          let catId = rawCatId;
          let catName = rawCatName;
          let subCatId: string | undefined;
          let subCatName: string | undefined;

          const catEntry = catsById[rawCatId];
          if (catEntry?.parent) {
            // The product's category is itself a subcategory — resolve its parent
            subCatId = rawCatId;
            subCatName = rawCatName;
            const parent = catsById[catEntry.parent];
            catId = parent?._id || catEntry.parent;
            catName = parent?.name || rawCatName;
          }
