...
export function defaultValues(product?: Partial<SubProductInput>): Partial<SubProductInput> {
  ...
  return {
    subProductData: {
      ...
      isPublished: product?.subProductData?.isPublished ?? true,
      ...
      lowStockThreshold: product?.subProductData?.lowStockThreshold ?? 10,
      reorderPoint:      product?.subProductData?.reorderPoint      ?? 5,
      reorderQuantity:   product?.subProductData?.reorderQuantity   ?? 50,
      currency:    product?.subProductData?.currency    ?? 'NGN',
