...
export function LocationsTab({
  ...
  const grandTotal = useMemo(
    () => rows.reduce((s, r) => s + (r.currentQuantity || 0), 0),
    [rows]
  );
  ...
  const adjust = async (row: WarehouseStockRow, type: AdjustType) => {
    ...
    try {
      await warehouseStockService.adjustStock(
        ...
        {
          subProduct: String(subProductId),
      ...
      {!loading && groups.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-gray-200 py-16 text-center">
          ...
          <p className="mt-1 text-xs text-gray-400">
            ...
            page to start tracking it here.
          </p>
          <Link
            href={routes.warehouses.list}
            className="mt-5 inline-flex items-center gap-1.5 rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-gray-700"
          >
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <Link
                  href={routes.warehouses.detail(g.warehouseId)}
              ...
              <div className="flex items-center gap-4 text-[10px] text-gray-400">
                <span>
                  On hand{' '}
                  <span className="font-bold text-gray-700">{g.onHand}</span>
                </span>
                <span>
                  Reserved{' '}
                  <span className="font-bold text-amber-600">{g.reserved}</span>
                </span>
                <span>
                  Available{' '}
                  <span className="font-bold text-green-600">
                    {Math.max(0, g.onHand - g.reserved)}
                  </span>
                </span>
              </div>
            </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            ...
                            title="Receive stock"
                            className="rounded border border-gray-200 px-2 py-1 text-[10px] font-semibold text-gray-600 hover:bg-gray-100"
                          >
                            + Receive
                          </button>
                          <button
