...
export default function SubProductInventory() {
  ...
  const subProductId =
    watch?.('subProductData._id') || watch?.('subProductData.id');

  const currencySymbol = CURRENCY_SYMBOLS[currency] || '\u20A6';
  const hasSizeVariants =
    sellWithoutSizeVariants === false && sizes?.length > 0;
  ...
  const [showServerAdjustmentModal, setShowServerAdjustmentModal] =
    useState(false);
  const [serverAdjustmentInitialType, setServerAdjustmentInitialType] =
    useState<string>('received');
  const [serverAdjustmentType, setServerAdjustmentType] = useState<
    'received' | 'adjustment_in' | 'adjustment_out'
  >('received');
  ...
  const [lastAdjustment, setLastAdjustment] = useState<StockAdjustment | null>(
    null
  );
  ...
  const finalAvailableStock = autoCalculateAvailable
    ? calculatedAvailable
    : availableStock;
  ...
  const totalAvailable = useMemo(
    () => inventoryQuants.reduce((sum, q) => sum + q.availableQuantity, 0),
    [inventoryQuants]
  );
  const totalReserved = useMemo(
    () => inventoryQuants.reduce((sum, q) => sum + q.reservedQuantity, 0),
    [inventoryQuants]
  );
  ...
  const filteredHistory = useMemo(
    () =>
      filterHistory(adjustmentHistory, {
        historyFilter,
        statusFilter,
        sizeFilter,
        searchQuery,
  ...
  const paginatedHistory = useMemo(
    () => paginateArray(filteredHistory, currentPage, itemsPerPage),
  ...
  const fetchInventoryData = useCallback(async () => {
    ...
    try {
      const [summaryRes, movementsRes] = await Promise.all([
        ...
        inventoryService.getMovements(session.user.token, {
          subProductId,
          limit: 50,
        }),
      ]);
  ...
  const addToHistory = useCallback(
    (
      ...
      const adjustment: StockAdjustment = {
        ...
        reference:
          options?.reference || `WH/MOV/${Date.now().toString().slice(-5)}`,
        productName: options?.productName || 'SubProduct',
        fromLocation:
          options?.fromLocation || (type === 'add' ? 'Vendors' : 'WH/Stock'),
        toLocation:
          options?.toLocation || (type === 'remove' ? 'Customers' : 'WH/Stock'),
          ...
          type === 'add'
            ...
            : type === 'remove'
              ...
              : type === 'transfer'
                ...
                : 'adjustment',
        sourceDocument:
          options?.sourceDocument || `Shop/${Date.now().toString().slice(-3)}`,
        demand: quantity,
        picked: quantity,
        sizeVariant,
  ...
  const recordInventoryMovement = useCallback(
    async (
      ...
      if (!subProductId || !session?.user?.token) {
        console.warn(
          '⚠️ Cannot record movement: missing subProductId or token'
        );
        return false;
      }
  ...
  const handleStockAdjust = useCallback(
    async (delta: number) => {
      console.log('🎯 handleStockAdjust CALLED:', {
        delta,
        subProductId,
      ...
      setValue?.(
        ...
        Math.max(0, newTotal - (reservedStock || 0))
      );
      ...
      toast.success(
        `Stock ${delta > 0 ? 'added' : 'removed'}: ${Math.abs(delta)} units`
      );
    },
    [
      totalStock,
  ...
  const handleReservedAdjust = useCallback(
    (delta: number) => {
      const newReserved = Math.max(0, (reservedStock || 0) + delta);
      setValue?.('subProductData.reservedStock', newReserved);
      addToHistory(
        delta > 0 ? 'reserve' : 'unreserve',
        Math.abs(delta),
        reservedStock,
        newReserved,
        delta > 0 ? 'Reserved for order' : 'Released reservation'
      );
    },
  ...
  const undoLastAdjustment = useCallback(() => {
    ...
    setAdjustmentHistory((prev) =>
      prev.map((h) =>
        h.id === lastAdjustment.id ? { ...h, canUndo: false } : h
      )
    );
  ...
  const submitAdjustment = async () => {
    ...
    console.log('🎯 submitAdjustment CALLED:', {
      adjustmentType,
      adjustmentQuantity,
      subProductId,
    });
    ...
    if (adjustmentType === 'add') {
      ...
      newTotal = hasSizeVariants
        ? Math.max(0, totalStock + (newVal - currentVal))
        : adjustmentQuantity;
    }
  ...
  const submitTransfer = async () => {
    if (transferQuantity <= 0 || !transferFromWarehouse || !transferToWarehouse)
      return;

    const fromLabel =
      WAREHOUSE_OPTIONS.find((w) => w.value === transferFromWarehouse)?.label ||
      transferFromWarehouse;
    const toLabel =
      WAREHOUSE_OPTIONS.find((w) => w.value === transferToWarehouse)?.label ||
      transferToWarehouse;
    ...
    addToHistory(
      ...
      totalStock,
      totalStock,
      `Transfer from ${fromLabel} to ${toLabel}`,
      transferNotes,
      {
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'transfer':
          ...
          if (!data.sizeId) {
            toast.error(
              'Select a size to transfer (stock is tracked per size)'
            );
            return;
          }
  ...
  const toggleRoute = (route: string) => {
    ...
    if (currentRoutes.includes(route)) {
      setValue?.(
        'subProductData.routes',
        currentRoutes.filter((r: string) => r !== route)
      );
    } else {
  ...
  return (
    <div className="space-y-0">
      {/* ── Inventory header ── */}
      <div className="overflow-hidden rounded-t-2xl border border-gray-200 bg-white">
        {/* Stock status strip */}
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span
              className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold capitalize ${sm.cls}`}
            >
              {sm.label}
            </span>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-gray-900 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-gray-700"
          >
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-4 py-3 text-xs font-semibold transition-colors ${
                  isActive
      ...
      <div className="rounded-b-2xl border border-t-0 border-gray-200 bg-white p-5">
        ...
        {activeTab === 'overview' && (
          <OverviewTab
            ...
            stockStatus={getCurrentStockStatus(
              stockStatus,
              finalAvailableStock,
              lowStockThreshold
            )}
        ...
        {activeTab === 'history' && (
          <HistoryTab
            ...
            onCancelMovement={async (id) => {
              await inventoryService.cancelMovement(
                id,
                session?.user?.token || '',
                'Manual cancel'
              );
        ...
        {activeTab === 'alerts' && (
          <AlertsTab
            ...
            onLowStockThresholdChange={(v) =>
              setValue?.('subProductData.lowStockThreshold', v)
            }
            onReorderPointChange={(v) =>
              setValue?.('subProductData.reorderPoint', v)
            }
        ...
        {activeTab === 'settings' && (
          <SettingsTab
            ...
            onStandardPriceChange={(v) =>
              setValue?.('subProductData.standardPrice', v)
            }
            onLowStockThresholdChange={(v) =>
              setValue?.('subProductData.lowStockThreshold', v)
            }
            ...
            onReorderQuantityChange={(v) =>
              setValue?.('subProductData.reorderQuantity', v)
            }
          />
        )}
