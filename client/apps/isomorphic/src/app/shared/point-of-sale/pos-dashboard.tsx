...
function SalesChart({
  ...
  const bestIdx = data.reduce(
    (bi, d, i) => (d.sales > data[bi].sales ? i : bi),
    0
  );

  const half = Math.floor(data.length / 2);
  const earlyAvg =
    data.slice(0, half).reduce((s, d) => s + d.sales, 0) / (half || 1);
  const lateAvg =
    data.slice(half).reduce((s, d) => s + d.sales, 0) /
    (data.length - half || 1);
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Daily average', value: formatCurrency(avgSales) },
          {
            label: 'Best day',
            value: data[bestIdx]
              ? new Date(data[bestIdx].date).toLocaleDateString('en-US', {
                  weekday: 'short',
                  month: 'short',
                  day: 'numeric',
                })
      ...
      <div className="flex items-center gap-1.5 text-[10px]">
        {trendPct >= 0 ? (
          ...
          <>
            ...
            <span className="font-semibold text-red-500">
              {Math.abs(trendPct).toFixed(0)}% trend down
            </span>
          </>
        )}
        ...
        {data[bestIdx] && (
          <span className="ml-auto flex items-center gap-1 text-gray-400">
            ...
            {formatCurrency(data[bestIdx].sales)}
          </span>
        )}
      </div>
    </div>
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div
        ...
        style={{ background: avatarColor ?? '#b20202' }}
      />
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        {info.sessionOpen && (
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-green-100 bg-green-50 px-3 py-2">
            ...
            <>
              <div className="rounded-lg bg-gray-50 px-3 py-2.5">
                ...
                <p className="mt-0.5 text-lg font-bold tabular-nums text-gray-900">
                  {info.sessionSales != null
                    ? formatCurrency(info.sessionSales)
                    : '—'}
                </p>
              </div>
              <div className="rounded-lg bg-gray-50 px-3 py-2.5">
...
function RecentOrderRow({ order }: { order: POSRecentOrder }) {
  const customer = order.customer
    ...
    : 'Walk-in';
  const isToday =
    new Date(order.placedAt || order.createdAt).toDateString() ===
...
export default function POSDashboard() {
  ...
  const salesGrowth =
    yesterday?.totalSales && yesterday.totalSales > 0
      ? (((today?.totalSales ?? 0) - yesterday.totalSales) /
          ...
          <div className="flex items-center gap-3">
            ...
            {today && (
              <div className="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 backdrop-blur-sm">
                ...
                {salesGrowth !== null && (
                  <div
                    className={`flex items-center gap-0.5 rounded-full px-2 py-0.5 text-[10px] font-bold ${salesGrowth >= 0 ? 'bg-green-500/30 text-green-200' : 'bg-red-500/30 text-red-200'}`}
                  >
                    <PiTrendUp
                      className={`h-3 w-3 ${salesGrowth < 0 ? 'rotate-180' : ''}`}
                    />
        ...
        {!token ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h2 className="text-base font-semibold text-gray-700">
              Not signed in to POS
            </h2>
            <p className="mt-1 text-sm text-gray-400">
              Log in as a cashier or manager to view the dashboard
            </p>
            <a
          ...
          <>
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
              <StatCard
                ...
                sub={
                  salesGrowth != null
                    ? `${salesGrowth >= 0 ? '↑' : '↓'} ${Math.abs(salesGrowth).toFixed(0)}% vs yesterday`
                    : undefined
                }
              ...
              <StatCard
                ...
                sub={
                  today?.orderCount && today.totalSales
                    ? `Avg ${formatCurrency(today.totalSales / today.orderCount)}`
                    : undefined
                }
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  {allShops.map((shop) => (
                    <TerminalCard
                      ...
                      id={shop._id}
                      title={shop.name}
                      badge={shop.mode === 'retail' ? 'CASHIERS' : 'MANAGERS'}
                      description={
                        shop.description ||
                        (shop.mode === 'retail'
                          ? 'Front-counter sales'
                      ...
                      avatarLetter={shop.name.charAt(0).toUpperCase()}
                      avatarBg=""
                      avatarColor={shop.color}
                      info={terminalInfo[shop._id] ?? DEFAULT_TERM}
                      onOpen={() => {}}
                    />
                  ))}
                </div>
                    ...
                    <div className="mb-4 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">
                          Revenue bars · dot = order count · hover for details
                        </p>
                      </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-5">
                    ...
                    <div className="space-y-2.5">
                      {breakdownEntries.map(([method, { total, count }]) => {
                        const pct = today?.totalSales
                          ? Math.round((total / today.totalSales) * 100)
                          : 0;
                        return (
                          <div key={method}>
