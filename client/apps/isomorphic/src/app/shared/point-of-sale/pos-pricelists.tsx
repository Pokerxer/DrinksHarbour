...
import {
  ...
  PiCheckSquare, PiSquare, PiDotsSixVertical, PiTrash, PiSpinner,
  PiWarningCircle, PiFloppyDisk, PiArrowLeft, PiLightning, PiCaretDown,
  PiArrowsClockwise, PiTag, PiReceipt, PiSealPercent,
  PiInfo, PiPercent, PiCurrencyNgn, PiFunction, PiPackage,
  PiArrowDown, PiArrowUp, PiWarning, PiPencilSimple,
} from 'react-icons/pi';
...
function priceLabel(rule: any) {
  if (rule.priceType === 'fixed')      return `${Number(rule.fixedPrice || 0).toFixed(2)} ₦`;
  if (rule.priceType === 'formula')    return `${rule.markupPercentage || 0}% markup on cost`;
  if (rule.priceType === 'flash_sale') return `⚡ ${rule.flashSalePercentage || 0}% flash sale`;
  if (rule.priceType === 'bundle') {
    const qty = rule.bundleQuantity || 2;
    const dt  = rule.bundleDiscountType;
    if (dt === 'markup_on_cost') return `📦 Buy ${qty}+ · Cost +${rule.bundleDiscount || 0}% markup`;
  ...
  if (rule.priceType === 'discount') {
    if (rule.discountType === 'fixed') return `-₦${Number(rule.discountAmount || 0).toFixed(2)}`;
...
function Tip({ text }: { text: string }) {
  return (
    <span title={text}
      className="inline-flex h-3.5 w-3.5 cursor-help items-center justify-center rounded-full bg-gray-200 text-[9px] font-bold text-gray-500">?</span>
  );
}
...
interface FieldProps { label: string; error?: string; hint?: string; children: React.ReactNode; }
function RuleField({ label, error, hint, children }: FieldProps) {
  return (
    <div>
      ...
      {error && <p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-red-500"><PiWarning className="h-3 w-3"/>{error}</p>}
    </div>
  );
}

interface RuleInputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  hasError?: boolean; prefix?: React.ReactNode; suffix?: React.ReactNode;
}
function RuleInput({ hasError, prefix, suffix, className: _cls, ...props }: RuleInputProps) {
  return (
    <div className={`flex h-9 items-center overflow-hidden rounded-lg border bg-white transition-colors focus-within:ring-1 ${
      hasError ? 'border-red-400 focus-within:border-red-400 focus-within:ring-red-200' : 'border-gray-200 focus-within:border-[#b20202] focus-within:ring-[#b20202]/10'
    }`}>
      {prefix && <span className="shrink-0 pl-3 text-sm text-gray-400">{prefix}</span>}
      <input {...props} className="h-full flex-1 bg-transparent px-3 text-sm tabular-nums outline-none" />
      {suffix && <span className="shrink-0 pr-3 text-sm text-gray-400">{suffix}</span>}
    </div>
  );
}

const RULE_EMPTY = {
  applyTo:'product', subProduct:'', appliedOn:'',
  priceType:'discount',
  fixedPrice:'', markupPercentage:'',
  discountType:'percentage', discountPercentage:'', discountAmount:'',
  flashSalePercentage:'', flashSaleQty:'',
  bundleName:'', bundleQuantity:'2', bundleDiscount:'', bundleDiscountType:'percentage',
  minQuantity:'', startDate:'', endDate:'',
};

const RULE_TYPE_META = {
  discount:   { label:'Discount',    Icon: PiPercent,     color:'#059669', bg:'#ecfdf5', border:'#a7f3d0', hint:'Applied at runtime — base price stays the same' },
  flash_sale: { label:'Flash Sale',  Icon: PiLightning,   color:'#d97706', bg:'#fffbeb', border:'#fcd34d', hint:'Highest priority — overrides regular discount' },
  fixed:      { label:'Fixed Price', Icon: PiCurrencyNgn, color:'#2563eb', bg:'#eff6ff', border:'#bfdbfe', hint:'Sets the base selling price directly' },
  formula:    { label:'Formula',     Icon: PiFunction,    color:'#7c3aed', bg:'#f5f3ff', border:'#ddd6fe', hint:'Price = cost × (1 + markup%)' },
  bundle:     { label:'Bundle',      Icon: PiPackage,     color:'#9333ea', bg:'#faf5ff', border:'#e9d5ff', hint:'Applied at order time when qty threshold is met' },
};

const PCT_PRESETS = [5, 10, 15, 20, 25, 30, 50];

function PctChips({ value, onChange, activeColor }: { value: string; onChange: (v: string) => void; activeColor: string }) {
  return (
    <div className="mt-2 flex flex-wrap gap-1.5">
      {PCT_PRESETS.map(v => (
        <button key={v} type="button" onClick={() => onChange(value === String(v) ? '' : String(v))}
          className={`rounded-full px-2.5 py-1 text-[11px] font-bold transition-all border ${value === String(v) ? 'text-white border-transparent' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}
...
function Seg({ options, value, onChange, activeColor }: { options: [string,string][]; value: string; onChange: (v:string)=>void; activeColor: string }) {
  return (
...
function ruleToFormValues(rule: any): typeof RULE_EMPTY {
  const subProductId = rule.subProduct?._id
    ...
    : rule.subProduct ? String(rule.subProduct) : '';
  ...
  return {
    ...
    discountPercentage:  rule.discountPercentage ? String(rule.discountPercentage) : '',
    discountAmount:      rule.discountAmount ? String(rule.discountAmount) : '',
    flashSalePercentage: rule.flashSalePercentage ? String(rule.flashSalePercentage) : '',
...
function CreateRuleModal({ token, products, onSave, onSaveNew, onDiscard, initialValues = null }) {
  const isEdit = !!initialValues;
  const [form,     setForm]     = useState(isEdit ? ruleToFormValues(initialValues) : { ...RULE_EMPTY });
  ...
  const dispName    = selProduct
    ...
    : (form.appliedOn && form.appliedOn !== 'All products' ? form.appliedOn : '');
  ...
  const conflict = (() => {
    ...
    if (pt === 'discount' && selProduct.isOnSale && selProduct.saleDiscountValue > 0)
  ...
  const preview = (() => {
    ...
    if (pt === 'fixed') {
      ...
      return { label: 'New selling price', value: fp, delta, sub: basePrice > 0 ? `was ${fmt(basePrice)}` : 'no current price', color: RULE_TYPE_META.fixed.color };
    }
    if (pt === 'formula') {
      const mp = parseFloat(form.markupPercentage) || 0;
      if (!mp) return null;
      if (!costPrice) return { label: 'Formula', value: 0, delta: 0, sub: 'select a product with a cost price to preview', color: RULE_TYPE_META.formula.color, noValue: true };
      const computed = Math.round(costPrice * (1 + mp / 100) * 100) / 100;
      const delta = basePrice > 0 ? computed - basePrice : 0;
      return { label: 'Computed price', value: computed, delta, sub: `${fmt(costPrice)} cost + ${mp}% markup`, color: RULE_TYPE_META.formula.color };
    }
    if (pt === 'discount') {
      if (!basePrice) return { label: 'Discount', value: 0, delta: 0, sub: 'select a product to preview savings', color: RULE_TYPE_META.discount.color, noValue: true };
      const isFixed = form.discountType === 'fixed';
      const val = isFixed ? (parseFloat(form.discountAmount) || 0) : (parseFloat(form.discountPercentage) || 0);
      if (!val) return null;
      const sale = isFixed ? Math.max(0, basePrice - val) : basePrice * (1 - val / 100);
      const saving = basePrice - sale;
      return { label: 'After discount', value: sale, delta: -saving, sub: `customer saves ${fmt(saving)} (${((saving / basePrice) * 100).toFixed(1)}%)`, color: RULE_TYPE_META.discount.color };
    }
    if (pt === 'flash_sale') {
      if (!basePrice) return { label: 'Flash price', value: 0, delta: 0, sub: 'select a product to preview', color: RULE_TYPE_META.flash_sale.color, noValue: true };
      const pct = parseFloat(form.flashSalePercentage) || 0;
      if (!pct) return null;
      const flash = basePrice * (1 - pct / 100);
      const saving = basePrice - flash;
      return { label: '⚡ Flash price', value: flash, delta: -saving, sub: `${pct}% off · saves ${fmt(saving)}`, color: RULE_TYPE_META.flash_sale.color };
    }
    if (pt === 'bundle') {
      ...
      if (dt === 'no_discount') {
        ...
        if (!basePrice) return { label: 'Bundle', value: 0, delta: 0, sub: 'select a product to preview', color: RULE_TYPE_META.bundle.color, noValue: true };
        return { label: `Buy ${qty}+ · No discount`, value: basePrice * qty, delta: 0, sub: `${fmt(basePrice)} each — sale discount removed`, color: RULE_TYPE_META.bundle.color };
      }
      if (dt === 'markup_on_cost') {
        if (!disc) return null;
        if (!costPrice) return { label: 'Bundle', value: 0, delta: 0, sub: 'select a product with a cost price to preview', color: RULE_TYPE_META.bundle.color, noValue: true };
        const unitPrice = Math.round(costPrice * (1 + disc / 100) * 100) / 100;
        const delta     = basePrice > 0 ? (unitPrice - basePrice) * qty : 0;
        return { label: `Buy ${qty}+ · Cost markup`, value: unitPrice * qty, delta, sub: `${fmt(unitPrice)} each (cost ${fmt(costPrice)} + ${disc}% markup)`, color: RULE_TYPE_META.bundle.color };
      }
      if (!disc) return null;
      if (!basePrice) return { label: 'Bundle', value: 0, delta: 0, sub: 'select a product to preview bundle total', color: RULE_TYPE_META.bundle.color, noValue: true };
      const unitSale = dt === 'fixed' ? Math.max(0, basePrice - disc) : basePrice * (1 - disc / 100);
      return { label: `Buy ${qty} total`, value: unitSale * qty, delta: -(basePrice - unitSale) * qty, sub: `${fmt(unitSale)} each (was ${fmt(basePrice)})`, color: RULE_TYPE_META.bundle.color };
    }
    return null;
  })();
  ...
  function validate(): Record<string, string> {
    ...
    const pt = form.priceType;
    if (pt === 'fixed'   && !(parseFloat(form.fixedPrice) || 0))       e.fixedPrice       = 'Enter a price';
    if (pt === 'formula' && !(parseFloat(form.markupPercentage) || 0)) e.markupPercentage = 'Enter a markup %';
    if (pt === 'discount') {
      if (form.discountType === 'percentage' && !(parseFloat(form.discountPercentage) || 0)) e.discountPercentage = 'Enter a discount %';
      if (form.discountType === 'fixed'      && !(parseFloat(form.discountAmount) || 0))     e.discountAmount     = 'Enter an amount';
    }
    if (pt === 'flash_sale' && !(parseFloat(form.flashSalePercentage) || 0)) e.flashSalePercentage = 'Enter a discount %';
    if (pt === 'bundle') {
      if ((parseFloat(form.bundleQuantity) || 0) < 2) e.bundleQuantity = 'Min 2 units';
      if (form.bundleDiscountType !== 'no_discount' && !(parseFloat(form.bundleDiscount) || 0))
        e.bundleDiscount = form.bundleDiscountType === 'markup_on_cost' ? 'Enter a markup %' : 'Enter a discount';
    }
  ...
  function buildPayload() {
    const qty = parseFloat(form.bundleQuantity) || 2;
    const disc = parseFloat(form.bundleDiscount) || 0;
    const bundleName = form.bundleName || `Buy ${qty}+ · ${form.bundleDiscountType === 'fixed' ? `₦${disc}` : `${disc}%`} off`;
    return {
  ...
  async function handle(mode: 'close' | 'new') {
    ...
    try {
      if (mode === 'close') {
        ...
        setForm(p => ({ ...RULE_EMPTY, priceType: p.priceType, discountType: p.discountType, bundleDiscountType: p.bundleDiscountType }));
        setSearch(''); setErrors({});
  ...
  const filtered = products.filter((p: any) =>
    `${p.product?.name || ''} ${p.sku || ''}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative flex w-full max-w-xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" style={{ maxHeight: '92vh' }}>

        {/* ── Top accent bar ── */}
        <div className="h-1 shrink-0 w-full" style={{ backgroundColor: typeMeta.color }} />

        {/* ── Header ── */}
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-5 py-3.5">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: typeMeta.bg, color: typeMeta.color }}>
              <TypeIcon className="h-4 w-4" />
            </div>
            <div>
              <p className="text-sm font-bold text-gray-900">{isEdit ? 'Edit Price Rule' : 'Add Price Rule'}</p>
              <p className="text-[11px] text-gray-400">{typeMeta.hint}</p>
            </div>
          </div>
          <button type="button" onClick={onDiscard} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {/* ── Price Type selector ── */}
          <div>
            <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-gray-400">Rule Type</p>
              ...
              {Object.entries(RULE_TYPE_META).map(([v, m]) => {
                ...
                return (
                  <button key={v} type="button" onClick={() => switchType(v)}
                    className={`flex flex-col items-center gap-1 rounded-xl border-2 py-2.5 px-1 text-center transition-all ${active ? 'shadow-sm' : 'border-gray-200 bg-gray-50 hover:border-gray-300 hover:bg-white'}`}
                    style={active ? { borderColor: m.color, backgroundColor: m.bg } : {}}>
                    <Icon className="h-4 w-4" style={{ color: active ? m.color : '#6b7280' }} />
                    <span className="text-[10px] font-semibold leading-tight" style={{ color: active ? m.color : '#374151' }}>{m.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          {/* ── Product picker ── */}
          <RuleField label="Product" hint="Leave blank to apply to all products">
            <div className="relative" ref={dropRef}>
              <div className={`flex h-9 items-center overflow-hidden rounded-lg border transition-colors ${showDrop ? 'border-[#b20202] ring-1 ring-[#b20202]/10' : 'border-gray-200 hover:border-gray-300'}`}>
                <PiMagnifyingGlass className="ml-3 h-4 w-4 shrink-0 text-gray-400" />
                <input ref={searchRef} type="text"
                  value={showDrop ? search : dispName}
                  onFocus={() => { setSearch(dispName); setShowDrop(true); }}
                ...
                {form.subProduct && (
                  <button type="button" onClick={() => { f('subProduct', ''); f('appliedOn', ''); }}
                    className="mr-2 shrink-0 text-gray-400 hover:text-gray-600"><PiX className="h-3.5 w-3.5" /></button>
                )}
              </div>
              {showDrop && (
                <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-52 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-xl">
                  <button type="button" onClick={() => { f('subProduct', ''); f('appliedOn', ''); setShowDrop(false); setSearch(''); }}
                    className="flex w-full items-center gap-3 border-b border-gray-100 px-4 py-2.5 text-sm text-gray-400 italic hover:bg-gray-50">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gray-100 text-xs text-gray-400">★</span>
                    All products
                  </button>
                  {filtered.slice(0, 50).map((p: any) => {
                    ...
                    return (
                      <button key={p._id} type="button"
                        onClick={() => { f('subProduct', p._id); f('appliedOn', name); setShowDrop(false); setSearch(''); }}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className={`truncate ${isSel ? 'text-[#b20202]' : ''}`}>{name}</div>
                          <div className="mt-0.5 flex items-center gap-1.5">
                            <span className="font-mono text-[10px] text-gray-400">{p.sku}</span>
                            {p.costPrice > 0 && <span className="text-[10px] text-gray-400">cost {fmt(p.costPrice)}</span>}
                            {p.isOnSale && <span className="rounded-full bg-emerald-100 px-1.5 py-px text-[9px] font-bold text-emerald-700">On Sale</span>}
                            {p.flashSale?.isActive && <span className="rounded-full bg-amber-100 px-1.5 py-px text-[9px] font-bold text-amber-700">⚡ Flash</span>}
                            {(p.bundleDeals?.length > 0) && <span className="rounded-full bg-purple-100 px-1.5 py-px text-[9px] font-bold text-purple-700">📦 Bundle</span>}
                          </div>
                        </div>
                        <span className="shrink-0 text-xs font-semibold tabular-nums text-gray-600">{fmt(p.baseSellingPrice || 0)}</span>
                      </button>
                    );
                  })}
                  {filtered.length === 0 && <p className="px-4 py-5 text-center text-xs text-gray-400">No products found</p>}
                </div>
              )}
            </div>
          </RuleField>
          ...
          <div className="rounded-xl border bg-gray-50 p-4 space-y-3" style={{ borderColor: typeMeta.border }}>

            {/* Fixed Price */}
            {form.priceType === 'fixed' && (
              <RuleField label="Selling Price" error={errors.fixedPrice}
                hint={basePrice > 0 ? `Current: ${fmt(basePrice)}` : undefined}>
                <RuleInput hasError={!!errors.fixedPrice} prefix="₦" type="number" min="0" step="0.01"
                  placeholder="e.g. 5000" value={form.fixedPrice} onChange={(e: any) => f('fixedPrice', e.target.value)} autoFocus />
              </RuleField>
            )}
            ...
            {form.priceType === 'formula' && (
              <>
                <RuleField label="Markup %" error={errors.markupPercentage} hint="New price = cost × (1 + markup%)">
                  <RuleInput hasError={!!errors.markupPercentage} suffix="%" type="number" min="0" step="0.1"
                    placeholder="e.g. 25" value={form.markupPercentage} onChange={(e: any) => f('markupPercentage', e.target.value)} autoFocus />
                </RuleField>
                {!costPrice && (
                  <div className="flex items-center gap-2 text-[11px] text-amber-700">
                    <PiInfo className="h-3.5 w-3.5 shrink-0" />
                    {form.subProduct ? 'No cost price on this product — rule will be skipped.' : 'Products without a cost price will be skipped when applied.'}
                  </div>
                )}
              </>
            )}
            ...
            {form.priceType === 'discount' && (
              <>
                <Seg options={[['percentage','% Off'],['fixed','₦ Off']]}
                  value={form.discountType} onChange={v => f('discountType', v)} activeColor={RULE_TYPE_META.discount.color} />
                {form.discountType === 'percentage' ? (
                  <RuleField label="Discount %" error={errors.discountPercentage}>
                    <RuleInput hasError={!!errors.discountPercentage} suffix="%" type="number" min="0" max="100" step="0.1"
                      placeholder="e.g. 15" value={form.discountPercentage} onChange={(e: any) => f('discountPercentage', e.target.value)} autoFocus />
                    <PctChips value={form.discountPercentage} onChange={v => f('discountPercentage', v)} activeColor={RULE_TYPE_META.discount.color} />
                  </RuleField>
                ) : (
                  <RuleField label="Amount Off (₦)" error={errors.discountAmount}>
                    <RuleInput hasError={!!errors.discountAmount} prefix="₦" type="number" min="0" step="0.01"
                      placeholder="e.g. 500" value={form.discountAmount} onChange={(e: any) => f('discountAmount', e.target.value)} autoFocus />
                  </RuleField>
                )}
              </>
            )}
            ...
            {form.priceType === 'flash_sale' && (
              <>
                <RuleField label="Flash Discount %" error={errors.flashSalePercentage}>
                  <RuleInput hasError={!!errors.flashSalePercentage} suffix="%" type="number" min="0" max="100" step="0.1"
                    placeholder="e.g. 30" value={form.flashSalePercentage} onChange={(e: any) => f('flashSalePercentage', e.target.value)} autoFocus />
                  <PctChips value={form.flashSalePercentage} onChange={v => f('flashSalePercentage', v)} activeColor={RULE_TYPE_META.flash_sale.color} />
                </RuleField>
                <RuleField label="Limited Qty" hint="Leave blank for unlimited">
                  <RuleInput type="number" min="0" step="1"
                    placeholder="e.g. 50" value={form.flashSaleQty} onChange={(e: any) => f('flashSaleQty', e.target.value)} />
                </RuleField>
              </>
            )}
            ...
            {form.priceType === 'bundle' && (
              <>
                <RuleField label="Bundle Name" hint="Auto-generated if blank">
                  <RuleInput type="text"
                    placeholder={
                      form.bundleDiscountType === 'markup_on_cost' ? `Buy ${form.bundleQuantity || 2}+ · Cost +${form.bundleDiscount || '?'}% markup`
                      : form.bundleDiscountType === 'no_discount'  ? `Buy ${form.bundleQuantity || 2}+ · No discount`
                      : form.bundleDiscountType === 'fixed'        ? `Buy ${form.bundleQuantity || 2}+ · ₦${form.bundleDiscount || '?'} off`
                      : `Buy ${form.bundleQuantity || 2}+ · ${form.bundleDiscount || '?'}% off`
                    }
                    value={form.bundleName} onChange={(e: any) => f('bundleName', e.target.value)} />
                </RuleField>

                <RuleField label="Min Quantity to Qualify" error={errors.bundleQuantity}>
                  <div className="flex flex-wrap items-center gap-1.5">
                    {[2, 3, 4, 6, 12, 24].map(q => (
                      <button key={q} type="button" onClick={() => f('bundleQuantity', String(q))}
                        ...
                        style={form.bundleQuantity === String(q) ? { backgroundColor: RULE_TYPE_META.bundle.color } : {}}>
                        {q}
                      </button>
                    ))}
                    <input type="number" min="2" step="1" value={form.bundleQuantity}
                ...
                <RuleField label="Pricing Type">
                  <div className="grid grid-cols-2 gap-1.5">
                    {([
                      ...
                      return (
                        <button key={v} type="button" onClick={() => f('bundleDiscountType', v)}
                          ...
                          style={active ? { borderColor: RULE_TYPE_META.bundle.color, backgroundColor: '#faf5ff' } : {}}>
                          <span className="text-xs font-bold" style={{ color: active ? RULE_TYPE_META.bundle.color : '#374151' }}>{l}</span>
                ...
                {form.bundleDiscountType !== 'no_discount' && (
                  <RuleField
                    ...
                    hint={form.bundleDiscountType === 'markup_on_cost' ? 'Bundle price = cost price × (1 + markup%)' : undefined}>
                    <RuleInput hasError={!!errors.bundleDiscount}
                      prefix={form.bundleDiscountType === 'fixed' ? '₦' : undefined}
                      suffix={form.bundleDiscountType !== 'fixed' ? '%' : undefined}
                      type="number" min="0" step="0.1"
                      placeholder={form.bundleDiscountType === 'fixed' ? '500' : form.bundleDiscountType === 'markup_on_cost' ? '15' : '20'}
                      value={form.bundleDiscount} onChange={(e: any) => f('bundleDiscount', e.target.value)} />
                    {(form.bundleDiscountType === 'percentage' || form.bundleDiscountType === 'markup_on_cost') && (
                      <PctChips value={form.bundleDiscount} onChange={v => f('bundleDiscount', v)} activeColor={RULE_TYPE_META.bundle.color} />
                    )}
                  </RuleField>
                )}
                ...
                {form.bundleDiscountType === 'no_discount' && (
                  <div className="flex items-start gap-2 rounded-xl border border-purple-200 bg-purple-50 px-3 py-2.5 text-[11px] text-purple-800">
                    ...
                    Customers buying {form.bundleQuantity || 2}+ units will be charged the base selling price — any active sale or flash-sale discount is removed.
                  </div>
                )}
              </>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            <RuleField label="Min Order Qty" hint="0 = any quantity">
              <RuleInput type="number" min="0" step="1"
                placeholder="0" value={form.minQuantity} onChange={(e: any) => f('minQuantity', e.target.value)} />
            </RuleField>
            <RuleField label="Valid From">
              <input type="date" min={today} value={form.startDate} onChange={e => f('startDate', e.target.value)}
            ...
            <RuleField label="Valid Until" error={errors.endDate}>
              <input type="date" min={form.startDate || today} value={form.endDate} onChange={e => f('endDate', e.target.value)}
          ...
          {preview && !preview.noValue ? (
            <div className="flex items-center justify-between rounded-xl border-2 px-4 py-3 transition-all"
              style={{ borderColor: typeMeta.border, backgroundColor: typeMeta.bg }}>
              <div>
                <p className="text-[11px] font-bold uppercase tracking-wider" style={{ color: typeMeta.color }}>{preview.label}</p>
                <p className="mt-0.5 text-[11px] text-gray-500">{preview.sub}</p>
                {preview.delta !== 0 && (
                  <p className="mt-0.5 flex items-center gap-1 text-[11px] font-semibold" style={{ color: preview.delta < 0 ? '#059669' : '#dc2626' }}>
                    {preview.delta < 0 ? <PiArrowDown className="h-3 w-3" /> : <PiArrowUp className="h-3 w-3" />}
                    {preview.delta < 0 ? '-' : '+'}{fmt(Math.abs(preview.delta))} vs current
                  </p>
                )}
              </div>
              <p className="text-2xl font-black tabular-nums" style={{ color: typeMeta.color }}>
                {fmt(preview.value)}
              </p>
            </div>
          ) : preview?.noValue ? (
          ...
          {!form.subProduct && (
            <div className="flex items-start gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-[11px] text-blue-700">
              ...
              <span>No product selected — this rule will apply to <strong>all products</strong> when the pricelist is applied.</span>
            </div>
          )}
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2 border-t border-gray-100 bg-white px-5 py-3">
          {/* Primary: Save */}
          <button type="button" onClick={() => handle('close')} disabled={!!saving}
            className="flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-bold text-white transition-opacity disabled:opacity-50 hover:opacity-90"
          ...
          {!isEdit && (
            <button type="button" onClick={() => handle('new')} disabled={!!saving}
              className="flex items-center gap-1.5 rounded-lg border-2 px-4 py-2 text-sm font-bold transition-colors disabled:opacity-50 hover:bg-opacity-10"
...
function ruleStatus(r: any): { label: string; cls: string } {
  const now = new Date();
  if (r.endDate   && new Date(r.endDate)   < now) return { label: 'Expired',  cls: 'bg-red-50 text-red-500' };
  if (r.startDate && new Date(r.startDate) > now) return { label: 'Pending',  cls: 'bg-blue-50 text-blue-600' };
  if (r.startDate || r.endDate)                   return { label: 'Active',   cls: 'bg-emerald-50 text-emerald-700' };
...
function ruleDescription(rule: any): string {
  switch (rule.priceType) {
    ...
    case 'bundle': {
      ...
      if (dt === 'fixed')          return `Buy ${qty}+ → -₦${Number(rule.bundleDiscount || 0).toFixed(0)} per unit`;
...
function RuleCard({ rule, onDelete, onEdit, deleting }: {
  ...
  const isAllProducts = !sp && (!rule.appliedOn || rule.appliedOn === 'All products');
  const productName   = isAllProducts ? null : (rule.appliedOn || sp?.product?.name || sp?.sku);
  ...
  return (
    <div className={`group relative flex gap-3 border-b border-gray-100 px-4 py-3.5 transition-colors hover:bg-gray-50/60 ${isExpired ? 'opacity-40' : ''}`}>
      {/* Coloured left bar */}
      <div className="absolute inset-y-0 left-0 w-1 rounded-r" style={{ backgroundColor: meta.color }} />
      ...
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {isAllProducts ? (
            ...
            <span className="truncate text-xs font-semibold text-gray-800">{productName}</span>
          ) : null}
          <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold ${status.cls}`}>
            {status.label}
          </span>
        </div>
        ...
        {constraints.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            {constraints.map((c, i) => (
              <span key={i} className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500">{c}</span>
            ))}
          </div>
        )}
        ...
        {hasProductState && (
          <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
            <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-300">Now:</span>
            ...
            {!sp.flashSale?.isActive && sp.isOnSale && sp.saleDiscountValue > 0 && (
              <span className="rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-700">
                {sp.saleType === 'fixed' ? `-₦${sp.saleDiscountValue}` : `${sp.saleDiscountValue}% off`} active
              </span>
            )}
            {(sp.bundleDeals?.length > 0) && (
              <span className="rounded bg-purple-50 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700">
                📦 {sp.bundleDeals.length} bundle deal{sp.bundleDeals.length > 1 ? 's' : ''}
              </span>
            )}
          </div>
        )}
        ...
        {(rule.priceType === 'fixed' || rule.priceType === 'formula') && (
          <p className="text-[10px] italic text-gray-400">
            Deleting this rule won't revert the base price — apply a new rule or update manually.
          </p>
        )}
      </div>
      ...
      <div className="flex shrink-0 items-start gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <button type="button" onClick={onDelete} disabled={deleting}
          ...
          {deleting ? <PiSpinner className="h-3.5 w-3.5 animate-spin" /> : <PiTrash className="h-3.5 w-3.5" />}
        </button>
      </div>
    </div>
  );
...
function PricelistPanel({ pl, token, onClose, onRefresh }) {
  ...
  useEffect(() => {
    ...
    subproductService.getSubProducts(token, { limit: 500 })
  ...
  async function handleApply() {
    ...
    if (activeRules.length === 0) { toast.error('No active rules to apply'); return; }
    setApplying(true);
    try {
      const res = await pricelistService.apply(pl._id, token);
      const d = res.data;
      toast.success(d.message || `${d.modified} product${d.modified === 1 ? '' : 's'} updated`);
      if (d.skipped > 0) toast(`${d.skipped} rule${d.skipped === 1 ? '' : 's'} skipped`, { icon: '⚠️' });
      if (d.errors?.length > 0) toast.error(`${d.errors.length} rule${d.errors.length === 1 ? '' : 's'} failed`);
  ...
  const now = new Date();
  const expiredCount = rules.filter((r: any) => r.endDate && new Date(r.endDate) < now).length;
  ...
  return (
    <div className="flex h-full flex-col bg-white">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {pl?.isSelectable && !dirty && (
              <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-bold text-emerald-700">Selectable</span>
            )}
          </div>
          <p className="mt-0.5 text-[10px] text-gray-400">
            {pl?.currency || 'NGN'} · {activeCount} rule{activeCount !== 1 ? 's' : ''}{expiredCount > 0 ? ` · ${expiredCount} expired` : ''}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          {dirty && (
            <button type="button" onClick={saveMeta} disabled={saving}
              className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-bold text-white disabled:opacity-50 hover:opacity-90"
              style={{ backgroundColor: '#b20202' }}>
              {saving ? <PiSpinner className="h-3 w-3 animate-spin" /> : <PiFloppyDisk className="h-3 w-3" />}
              Save
            </button>
          )}
          <button type="button" onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600">
      ...
      <div className="shrink-0 flex items-center gap-3 border-b border-gray-100 bg-gray-50/50 px-4 py-2 text-[11px]">
        <div className="flex items-center gap-1.5">
          <span className="text-gray-400">Currency</span>
          <select value={currency} onChange={e => { setCurrency(e.target.value); setDirty(true); }}
        ...
        <div className="flex flex-1 items-center gap-1.5 min-w-0">
          ...
          <input value={website} onChange={e => { setWebsite(e.target.value); setDirty(true); }}
        ...
        <label className="flex cursor-pointer items-center gap-1.5">
          <input type="checkbox" checked={selectable} onChange={e => { setSelectable(e.target.checked); setDirty(true); }}
        ...
        {(['rules', 'ecommerce'] as const).map(t => (
          <button key={t} type="button" onClick={() => setTab(t)}
            className={`flex flex-1 items-center justify-center py-2.5 capitalize transition-colors ${
              tab === t ? 'border-b-2 border-[#b20202] text-[#b20202]' : 'border-b-2 border-transparent text-gray-400 hover:text-gray-600'
            }`}>
            {t === 'rules' ? `Price Rules${rules.length > 0 ? ` (${rules.length})` : ''}` : 'Ecommerce'}
          </button>
        ))}
      </div>
      ...
      {tab === 'rules' && (
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex shrink-0 items-center gap-2 border-b border-gray-100 px-4 py-2">
            ...
            <button type="button" onClick={handleApply} disabled={applying || rules.length === 0}
              className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40 hover:opacity-90 transition-opacity"
              style={{ backgroundColor: '#b20202' }}>
              {applying ? <PiSpinner className="h-3 w-3 animate-spin" /> : <PiLightning className="h-3 w-3" />}
              Apply prices
            </button>
            ...
            {productsLoading && rules.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-gray-400">Click "Add rule" to create your first rule</p>
                </div>
                <button type="button" onClick={() => setShowModal(true)}
              ...
              <>
                {rules.length > PAGE && (
                  <div className="flex items-center justify-between border-b border-gray-100 px-4 py-1.5 text-[10px] text-gray-400">
                    <span>{(page - 1) * PAGE + 1}–{Math.min(page * PAGE, rules.length)} of {rules.length}</span>
                ...
                {expiredCount > 0 && (
                  <p className="px-4 py-2 text-center text-[10px] text-gray-400">
                    {expiredCount} expired rule{expiredCount !== 1 ? 's' : ''} above — they are skipped when applying
                  </p>
                )}
              </>
            )}
      ...
      {showModal && (
        <CreateRuleModal
          token={token}
          products={products}
          onSave={async r => { await saveRule(r, false); }}
          onSaveNew={async r => { await saveRule(r, true); }}
      ...
      {editRule && (
        <CreateRuleModal
          ...
          onSave={async r => { await saveEditedRule(editRule._id, r); }}
          onSaveNew={async r => { await saveEditedRule(editRule._id, r); }}
...
export default function POSPricelists() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await pricelistService.list(token, { search, page, limit: PAGE });
  ...
  const allChecked  = displayList.length>0 && displayList.every(p=>checked.has(p._id));
  const someChecked = checked.size>0 && !allChecked;
  function toggleAll()         { setChecked(allChecked?new Set():new Set(displayList.map(p=>p._id))); }
    ...
    <div className="flex h-dvh flex-col bg-gray-50">
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-4 py-2.5">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-400">{total} total · {filtered.length} shown</p>
        </div>

        {/* Search */}
        <div className="relative flex-1 max-w-md">
          <div className={`flex overflow-hidden rounded-xl border bg-white transition-all ${search?'border-[#b20202] ring-1 ring-[#b20202]/10':'border-gray-200'}`}>
            ...
            {search && (
              <button type="button" onClick={()=>{setSearch('');setPage(1);}} className="flex items-center px-2 text-gray-400 hover:text-gray-600"><PiX className="h-3.5 w-3.5"/></button>
            )}
          </div>
        </div>
        ...
        <div className="flex rounded-xl border border-gray-200 bg-gray-50 p-0.5 text-xs font-semibold">
          {([['all','All'],['selectable','Selectable'],['website','Has Website']] as const).map(([k,l])=>(
      ...
      <div className="flex shrink-0 divide-x divide-gray-100 border-b border-gray-200 bg-white">
        {[
          { label:'Total',       value:String(rows.length),          icon:<PiTag className="h-4 w-4"/>     },
          { label:'Selectable',  value:String(totalSelectable),      icon:<PiReceipt className="h-4 w-4"/>, red:true },
          { label:'With Website',value:String(totalWebsite),         icon:<PiSealPercent className="h-4 w-4"/> },
          { label:'Currencies',  value:String(new Set(rows.map(r=>r.currency||'NGN')).size), icon:<PiInfo className="h-4 w-4"/> },
          ...
          {checked.size>0 && (
            <div className="shrink-0 flex items-center gap-3 bg-white px-4 py-2.5" style={{borderBottom:'2px solid #b20202'}}>
              <div className="flex-1 text-xs font-semibold text-gray-700">
                <span className="font-bold" style={{color:'#b20202'}}>{checked.size}</span> selected
              </div>
              ...
              <button type="button" onClick={handleDeleteSelected}
                ...
                style={{backgroundColor:'#b20202'}}>
                <PiTrash className="h-3.5 w-3.5"/> Delete {checked.size>1?`${checked.size} items`:'item'}
              </button>
            </div>
          )}
                ...
                <tbody>
                  ...
                  {creating && (
                    <tr className="border-b border-gray-100 bg-[#b20202]/5">
                      <td className="px-2 py-2.5"/><td className="px-1 py-2.5"/>
                      <td className="px-3 py-2.5" colSpan={5}>
                        <div className="flex items-center gap-2">
                          ...
                          <button type="button" onClick={handleCreate}
                            ...
                            style={{backgroundColor:'#b20202'}}>Create</button>
                          <button type="button" onClick={()=>{setCreating(false);setNewName('');}}
                            className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-500 hover:bg-gray-50">Cancel</button>
                        </div>
                      </td>
                  ...
                  {displayList.length===0 && !creating ? (
                    <tr>
                      <td colSpan={8} className="py-20 text-center">
                        ...
                        <p className="text-sm text-gray-400">No pricelists — click New to create one</p>
                      </td>
                    </tr>
                  ) : displayList.map(pl => {
                    ...
                    return (
                      <tr key={pl._id}
                        className={`border-b border-gray-100 cursor-pointer transition-colors ${isSel?'text-white':''+isChk?'bg-[#b20202]/5':'bg-white hover:bg-gray-50'}`}
                        style={isSel?{backgroundColor:'#b20202'}:isChk?{borderLeft:'2px solid #b20202'}:{borderLeft:'2px solid transparent'}}
                        onClick={()=>setSelected(isSel?null:pl)}>
                        <td className="w-8 px-2 py-2.5 text-center" onClick={e=>e.stopPropagation()}>
                          <button type="button" onClick={()=>toggleOne(pl._id)} className="text-gray-400 hover:text-[#b20202]">
                        ...
                        <td className={`px-3 py-2.5 ${isSel?'text-red-100':'text-gray-600'}`}>{pl.currency||'NGN'}</td>
                        <td className="px-3 py-2.5 text-center" onClick={e=>e.stopPropagation()}>
                          {pl.isSelectable
                            ?<PiCheckSquare className="mx-auto h-4 w-4" style={isSel?{color:'#fff'}:{color:'#b20202'}}/>
                            :<PiSquare className={`mx-auto h-4 w-4 ${isSel?'text-red-200':'text-gray-300'}`}/>}
                        </td>
                        <td className={`px-3 py-2.5 ${isSel?'text-red-100':'text-gray-600'}`}>
                          {pl.website||<span className={isSel?'text-red-200':'text-gray-300'}>—</span>}
                        </td>
                        <td className="w-8 px-2 py-2.5 text-center" onClick={e=>e.stopPropagation()}>
                          <button type="button" onClick={()=>handleDelete(pl._id)}
