...
import {
  PiList, PiX, PiClockCounterClockwise,
  PiArrowFatDown, PiArrowFatUp, PiArrowDown, PiArrowUp,
  PiStorefront, PiHouseSimple, PiLockKey,
  PiTimer, PiShoppingCart, PiCurrencyNgn,
  PiUserCircle, PiWarningCircle, PiCaretDown,
  PiTag, PiCheckCircle,
} from 'react-icons/pi';
...
function CashInOutModal({
  session, token, onClose, onMovementRecorded,
}: {
  session: POSSession;
  token: string;
  onClose: () => void;
  ...
  useEffect(() => {
    posApi.getCashMoves(token, session._id)
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const result = await posApi.recordCashMove(token, session._id, type, num, reason.trim() || undefined);
      setHistory(result.cashMovements || []);
      toast.success(`Cash ${type === 'in' ? 'In' : 'Out'}: ${formatCurrency(num)}${reason.trim() ? ` · ${reason.trim()}` : ''}`);
      setAmount(''); setReason('');
      onMovementRecorded?.();
    } catch (err: any) {
  ...
  const totalIn  = history.filter(m => m.type === 'in').reduce((s, m) => s + m.amount, 0);
  const totalOut = history.filter(m => m.type === 'out').reduce((s, m) => s + m.amount, 0);
      ...
      <div className="flex w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div>
            ...
            <p className="text-xs text-gray-400">{session.terminalType || 'retail'} terminal</p>
          </div>
          <button type="button" onClick={onClose} className="text-gray-400 hover:text-gray-600"><PiX className="h-5 w-5" /></button>
        </div>
        <div className="flex flex-1 overflow-hidden">
          <div className="flex w-64 shrink-0 flex-col border-r border-gray-100 px-5 py-5 space-y-4">
              ...
              {(['in', 'out'] as const).map((t) => (
                <button key={t} type="button" onClick={() => setType(t)}
                  className={`flex items-center justify-center gap-1.5 rounded-lg py-2.5 text-sm font-semibold transition-all ${
                    type === t ? (t === 'in' ? 'bg-white text-emerald-700 shadow-sm' : 'bg-white text-red-600 shadow-sm') : 'text-gray-400 hover:text-gray-600'
                  }`}>
                  {t === 'in' ? <PiArrowFatDown className="h-4 w-4" /> : <PiArrowFatUp className="h-4 w-4" />}
                  Cash {t === 'in' ? 'In' : 'Out'}
                </button>
              ))}
            </div>
            <div>
              <label className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-gray-400">Amount (₦)</label>
            ...
            <div>
              <label className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-gray-400">Reason <span className="font-normal text-gray-300">(optional)</span></label>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1 rounded-lg bg-red-50 px-3 py-2 text-center">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-red-500">Total Out</p>
                <p className="text-sm font-bold text-red-600">{formatCurrency(totalOut)}</p>
              </div>
            </div>
            <button type="button" onClick={handleSubmit}
              disabled={!amount || Number(amount) <= 0 || submitting}
              className="w-full rounded-xl py-3 text-sm font-bold text-white transition-opacity disabled:opacity-40 hover:opacity-90"
              style={{ backgroundColor: type === 'in' ? '#16a34a' : '#b20202' }}>
              {submitting ? 'Saving…' : `${type === 'in' ? 'Add Cash In' : 'Remove Cash Out'}`}
            </button>
          </div>
          <div className="flex flex-1 flex-col overflow-hidden">
            <div className="border-b border-gray-100 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Movement History</p>
            </div>
            {loadingHistory ? (
              <div className="flex flex-1 items-center justify-center"><div className="h-5 w-5 animate-spin rounded-full border-2 border-gray-200 border-t-gray-500" /></div>
            ) : history.length === 0 ? (
              <div className="flex flex-1 items-center justify-center p-6 text-center"><p className="text-sm text-gray-400">No movements yet</p></div>
            ) : (
              <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
                {[...history].reverse().map((m) => {
                  const cashier = m.performedBy ? (m.performedBy.posName || `${m.performedBy.firstName} ${m.performedBy.lastName}`) : '—';
                  const time = new Date(m.performedAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
                  return (
                    <div key={m._id} className="flex items-center gap-3 px-4 py-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${m.type === 'in' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                        {m.type === 'in' ? <PiArrowDown className="h-4 w-4" /> : <PiArrowUp className="h-4 w-4" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-gray-800">{m.reason || (m.type === 'in' ? 'Cash In' : 'Cash Out')}</p>
                        <p className="text-[10px] text-gray-400">{time} · {cashier}</p>
                      </div>
                      <span className={`shrink-0 text-sm font-bold tabular-nums ${m.type === 'in' ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        <div className="border-t border-gray-100 px-6 py-3 text-right">
          <button type="button" onClick={onClose} className="rounded-xl border border-gray-200 px-5 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Close</button>
        </div>
      </div>
    </div>
  );
...
function HamburgerMenu({
  session, token, onCloseRegister, onClose,
}: {
  session: POSSession;
  token: string;
  ...
  const items = [
    { label: 'Orders', icon: <PiClockCounterClockwise className="h-4 w-4" />, badge: session.orderCount > 0 ? session.orderCount : null, action: () => { router.push(routes.pos.orders); onClose(); } },
    { label: 'Cash In/Out', icon: <PiArrowFatDown className="h-4 w-4" />, action: () => setShowCashModal(true) },
    { label: 'Create Product', icon: <PiStorefront className="h-4 w-4" />, action: () => { window.open(routes.eCommerce.createProduct, '_blank'); onClose(); } },
    { label: 'Backend', icon: <PiHouseSimple className="h-4 w-4" />, action: () => { router.push(routes.pos.index); onClose(); } },
    { label: 'Close Register', icon: <PiLockKey className="h-4 w-4" />, danger: true, action: () => { onCloseRegister(); onClose(); } },
  ];

  return (
    <>
      <div ref={menuRef} className="absolute right-3 top-full mt-1.5 z-50 min-w-[220px] overflow-hidden rounded-2xl bg-white py-1 shadow-xl ring-1 ring-black/8">
        {items.map((item, i) => (
          <button key={item.label} type="button" onClick={item.action}
            ...
            {item.badge != null && (
              <span className="flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-[#b20202] px-1.5 text-[10px] font-bold text-white">{item.badge}</span>
            )}
          </button>
        ))}
      </div>
      {showCashModal && (
        <CashInOutModal session={session} token={token}
    ...
    <div className="relative" ref={menuRef}>
      <button
        ...
        className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-semibold transition-all ${
          open ? 'bg-white text-[#b20202]' : selectedPricelist ? 'bg-white/20 text-white' : 'bg-white/10 text-white/80 hover:bg-white/20 hover:text-white'
        }`}
      >
        ...
        <PiCaretDown className={`h-3 w-3 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
...
function BrandBlock({ tenantLogo, storeName, termLabel }: { tenantLogo: string | null; storeName: string; termLabel: string }) {
  return (
      ...
      <div className="flex h-9 items-center rounded-lg bg-white px-2.5">
        {tenantLogo ? (
          <div className="relative h-6 w-24 shrink-0 overflow-hidden">
            <Image src={tenantLogo} alt={storeName} fill style={{ objectFit: 'contain', objectPosition: 'left center' }} />
          </div>
        ) : (
          ...
          <img
            src="/logo.png"
            alt="Drinks Harbour"
            style={{ height: 24, width: 'auto', maxWidth: 150, objectFit: 'contain' }}
          />
        )}
      </div>
...
function StatChip({ icon, label, value, highlight }: { icon: React.ReactNode; label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`flex items-center gap-2 rounded-lg px-3 py-1.5 ${highlight ? 'bg-white/20' : 'bg-white/10'}`}>
      <span className="text-white/60">{icon}</span>
      <div>
        <p className="text-[9px] font-semibold uppercase tracking-widest text-white/60">{label}</p>
        <p className={`text-sm font-bold tabular-nums leading-tight ${highlight ? 'text-white' : 'text-white/90'}`}>{value}</p>
      </div>
    </div>
  );
}
...
export default function POSSessionBar({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    posApi.getSessionInfo(token, terminal ?? 'retail')
  ...
  const termLabel  = (terminal === 'wholesale' ? 'Wholesale' : 'Retail').toUpperCase();

  // ── No session ──────────────────────────────────────────────────────────────
  if (!session) {
    return (
      <div className={`relative flex h-14 shrink-0 items-center bg-[#b20202] px-4 ${className ?? ''}`}>

        {/* Left: brand */}
        <BrandBlock tenantLogo={tenantLogo} storeName={storeName} termLabel={termLabel} />
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">No open session</span>
        </div>
        ...
        {showOpenDialog ? (
          <div className="flex items-center gap-2">
            ...
            <input type="number" value={openingCash}
              ...
              className="w-28 rounded-lg border border-white/30 bg-white/15 px-3 py-1.5 text-sm text-white placeholder-white/40 outline-none focus:bg-white/25 focus:border-white" />
            <button onClick={handleOpenSession} disabled={opening}
              className="rounded-lg bg-white px-4 py-1.5 text-sm font-bold text-[#b20202] disabled:opacity-60 hover:bg-white/90">
              {opening ? 'Opening…' : 'Confirm'}
            </button>
            <button onClick={() => setShowOpenDialog(false)} className="text-xs text-white/60 hover:text-white">
              Cancel
            </button>
          </div>
        ) : (
    ...
    <>
      <div className={`relative flex h-14 shrink-0 items-center bg-[#b20202] px-4 ${className ?? ''}`}>
        ...
        <div className="flex items-center gap-2 mr-2">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-white/80">Open</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          <StatChip icon={<PiTimer className="h-3.5 w-3.5" />} label="Open" value={elapsed || '—'} />
          <StatChip icon={<PiShoppingCart className="h-3.5 w-3.5" />} label="Orders" value={String(session.orderCount)} />
          <StatChip icon={<PiCurrencyNgn className="h-3.5 w-3.5" />} label="Sales" value={formatCurrency(session.totalSales)} highlight />
        </div>
        ...
        <div className="flex items-center gap-2 mr-1">
          ...
          <div className="hidden sm:block">
            <p className="text-xs font-semibold leading-none text-white">{cashierName}</p>
            <p className="text-[9px] text-white/60 mt-0.5">Cashier</p>
          </div>
        </div>
        ...
        >
          ...
          <PiCaretDown className={`h-3 w-3 transition-transform ${showMenu ? 'rotate-180' : ''}`} />
        </button>
      ...
      {showCloseModal && (
        <POSCloseSessionModal
          session={session}
          onSessionClosed={() => { setSession(null); setShowCloseModal(false); }}
