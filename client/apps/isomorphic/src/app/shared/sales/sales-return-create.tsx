...
export default function SalesReturnCreate() {
  ...
  async function handleReturn() {
    ...
    setSubmitting(true);
    try {
      const res = await salesOrderService.return(
        so._id,
        { warehouseId, items },
        token
      );
      ...
      if (failures.length > 0) {
        toast(`Restocked ${successCount} line(s); ${failures.length} failed`, {
          icon: '⚠️',
          duration: 6000,
        });
      } else {
              ...
              <tbody className="divide-y divide-gray-100">
                {so.items.map((item) => {
                  ...
                  return (
                    <tr
                      key={item._id}
                      className={now > 0 ? 'bg-amber-50/40' : ''}
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            type="button"
                            disabled={max === 0}
                            onClick={() => adjustQty(item._id, max, -1)}
