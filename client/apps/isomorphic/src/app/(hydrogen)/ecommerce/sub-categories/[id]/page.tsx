...
import { Badge, Text, Title, Button } from 'rizzui';
import {
  PiArrowLeftBold,
  PiPencilLineBold,
  PiPackageBold,
...
export default function SubCategoryProductsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  async function fetchData(pageNum = 1) {
    ...
    setError(null);
    try {
      const [subCatRes, prodsRes] = await Promise.all([
        fetch(`${API_URL}/api/subcategories/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(
          `${API_URL}/api/products/admin/list?subCategory=${id}&page=${pageNum}&limit=${LIMIT}`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
      ]);

      const subCatJson = await subCatRes.json();
      if (subCatJson.success) setSubCategory(subCatJson.data);
  ...
  const parentName = subCategory?.parent
    ? (typeof subCategory.parent === 'object' ? subCategory.parent.name : '')
    : '';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href={routes.eCommerce.subCategories}>
          ...
          <div>
            ...
            <nav className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-400">
              ...
              <span>/</span>
              <Link href={routes.eCommerce.subCategories} className="hover:text-gray-600">SubCategories</Link>
              {parentName && (
                <>
                  <span>/</span>
        ...
        <div className="flex items-center gap-2">
          <Link href={routes.eCommerce.editSubCategory(id)}>
            <Button variant="outline" size="sm" className="gap-1.5">
              <PiPencilLineBold className="h-4 w-4" />
              Edit SubCategory
            </Button>
          </Link>
          <Link href={`${routes.eCommerce.createProduct}?subCategory=${id}`}>
      ...
      {subCategory && !loading && (
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="rounded-lg border border-gray-200 bg-white px-4 py-2.5">
            <Text className="text-xs text-gray-400">SubCategory Status</Text>
            <span className={cn(
              'inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize',
              subCategory.status === 'published' ? 'bg-green-100 text-green-700' :
              subCategory.status === 'draft' ? 'bg-gray-100 text-gray-600' :
          ...
          {parentName && (
            <div className="rounded-lg border border-gray-200 bg-white px-4 py-2.5">
              <Text className="text-xs text-gray-400">Parent Category</Text>
              <Text className="text-sm font-semibold text-gray-700">{parentName}</Text>
            </div>
          )}
        </div>
      )}
      ...
      {subCategory?.notes && (
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="whitespace-pre-wrap text-sm text-gray-700">{subCategory.notes}</p>
        </div>
      )}
